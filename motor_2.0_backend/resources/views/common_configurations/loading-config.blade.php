...
<div class="card mt-3">
    <div class="card-body">
        <h5 class="card-title text-primary">Loading Amount Configurator</h5>
        @if (session('status') && session('formType') == 'loadingConfig')
        ...
        @endphp
         <form action="{{ route('admin.common-config-save') }}" method="POST" class="mt-3 loadingConfig">
            ...
            <div class="mb-2">
                ...
                <input class="form-check-input" type="checkbox" id="loading-config-apply">
            </div>
           <div class="row">
                @foreach ($icList as $lists)
                    <div class="col-md-6">
                        <table class="table table-striped col-sm-6 loading-table">
                            <thead>
