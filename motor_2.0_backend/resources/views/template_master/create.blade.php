            ...
            <div class="col-sm-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ route('admin.template.store') }}" method="POST">@csrf
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;">
                                ...
                                <div class="col-sm-10">
                                    <div class="dropdown input-group">
                                        <span class="input-group-text" id="dropbtn" onclick="myFunction()" style="cursor: pointer;">↓</span>
                                        <input type="text" class="form-control" placeholder="Alias" aria-label="alias" id="alias" name="alias" readonly value="{{ old('alias') }}">
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;">
                                <label for="communication_type" class="col-sm-2 col-form-label required">Communication Type</label>
                                <div class="col-sm-10">
                                    <input type="text" name="communication_type_email" class="form-control" id="communication_email" readonly>
                                    <select name="communication_type" data-style="btn-sm btn-primary"data-actions-box="true" class="selectpicker w-100 communication_type_hide" data-live-search="true" id="communication_type">
                                        <option value="">Nothing Selected</option>
                                        <option value="email"
                                            {{ old('communication_type') == 'email' ? 'selected' : '' }}>
                                            Email</option>
                                        <option value="sms" {{ old('communication_type') == 'sms' ? 'selected' : '' }}>
                                            SMS</option>
                                        <option value="whatsapp"
                                            {{ old('communication_type') == 'whatsapp' ? 'selected' : '' }}>Whatsapp
                                        </option>
                                    </select>
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;"
                                ...
                                <label for="message_type" class="col-sm-2 col-form-label required">Message type</label>
                                <div class="col-sm-10">
                                    <select name="message_type" data-style="btn-sm btn-primary" data-actions-box="true"
                                        ...
                                        <option value="Plane Text"
                                            {{ old('message_type') == 'Plane Text' ? 'selected' : '' }}>Plane text</option>
                                        <option value="Image" {{ old('message_type') == 'Image' ? 'selected' : '' }}>
                                            Image
                                        </option>
                                    </select>
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;"
                                ...
                                <label for="global_header" class="col-sm-2 col-form-label required">Global Header</label>
                                <div class="col-sm-10">
                                    <textarea name="global_header" id="global_header" placeholder="Global Header" autocomplete="off"
                                        style="height:200px;width:100%">{{ $global_header->content ?? '' }}</textarea>
                                    @error('global_header')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;">
                                ...
                                <div class="col-sm-10">
                                    <textarea name="content" id="content" placeholder="Content" autocomplete="off" style="height:200px;width:100%">{{ old('content') }}</textarea>
                                    @error('content')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;"
                                ...
                                <div class="col-sm-10">
                                    <textarea name="footer" id="footer" placeholder="Footer" autocomplete="off" style="height:200px;width:100%">{{ $global_footer->content ?? '' }}</textarea>
                                    @error('footer')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div id="email_field">
                                <div class="form-group row"
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="to" class="form-control" id="to"
                                            placeholder="To Email" value="{{ old('to') }}">
                                    </div>
                                </div>
                                <div class="form-group row"
                                    style="display: flex; flex-direction: row; align-items: center;">
                                ...
                                <div class="form-group row"
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="bcc" class="form-control" id="bcc"
                                            placeholder="Bcc" value="{{ old('bcc') }}">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group row" style="display: flex; flex-direction: row; align-items: center;">
                                ...
                                <div class="col-sm-2">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="on" {{ old('status') == 'off' ? '' : 'checked' }}>
                                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function aliasOption(alias) {
            ...
            if (alias == 'global_header') {
                $('.ck').remove();
                $('#title').val('{{ $global_header->title ?? '' }}');
                $("#content").html(" ");
                $("#content").html("{{ $global_header->content ?? '' }}");
                $('.communication_type_hide').hide();
                $('#communication_email').show();
                $('#communication_email').val('email');
                ...
                $('.ck').remove();
                $('#title').val('{{ $global_footer->title ?? '' }}');
                $("#content").html(" ");
                $("#content").html("{{ $global_footer->content ?? '' }}");
                $('.communication_type_hide').hide();
                $('#communication_email').show();
                $('#communication_email').val('email');
                ckeditor(autocomplete);
        ...
        function communicationTypeOption(communication_type) {
            ...
            if (($('#alias').val() == 'Global Header' || $('#alias').val() == 'Global Footer') && communication_type == "email") {
                $('#global_header_field').hide();
                $('#footer_field').hide();
                $('#message_type_field').hide();
                $('#email_field').hide();
