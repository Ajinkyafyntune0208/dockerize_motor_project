@extends('admin_lte.layout.app', ['activePage' => 'Product', 'titlePage' => __('Product')])
...
<div class="card">
    <div class="card-body">
        <button id="downloadExcelBtn" class="btn btn-primary mb-4">Download Excel</button>
                            ...
                            <div class="row">
                                <div class="col-md-5 col-lg-3">
                                    <div class="form-group w-100">
                                        <label class = "required" for="broker_url">Product Name</label>
                                        <select name="broker_url" id="broker_url" required
                                ...
                                <div class="col-md-5 col-lg-3">
                                    <div class="form-group w-100">
                                        <label for="view" class = "required">IC Name</label>
                                        <select required name="view" id="view" data-style="btn btn-light"
                                            class="border selectpicker w-100" data-live-search="true">
                                            <option value="">Nothing selected</option>
                                ...
                                <div class="col-md-5 col-lg-3">
                                    <div class="form-group w-100">
                                        <label class = "required" for="premiumType">Premium Type</label>
                                        <select name="premiumType" id="premiumType" required
                                        class="border selectpicker w-100" data-live-search="true">>
                                            <option value="" selected disabled>Nothing selected</option>
                                            @foreach ($premiumType as $item)
                                            <option value="{{$item['id']}}">{{$item['premium_type']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-2" style="padding-top:33px;">
                                        <button type="submit" class="btn btn-info btn w-100">Search</button>
                                </div>
                            </div>
                        </div>
                    @endcan
            ...
            <div class="d-flex flex-row">
                <div class="d-flex align-items-left h-100 col-sm-9">
                    <button type="button" id="nextBtn" class="btn btn-success mx-2"
                        style="border-radius: 40px"><i class="fa fa-arrow-left mr-2"></i> IC Credential</button>
                </div>
                <div class="d-flex justify-content-end align-items-center h-100 col-sm-3">
                    <button type="button" id="newBtn" class="btn btn-success"
                        style="border-radius: 40px">IC Miscellaneous <i class="fa fa-arrow-right ml-2"></i></button>
                </div>
            </div>
        </form>
    </div>
...
function initializeDataTable() {
    if (!dataTableInitialized) {
        $('#policy_reports').DataTable({
            "initComplete": function() {
                var notApplyFilterOnColumn = [5, 6, 7, 8, 9, 10, 12, 13];
        ...
        console.warn("DataTable is already initialized on this table.");
            ...
            $('#policyReportForm').submit(function(event) {
                ...
                let premiumType = $('#premiumType').val();
                        ...
                        if (response.data.length === 0) {
                                ...
                                <table class="table table-striped" id="policy_reports">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col">Is Premium Online</th>
                                            <th scope="col">Is Proposal Online</th>
                                            <th scope="col">Is Payment Online</th>
                           ...
                           let contents;
                            ...
                            $.each(response.data, function(index, report) {
                                ...
                                let isPremiumOnline = `
                                    <input type="checkbox" value="Yes" onchange="changePremiumOnline(this)" ${report.is_premium_online == 'Yes' ? 'checked' : ''}>
                                    <input type="hidden" name="is_premium_online[]" value = '${report.is_premium_online == 'Yes' ? 'Yes' : 'No'}'>`

                                let isProposalOnline = `
                                    <input type="checkbox" value="Yes" onchange="changePremiumOnline(this)" ${report.is_proposal_online == 'Yes' ? 'checked' : ''}>
                                    <input type="hidden" name="is_proposal_online[]" value = '${report.is_proposal_online == 'Yes' ? 'Yes' : 'No'}'>`

                                let isPaymentOnline = `
                                    <input type="checkbox" value="Yes" onchange="changePremiumOnline(this)" ${report.is_payment_online == 'Yes' ? 'checked' : ''}>
                                    <input type="hidden" name="is_payment_online[]" value = '${report.is_payment_online == 'Yes' ? 'Yes' : 'No'}'>`
                                ...
                                var gcvCarrierTypeDropdown = '';
                                   ...
                                   contents += '<tr>' +
                                    ...
                                    selectId +
                                    '" data-style="btn-cus-v2" data-live-search="true" data-actions-box="true" multiple class="selectpicker w-100" required>' +
                                    ...
                                    (report.premium_type_ids ? report
                                        .premium_type_ids : '') + '">' +
                                    '</td>' +
                                    '<td>' + isPremiumOnline + '</td>' +
                                    '<td>' + isProposalOnline + '</td>' +
                                    '<td>' + isPaymentOnline + '</td>' +
                                    '<td>' +
                                        visibilityCheckBox
                                    '</td>' +
                                    '</tr>'
                            ...
                            $('#policy_reports').DataTable({
                                "initComplete": function() {
                                var notApplyFilterOnColumn = [5, 6, 7, 8, 9, 10, 11, 12, 13];
                                var showFilterBox = 'afterHeading';
        ...
        function changePremiumOnline(checkbox) {
            if (checkbox.checked) {
                checkbox.nextElementSibling.value = "Yes"
            } else {
                checkbox.nextElementSibling.value = "No"
            }
        }
         setTimeout(function() {
            $('.alert').fadeOut();
    ...
    <script>
        const ProductList = "{{ route('admin.ic-config.ProductList') }}";
    </script>
@endsection
