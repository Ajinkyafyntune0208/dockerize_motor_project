                            ...
                            <table class="styled-table" align='center' cellspacing=2 cellpadding=1 id="data_table" border=1>
                                ...
                                @foreach ($vahan_services as $key => $vahan_service)
                                    <tr id={{ 'row'.$vahan_service->id }} align='center'>
                                        ...
                                        <td align='center'>
                                            @can('vahan_credentials.edit')
                                            <a class="btn btn-outline-success btn-sm"
                                            title='Edit' id={{ 'edit_button'.$vahan_service->id }} value="Edit" class="edit"
                                            onclick="edit_row('{{ $vahan_service->id }}')"><i class="fas fa-edit"></i></a>
                                            @endcan

                                            @can('vahan_credentials.show')
                                                <a class="btn btn-outline-warning btn-sm"
                                                title='save' id={{ 'save_button'.$vahan_service->id }} value="Save" class="save"
                                                onclick="save_row('{{ $vahan_service->id }}')"><i class="fa fa-solid fa-save"
                                                        aria-hidden="true"></i></a>
                                                        @endcan

                                             @can('vahan_credentials.delete')
                                                <a class="btn btn-outline-danger btn-sm"
                                                title='Delete' value="Delete" class="delete" onclick="delete_row('{{ $vahan_service->id }}')"><i class=" fa fa-solid fa-trash"></i></a>
                                                @endcan
                                        </td>
                                    </tr>
                                @endforeach
