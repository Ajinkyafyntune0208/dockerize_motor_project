@extends('layout.app', ['activePage' => 'user-activity-logs', 'titlePage' => __('User Activity Logs')])
@section('content')
<style>
    .inner-table > th > td {
...
/* Make the modal cover 100% of height and width */
.modal-fullscreen {
    display: flex;
    align-items: center;
...
.inner-table thead, .inner-table tbody, .inner-table tfoot, .inner-table tr, .inner-table td, .inner-table th {
                        ...
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label class="active required" for="operation">Operation</label>
                                    <select name="operation[]" multiple data-style="btn-sm btn-primary" data-actions-box="true" class="selectpicker w-100" required>
                            ...
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <select name="users[]" multiple data-style="btn-sm btn-primary" data-actions-box="true" class="selectpicker w-100" required>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if(count(json_decode($item['new_data'], true)) > 2)
                                                ...
                                                @endphp
                                                <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#show-user-activity-model" onclick="openModal(event, '{{ $route }}' )"><i class="fa fa-eye"></i></button>
                                            @else
...
<script>
    ...
    function openModal(event, src) {
        ...
        $('#show-user-activity-model .modal-body iframe').attr('src', ''); // Need to clear the older content first before loading new page
        $('#show-user-activity-model').modal("show");

        $('#show-user-activity-model .modal-body iframe').attr('src', src);
    }

    function stopSpinner() {
        $('#show-user-activity-model .modal-body iframe').show();
        $('.spinner-border').hide();
        $('.spinner-border').css('position', '');
    }
