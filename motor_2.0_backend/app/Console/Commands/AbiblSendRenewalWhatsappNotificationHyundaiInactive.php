...
use App\Jobs\AbiblRenewalJobWhatsappInactive;

class AbiblSendRenewalWhatsappNotificationHyundaiInactive extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AbiblSendRenewalWhatsappNotificationHyundaiInactive';
    ...
    public function handle()
    {
        ...
        foreach ($renewal_days as $key => $days) {
            $user_product_journeys = UserProductJourney::whereIn('lead_source', $anyCampgain )->with(['user_proposal' => function ($query) {
                ...
                ->orWhereRaw("DATE_FORMAT(STR_TO_DATE(policy_end_date,'%d-%m-%Y'),'%Y-%m-%d') = DATE_SUB( CURDATE(), INTERVAL 1 YEAR ) + INTERVAL {$days} DAY");
            ...
            foreach ($user_product_journeys as $key => $value) {
                ...
                foreach ($value as $data_key => $data_value) {
                    ...
                    AbiblRenewalJobWhatsappInactive::dispatch($trace_id, $queue_value)->onQueue(env('ABIBIL_RENEWAL_WHATSAPP_QUEUE') . $queue_value);
                }
            }
        }
        return 0;
