...
function getRenewalQuote($enquiryId, $requestData, $productData)
{
   ...
   // include_once app_path() . '/Quotes/Car/' . $productData->company_alias . '.php';
    ...
    //https://dtcdocstag.royalsundaram.in/Services/Renewal/ReadPolicy
    ...
    if(isset($response_data['statusCode']) && $response_data['statusCode'] == 'S-0001')
    {
        ...
        if (isset($policyType) && strtolower(str_replace(' ', '_', $policyType)) != $premiumType) {
            return [
                'status' => false,
        ...
        if(isset($response_data['addonCoverages']) && !empty($response_data['addonCoverages']))
        {
            foreach ($response_data['addonCoverages'] as $key => $addonCoverages) {
                ...
                else if($addonCoverages['name'] == 'VehicleReplacementCover')
                {
                    $canTakeInvoicePrice = $addonCoverages['premium'];
                    $additional['return_to_invoice'] = round($addonCoverages['premium']);
                }
        ...
        $NCBProtectorCover = $RoadSideAssistanceCover = $TyreCoverClause = $InvoicePrice =  0;
        ...
        $other_addons= [];
        $VehicleReplacementCover =  0;
        if(isset($response_data['coverages']) && !empty($response_data['coverages']))
        {   
            foreach ($response_data['coverages'] as $key => $coverages) {
                ...
                else if($coverages['name'] == 'RoadSideAssistanceCover')
                {
                ...
                else if($coverages['name'] == 'VPC_CNGLPGforOD')
                {
                    $cng_lpg = $coverages['premium'];
                }
                else if($coverages['name'] == 'VPC_CNG_LPG')
                {
                    $cng_lpg_tp = $coverages['premium'];
                }
            }
        }
        ...
        $addons_list = [
            ...
            'consumables'          => 0,
            'roadSideAssistance'   => round($RoadSideAssistanceCover),
        ];
        ...
        $addons_data = [
            'in_built'   => $in_bult,
            'additional' => $additional,//This addons are restricted for future scope developemnt
            'other'=>[
                'Other' => array_sum($other_addons)
            ]
        ];

        if(array_sum($other_addons) <= 0)
        {
            $addons_data['other'] = [];
        }
        ...
        if($cover_pa_owner_driver_premium > 0)
        {
            $final_payable_amount = round($final_payable_amount - ($cover_pa_owner_driver_premium * 1.18));
        }
