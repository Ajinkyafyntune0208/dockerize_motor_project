...
function getRenewalQuoteIdv($enquiryId, $requestData, $productData)
{
    ...
    if (($carAge > 5) && ($productData->zero_dep == '0'))
    {
        return [
            'premium_amount' => 0,
            'status'         => false,
    ...
    $url = config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_TOKEN_GENERATION_URL_MOTOR');
    ...
    if (!empty($token)) {
        ...
        $response['proposalDetails']['isQuoteDeviation'] == false &&
        $response['proposalDetails']['breakingFlag'] == false &&
        $response['proposalDetails']['isApprovalRequired'] == false
        )
        {
            $proposalDetails    = $response['proposalDetails'];
            ...
            $coverDetails = $response['coverDetails'] ?? null;
            ...
            if($allow_idv_calculation)
            {
                ...
                $token_for_idv_url = config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_TOKEN_GENERATION_URL_MOTOR');
                ...
                $ZeroDepPlanName = !empty($riskDetails['zeroDepreciation']) ? (env('APP_ENV') == 'local' ? 'Silver PVT' : 'ZD') : '';
                $IsEngineProtectPlus = !empty($riskDetails['engineProtect']) ? true : false;
                $KeyProtectPlan = !empty($riskDetails['keyProtect']) ? 'KP1' : '';
                $RSAPlanName = !empty($riskDetails['roadSideAssistance']) ? 'RSA-Standard' : '';
                ...
                $isTyreProtect = !empty($riskDetails['tyreProtect']) ? true : false;
