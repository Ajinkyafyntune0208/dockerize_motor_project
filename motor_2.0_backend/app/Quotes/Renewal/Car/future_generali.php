...
function getRenewalQuote($enquiryId, $requestData, $productData)
{
    ...
    {
        return getRenewalQuoteNew($enquiryId, $requestData, $productData);
    }
    ...
    $policy_data = [
        ...
        "ExpiryDate" => '',
        "RegistrationNo" => '',
        "VendorCode" => 'webnew', #config value
    ];
    ...
    if ($policy_data_response) {
        ...
        if (($quote_policy_output['PremiumBreakup']['Root']['Policy']['Status'] ?? '') != 'Successful') {
            ...
            $total_idv = $quote_output['VehicleIDV'] ?? 0;
            $total_od_premium = 0;
            $total_tp_premium = 0;
            $od_premium = 0;
            $tp_premium = 0;
            ...
            $policy_end_date = $quote_policy_output['Root']['PolicyHeader']['PolicyEndDate'];
            ...
            $applicable_ncb = (int)$quote_policy_output['Root']['Risk']['AdditionalBenefit']['NCB'] ?? 0;
            ...
            $data_response = [
                ...
                'Data' => [
                    ...
                    'policyEndDate' => DateTime::createFromFormat('d/m/Y h:i:s', $policy_end_date)->format('d-m-Y'),
...
function getRenewalQuoteNew($enquiryId, $requestData, $productData)
{
    $vehicle_block_data = DB::table('vehicle_block_data')
        ->where('registration_no', str_replace("-", "", $requestData->vehicle_registration_no))
        ->where('status', 'Active')
    ...
    if ($policy_data_response) {
        ...
        if (($quote_policy_output['Policy']['Status'] ?? '') == 'Fail') {
            ...
            foreach ($quote_output as $key => $cover) {
                ...
                $productByAddonPackage = ['basic', 'FG car tp', 'STZDP','ZDCET','STRSA','RSPBK','STNCB','ZCETR','ZDCNT','ZDCNE','ZDCNS','zd'];
                if (in_array($cover['Code'], $productByAddonPackage) && ($cover['Code'] == $productData->product_identifier)) {
                    if((int)$value == 0){
                        return [
                            'premium_amount' => 0,
                            'status' => false,
                            'message' => $cover['Code'].' Package Is Not Applicable Beacause Some Of Addon Having Zero Value.',
            ...
            $total_od = $od_premium + $electrical_amount + $non_electrical_amount + $lpg_cng_amount;
            $total_tp = $tp_premium + $liability + $legal_liability_to_employee + $pa_unnamed + $lpg_cng_tp_amount + $pa_paidDriver;
            ...
            if($pa_unnamed > 0)
            {
                $data_response['Data']['cover_unnamed_passenger_value'] = (int)$pa_unnamed;
                array_push($included_additional, 'coverUnnamedPassengerValue'); 
            }
            if($liability > 0)
            {
                $data_response['Data']['default_paid_driver'] = (int)$liability;
                array_push($included_additional, 'defaultPaidDriver');
            ...
            if($lpg_cng_tp_amount > 0)
            {
                $data_response['Data']['cng_lpg_tp'] = (int)$lpg_cng_tp_amount;
                array_push($included_additional, 'cngLpgTp');

            }
            if($tppd_discount > 0)
            {
                $data_response['Data']['tppd_discount'] = (int)$tppd_discount;
                array_push($included_additional, 'tppdDiscount'); 
            }
            $data_response['Data']['included_additional']['included'] = $included_additional;
            return camelCase($data_response);
        }
    }
}
