...
class godigit
{
    ...
    {
        if(config('IC.GODIGIT.V2.BIKE.RENEWAL.ENABLE') == 'Y')  return self::getOneApiRenewalData($enquiryId, $value);
        ...
        if (isset($response->error->errorCode) && $response->error->errorCode == '0') {
                ...
                switch ($key) {
                            ...
                            if (($addon->selection == 'true' && $addon->coverAvailability == 'AVAILABLE')) {
                                switch ($key) {
                                    case 'partsDepreciation':
                                        $value['zero_dep'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'roadSideAssistance':
                                        $value['rsa'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'engineProtection':
                                        $value['engine_protector'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'returnToInvoice':
                                        $value['return_to_invoice'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'consumables':
                                        $value['consumable'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'personalBelonging':
                                        $value['loss_of_personal_belonging'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
    
                                    case 'keyAndLockProtect':
                                        $value['key_replacement'] = round(str_replace("INR ", "", ($addon->netPremium ?? '')));
                                        break;
                                }
                            }
                    ...
                    case 'personalAccident';
                        if (isset($cover->selection) && ($cover->selection == 1) && (isset($cover->netPremium))) {
                            $value['cpa_amount'] = round(str_replace("INR ", "", $cover->netPremium));
                        }
                        break;
                    case 'accessories':
                        foreach ($cover as $key => $addon) {
                            if (($addon->selection == 'true' && $addon->coverAvailability == 'AVAILABLE')) {
                                switch ($key) {
                                    case 'cng':
                                        $value['external_bifuel_cng_lpg'] =  round(str_replace("INR ", "", $addon->netPremium));
                                        $value['external_bifuel_cng_lpg_si_amount'] =  $addon->insuredAmount ?? null;
                                        break;
                                    case 'electrical':
                                        $value['electrical'] =  round(str_replace("INR ", "", $addon->netPremium));
                                        $value['electrical_si_amount'] =  $addon->insuredAmount ?? null;
                                        break;
                                    case 'nonElectrical':
                                        $value['not-electrical'] =  round(str_replace("INR ", "", $addon->netPremium));
                            ...
                            if (($addon->selection == 'true' && $addon->coverAvailability == 'AVAILABLE')) {
                                switch ($key) {
                                    case 'unnamedPaidDriver':
                                        $value['pa_cover_for_additional_paid_driver'] =  round(str_replace("INR ", "", $addon->netPremium));
                    ...
                    case 'legalLiability' :
                        foreach ($cover as $cover => $addon) {
                            if ($addon->selection == 1) {
                                switch ($cover) {
                                    case 'paidDriverLL':
                                        $value['ll_paid_driver'] = round(str_replace("INR ", "", $addon->netPremium));
            ...
            $value['nominee_name'] = null;
            $value['nominee_age'] = null;
            $value['relationship_with_nominee'] = null;
    ...
    public static function getOneApiRenewalData($enquiryId, &$value){
        ...
        $fetchUrl = config('IC.GODIGIT.V2.BIKE.END_POINT_URL');
