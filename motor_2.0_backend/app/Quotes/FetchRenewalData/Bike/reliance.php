    ...
    public static function getRenewalData($enquiryId, &$value)
    {
        ...
        if ($response) {
            ...
            if (!isset($response->PolicyDetails->ErrorMessages->ErrMessages)) {
                ...
                $value['financier_name'] = strtoupper($vehicle->FinancierName);
                ...
                $value['productcode'] = $policy->productcode;
                ...
                $value['gender'] = in_array(($clientDetails->Gender ?? ''), ['0', 'M']) ? 'M' : (in_array(($clientDetails->Gender ?? ''), ['1', 'F']) ? 'F' : null);
