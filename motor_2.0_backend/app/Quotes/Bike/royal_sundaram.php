...
function getQuote($enquiryId, $requestData, $productData)
{
    if (config('IC.ROYAL_SUNDARAM.V1.BIKE.ENABLE') == 'Y') {
        include app_path() . '/Quotes/Bike/V1/royal_sundaram.php';
    ...
    //     ];
    // }
    if ($vehicle_age2 > 5 && $productData->zero_dep == 0 && in_array($productData->company_alias, explode(',', config('BIKE_AGE_VALIDASTRION_ALLOWED_IC')))) {
        return [
            'status' => false,
            'premium' => '0',
    ...
    if ($data) {
        ...
        if (isset($premium_response['PREMIUMDETAILS']['Status']['StatusCode']) && $premium_response['PREMIUMDETAILS']['Status']['StatusCode'] == 'S-0002') {
            if (!$tp_only) {
                ...
                $default_idv = $premium_response['PREMIUMDETAILS']['DATA']['MODEL_IDV'];
                ...
                $premium_request['vehicleDetails']['discountIdvPercent'] = round((($premium_request_idv) * 100) / $default_idv) - 100;
            ...
            $ncb_discount = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['NO_CLAIM_BONUS'] ?? 0;
            $electrical_accessories_amt = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ELECTRICAL_ACCESSORIES'];
            $non_electrical_accessories_amt = $non_electrical_accessories == 'Yes' ? $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['NON_ELECTRICAL_ACCESSORIES'] : 0;
            ...
            $underwriting_loading_amt = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['UNDERWRITER_LOADING'] ?? 0;
            ...
            if (!$tp_only) {
                if ($vehicle_age <= 5) {
                    ...
                    $addons_data['additional'] = [
                        ...
                        'return_to_invoice' => $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['INVOICE_PRICE_INSURANCE'],

                    ];
                }
                $addons_data['additional']['road_side_assistance'] = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ROADSIDEASSISSTANCE'];
                $addons_data['additional']['engine_protector'] = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ENGINE_PROTECTOR'];
                $addons_data['additional']['road_side_assistance'] = $premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ROADSIDEASSISSTANCE'];
