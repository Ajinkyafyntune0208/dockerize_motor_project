...
public static function getQuote($enquiryId, $requestData, $productData)
{
    ...
    if ($premium_type == "third_party") {
        $product_code = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_TP");
        $requestData->applicable_ncb = 0;
        $requestData->previous_ncb = 0;
    } else if ($requestData->business_type == 'newbusiness') {
        $product_code = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_NEW_BUSINESS");
    } else if ($premium_type == 'own_damage') {
        $product_code = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_OD");
    } else {
        $product_code = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE");
    }
    ...
    if (trim($mmv_data->ic_version_code) == '') {
        ...
        if(!empty($rto_details)) {
            ...
            if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
                $pUserId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.USERNAME_TP");
            }
            ...
            if ($is_pos_enabled == 'Y')
            {
                ...
                {
                    $extCol40 = $pos_data->pan_no;
                    $pUserId = ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') ? config("IC.BAJAJ_ALLIANZ.V1.BIKE.USERNAME_POS_TP") : config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_NAME_POS");
                }
            ...
            if($is_data_exits_for_checksum['found'] && $refer_webservice && $is_data_exits_for_checksum['status'])
            {
                ...
                if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
                    $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.QUOTE_TP_URL'), $input_array, 'bajaj_allianz', $additional_data);
                } else {
                    $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL'), $input_array, 'bajaj_allianz', $additional_data);
                }
            }
...
//            }
            ...
            if(config('IC.BAJAJ_ALLIANZ.V1.BIKE.IS_RSA_ENABLED') == 'Y')
            {
                array_push($applicable_addons, "roadSideAssistance");
            }
            ...
            $final_od_premium = $basic_od + $ExtraPremiumForRejectedRTO + $non_electrical_accessories + $electrical_accessories; //A
