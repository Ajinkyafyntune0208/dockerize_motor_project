...
function getV1Quote($enquiryId, $requestData, $productData)
{
    ...
    // {
        ...
        }else
        {
            ...
            $token_cache_name = 'IC.EDELWEISS.V1.BIKE.END_POINT_URL_TOKEN_GENERATION' . $enquiryId;  
            ...
            if(empty($token_data) || !isset($token_data['access_token']) ) {
                ...
                if(isset($token_data['access_token']))
                {
                    $service_request_data = [
                            'channelCode'       => '002',
                            'branch'            => config('IC.EDELWEISS.V1.BIKE.EDELWEISS_BRANCH'), //constants.IcConstants.edelweiss.EDELWEISS_BRANCH
                            'make'              => $mmv_data->make,
                ...
                else
                {
                    ...
                    $get_response = getWsData(config('IC.EDELWEISS.V1.BIKE.END_POINT_URL_QUOTE_GENERATION'),$service_request_data, 'edelweiss',
                        [
                            'enquiryId' => $enquiryId,
                            'requestMethod' =>'post',
                            'productName'  => $productData->product_name,
