...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    {
        include_once app_path() . '/Quotes/Bike/V1/future_generali.php';
    ...
    try{
        ...
        // rsa is not applicable for future generali and consumables is not for standalone policy #31453
        if ($bike_age <= 3 &&  !in_array($premium_type, ['own_damage', 'third_party', 'third_party_breakin', 'own_damage_breakin']) && ($productData->zero_dep == 0))
        {
            $AddonReq = 'Y';
            // $addon[] = [
            //     'CoverCode' => 'RODSA',
        ...
        if ($data) 
        {
            ...
            if (isset($quote_output['s:Body']['CreatePolicyResponse']['CreatePolicyResult']['Root']['Policy'])) 
            {
                ...
                if ($quote_output['Status'] == 'Fail') {
                    ...
                    $idv = ($premium_type == 'third_party') ? 0 : ($quote_output['VehicleIDV']);
                    $min_idv = ($idv * 0.9);   //changes as per kit #20121
                    $max_idv = ($idv * 1.1);
                    ...
                    $data_response = [
                        ...
                        'Data' => [
                            ...
                            'bike_electric_accessories_value' =>($electrical_amount),
                            'bike_non_electric_accessories_value' => ($non_electrical_amount),
                            'bike_lpg_cng_kit_value' => ($lpg_cng_amount),
