...
include_once app_path() . '/Quotes/Bike/V1/shriram.php';
...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    if (config('IC.SHRIRAM.V1.BIKE.ENABLE') == 'Y') {
        ...
        return getQuoteV1JSON($enquiryId, $requestData, $productData);
    } 
    elseif (config('constants.motor.shriram.SHRIRAM_BIKE_JSON_REQUEST_TYPE') == 'JSON') {
...
function getQuoteXML($enquiryId, $requestData, $productData)
{
    ...
    if (empty($mmv_data->ic_version_code) || $mmv_data->ic_version_code == '') {
        ...
        if ($bike_age > 4 && $productData->zero_dep == '0' && in_array($productData->company_alias, explode(',', config('BIKE_AGE_VALIDASTRION_ALLOWED_IC')))) {
            return [
                'premium_amount' => 0,
                'status' => false,
        ...
        if ($requestData->business_type== 'newbusiness') {
            ...
            if ($requestData->vehicle_registration_no != '') {
                ...
                if(!empty($requestData->engine_no) && !empty($requestData->chassis_no))
                {   
                    $EngineNo =  $requestData->engine_no;
                    $ChassisNo =  $requestData->chassis_no;

                }else if( !empty($user_proposal) && !empty($user_proposal->engine_number) && !empty($user_proposal->chassis_number))
                {
                    $EngineNo =  $user_proposal->engine_number;
                    $ChassisNo =  $user_proposal->chassis_number;
                }
            } else {
        ...
        //     }else
        //     {
        //         return ['premium_amount' => 0, 'status' => true, 'message' => 'RTO city not available', ];
        //     }
        ...
        $zero_dep = false;
        $without_addon_product = $productData->product_identifier == 'without_addon';
            ...
            "soap:Body" => [
                $soapAction => [
                    "objPolicyEntryETT" => [
                        ...
                        "Address2" => "Address2",
                        "Address3" => "Address3",
                        "State" => 'MH',//explode('-', $rto_code)[0], //$rto_details->state, 
                        "City" => 'Mumbai',//$rto_details->city, 
                        "PinCode" => '400005', //$rto_details->pin_code,
                        "PanNo" => "",
                        'GSTNo' => '',
                        'TelephoneNo' => '',
                        "FaxNo" => "",
                        ...
                        "EngineNo" => $EngineNo,//"6584D218",//Str::upper(Str::random(8)),
                        "FirstRegDt" => date('d/m/Y', strtotime($requestData->vehicle_register_date)), //"10/07/2021", //,
                        "VehicleType" => $BusinessType == "1" ? "W" : "U",
                        "ChassisNo" => $ChassisNo,//"6589311F4SDSA3FFH",//Str::upper(Str::random(12)),
                        ...
                        "PreviousPolicyNo"      => ($BusinessType == "1" || $previous_not_sure) ? "" : "D044353536",
                        "PreviousInsurer"       => ($BusinessType == "1" || $previous_not_sure) ? "" : "Go Digit General Insurance Ltd",
                        ...
                        "PreviousPolicyNCBPerc" => $tp_only ? '0': $previous_ncb,
                        ...
                        "MultiCarBenefitYN"     => "N",
                        "RSACover"              => $tp_only || $without_addon_product ? '0' : '1',
                        "InvReturnYN"           => "N",
       ...
       $temp_data = $data;
       unset($temp_data['soap:Body']['GenerateLTTwoWheelerProposal']['objPolicyEntryETT']['RegNo3'],$temp_data['soap:Body']['GenerateLTTwoWheelerProposal']['objPolicyEntryETT']['RegNo4']);
       ...
       $skip_second_call = false;
        ...
        {
            $quote_response = isset($response['soap:Body']['GenerateLTTwoWheelerProposalResponse']['GenerateLTTwoWheelerProposalResult']) ? $response['soap:Body']['GenerateLTTwoWheelerProposalResponse']['GenerateLTTwoWheelerProposalResult'] : $response['soap:Body']['GenerateProposalResponse']['GenerateProposalResult'];
        }else{
            if (!isset($response['soap:Body']['GenerateLTTwoWheelerProposalResponse']['GenerateLTTwoWheelerProposalResult']) && !isset($response['soap:Body']['GenerateProposalResponse']['GenerateProposalResult'])) {
                return [
                    'webservice_id' => $get_response['webservice_id'],
                    'table' => $get_response['table'],
                    'status' => false,
                    'msg' => 'Invalid response received from the quote calculation API.',
                ];
            }
            $quote_response = isset($response['soap:Body']['GenerateLTTwoWheelerProposalResponse']['GenerateLTTwoWheelerProposalResult']) ? $response['soap:Body']['GenerateLTTwoWheelerProposalResponse']['GenerateLTTwoWheelerProposalResult'] : $response['soap:Body']['GenerateProposalResponse']['GenerateProposalResult'];
        }
        ...
        if ($quote_response['ERROR_CODE'] == 0) {
            ...
            $ll_paid_driver =$engine_protection = $consumables_cover = $return_to_invoice = $loading_amount = $basic_tp_premium = $detariffic_discount = 0;
            ...
            foreach ($quote_response['CoverDtlList']['CoverDtl'] as $key => $value) {
                ...
                if ( in_array($value['CoverDesc'], array('BASIC TP COVER', 'BASIC TP COVER - 1 YEAR','BASIC TP COVER - 2 YEAR','BASIC TP COVER - 3 YEAR','BASIC TP COVER - 4 YEAR','BASIC TP COVER - 5 YEAR','Basic Premium - 1 Year - TP','Basic Premium - 2 Year - TP','Basic Premium - 3 Year - TP','Basic Premium - 4 Year - TP','Basic Premium - 5 Year - TP')) ) {
            ...
            if($add_ons['additional']['road_side_assistance'] > 0 && $productData->product_identifier == 'basic' && !in_array('roadSideAssistance', $applicable_addons)){
            ...
            $data_response = [
                ...
                'Data' => [
                    ...
                    'addonCover' => null,
                    "UnderwritingLoadingAmount" => $loading_amount - $detariffic_discount,
                    'addon_cover_data_get' => '',
                    'rto_decline' => null,
                    'rto_decline_number' => null,
                    'mmv_decline' => null,
                    'mmv_decline_name' => null,
                    'policy_type' => $premium_type == 'third_party' ? 'Third Party' :(($premium_type == "own_damage") ? 'Own Damage' : 'Comprehensive'),#$policy__type,
...
function getQuoteJSON($enquiryId, $requestData, $productData)
{
    ...
    if ($requestData->vehicle_owner_type == 'I' && $premium_type != "own_damage" )
    {
       ...
       if($is_data_exist_for_checksum['found'] && $refer_wbservice && $is_data_exist_for_checksum['status']){
        ...
        if(isset($cpa_tenure))
        {
           $quote_data['objPolicyEntryETT']['PAYEAR'] =  $cpa_tenure;
           $get_response = getWsData($URL, $quote_data, 'shriram', $additional_data);

            if ($requestData->business_type == 'newbusiness' && $cpa_tenure =='5') {
                $quote_data['objPolicyEntryETT']['PAYEAR'] = $cpa_tenure;
                $additional_data['productName'] = $productData->product_name . " ($requestData->business_type)" . " CPA 5 Year";
                $get_response_cpa = getWsData($URL, $quote_data, 'shriram', $additional_data);
        ...
        if (isset($response['MessageResult']['Result']) && $response['MessageResult']['Result'] == 'Success') {
            ...
            if ($response['MessageResult']['Result'] != 'Success') {
                return [
                    ...
                    'premium_amount' => 0,
                    'status' => false,
                    'msg' => isset($quote_response['ERROR_DESC']) ? $quote_response['ERROR_DESC'] : 'Execution Timeout Expired. The timeout period elapsed prior to completion of the operation or the server is not responding.',
                ];
            }
            $idv = $quote_response['VehicleIDV'];
            ...
            foreach ($quote_response['CoverDtlList'] as $key => $value) {
                ...
                else if (in_array($value['CoverDesc'], ['Basic Premium - 2 Year - TP'])) {
                    $tppd = $tppd + (float)($value['Premium']);
                }
                else if (in_array($value['CoverDesc'], ['Basic Premium - 3 Year - TP'])) {
                    $tppd = $tppd + (float)($value['Premium']);
                }
                else if (in_array($value['CoverDesc'], ['Basic Premium - 4 Year - TP'])) {
                    $tppd = $tppd + (float)($value['Premium']);
                }
                else if (in_array($value['CoverDesc'], ['Basic Premium - 5 Year - TP'])) {
                    $tppd = $tppd + (float)($value['Premium']);
                }
            ...
            $final_total_discount = abs($ncb_discount) + abs($other_discount) + abs($voluntary_excess) + abs($tppd_discount) + abs($anti_theft);
            ...
            {
                $basic_od = $basic_od + $Minimum_OD_Loading;     
            }
            ...
            if ($productData->zero_dep == '0')
            {
                ...
                $add_ons = [
                    'in_built' => [ 
                        'zero_depreciation'     => $zero_dep_amount + $NilDepreciationLoading,   
                    ],
                    'additional' => [   
            ...
            $data_response = [
                ...
                'Data' => [
                    ...
                    'tppd_premium_amount' => $tppd, //Basic TP value to be shown according to Shyam
                    'tppd_discount' => $tppd_discount,
            ...
            if ($requestData->business_type == 'newbusiness' && $cpa_tenure=='5') {
                ...
                foreach ($cpa_multiyear['GetQuotResult']['CoverDtlList'] as $key => $value) {
                    if (in_array($value['CoverDesc'], [
                        'GR36A-PA FOR OWNER DRIVER - 5 Year - TP',
                        'GR36A-PA FOR OWNER DRIVER - 4 Year - TP',
