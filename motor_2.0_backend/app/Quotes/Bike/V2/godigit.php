...
function oneApiGetQuote($enquiryId, $requestData, $productData)
{
    ...
    if ($premium_type == "own_damage") {
        $premium_req_array['motorQuickQuote']['previousInsurer']['originalPreviousPolicyType'] = "1OD_5TP";
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['isCurrentThirdPartyPolicyActive'] = true;
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['currentThirdPartyPolicyInsurerCode'] = "";
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['currentThirdPartyPolicyNumber'] = "";
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['currentThirdPartyPolicyStartDateTime'] = Carbon::parse($requestData->vehicle_register_date)->format('Y-m-d');
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['currentThirdPartyPolicyExpiryDateTime'] = Carbon::parse($requestData->vehicle_register_date)->addYear(5)->subDay(1)->format('Y-m-d');
    }
    if ($isPreviousInsurerKnown == "false") {
        $premium_req_array['motorQuickQuote']['previousInsurer']['isPreviousInsurerKnown'] = $isPreviousInsurerKnown;
        ...
        if (isset($response->error->errorCode) && $response->error->errorCode == '0') {
            ...
            if ($requestData->is_idv_changed == 'Y') {
                ...
                switch ($getIdvSetting) {
                    case 'default':
                        $premium_req_array["motorQuickQuote"]['vehicle']['vehicleIDV']['idv'] = $default_idv;
            ...
            $return_to_invoice = 0;
            $consumables = 0;
            $tppd_discount = ($is_tppd && $tp_check) ? (($requestData->business_type == 'newbusiness') ? 250 : 50) : 0;
            $tppd = 0;
            ...
            //Adding a basic product according to git #32643
            $add_ons_data =
                [
                    'in_built'   => [],
                    'additional' => []
