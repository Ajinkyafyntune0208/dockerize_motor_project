...
namespace App\Quotes\Bike\Multiyear;
...
class iciciLombardMultiyear
{
    public function getQuote($enquiryId, $requestData, $productData)
    {
    ...
    foreach($rto_cities as $rto_city)
    {
        ...
        if($rto_data)
        {
            ...
            if (!empty($token)) {
                ...
                if (in_array($policy_type, ['Comprehensive', 'Third Party']) &&  $productData->tenure == 22) {
                    $tenure_year = 2;
                    $model_config_premium['Tenure'] = 2;
                    $model_config_premium['TPTenure'] = 2;
                } elseif (in_array($policy_type, ['Comprehensive', 'Third Party']) && $productData->tenure == 33) {
                    $tenure_year = 3;
                    $model_config_premium['Tenure'] = 3;
                    $model_config_premium['TPTenure'] = 3;
                } elseif (in_array($policy_type, ['Comprehensive', 'Third Party']) && $productData->tenure == 02) {
                    $tenure_year = 2;
                    $model_config_premium['Tenure'] = null;
                    $model_config_premium['TPTenure'] = 2;
                } elseif (in_array($policy_type, ['Comprehensive', 'Third Party']) && $productData->tenure == 03) {
                    $tenure_year = 3;
                    $model_config_premium['Tenure'] = null;
                    $model_config_premium['TPTenure'] = 3;
                }
                $PolicyEndDate = date('Y-m-d', strtotime(date('Y-m-d', strtotime("+$tenure_year year -1 day", strtotime(strtr($PolicyStartDate, ['-' => '']))))));
                $model_config_premium['PolicyEndDate'] = $PolicyEndDate;
                ...
                if ($premium_type != 'third_party' && $enable_idv_service == 'Y') {
                    ...
                    $get_response = cache()->remember('constants.IcConstants.icici_lombard.ICICI_LOMBARD_TOKEN_GENERATION_URL_MOTOR', 60 * 45, function () use ($additionData, $tokenParam) {
                        return $token = getWsData(config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_TOKEN_GENERATION_URL_MOTOR'), http_build_query($tokenParam), 'icici_lombard', $additionData);
                    });
                ...
                if (!empty($data)) {
                    ...
                    $tenure =isset ($arr_premium['generalInformation']['tenure'])?  $arr_premium['generalInformation']['tenure'] : 0;
                    $tptenure =isset ($arr_premium['generalInformation']['tpTenure'])?  $arr_premium['generalInformation']['tpTenure'] : 0;
                    ...
                    $data_response =
                        [
                            ...
                            'Data' => [
                                ...
                                'voluntary_excess' => round($voluntary_deductible),
                                ...
                                'tenure'=>$tenure, // keys for multiyear quote
                                'tp_tenure'=>$tptenure, // keys for multiyear quote
