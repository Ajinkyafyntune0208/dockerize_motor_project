...
use Carbon\Carbon;
include app_path() . '/Quotes/Cv/V2/MISCD/universal_sompo_miscd_v2.php';
include_once app_path() . '/Helpers/CvWebServiceHelper.php';

function getMiscQuote($enquiryId, $requestData, $productData)
{
    if (config('IC.UNIVERSAL_SOMPO.V2.MISCD.ENABLED') == 'Y'){
        return getMiscQuoteV2($enquiryId, $requestData, $productData);
    }
    ...
    if($premium_type == 'breakin') {
        ...
        return  [
            'status' => false,
            'message' => 'Quotes are not available for breakin case',
        ];
    }
    ...
    try {
        ...
        if ($mmv_data->cnvm_fuel_type == "CNG" || $mmv_data->cnvm_fuel_type == "LPG")
        {
        ...
        if ($masterProduct->product_identifier == 'Zero dep' && $usgi_applicable_addons['zero_dep'] == 'True')
        {
        ...
        $covers_data_section = [
            ...
            '6' => [
                ...
                'Applicable' => [
                    ...
                    '@attributes' => [
                        'Name' => 'Applicable',
                        'Value' => $mmv_data->cnvm_fuel_type == 'CNG' ? 'True' : 'False'
                    ],
                ],
                'CoverGroups' => [
                    '@value' => '',
        ...
        //APAction : http://tempuri.org/IService1/commBRIDGEFusionMOTOR
       ...
       /* if($comprehensive_product_response)
        ...
        if ($comprehensive_product_response)
        {
            ...
            if($vehicle_age >= 0 && $vehicle_age < 6)
            {
                $deviation = 0.1;
            }
            elseif($vehicle_age >=6 && $vehicle_age <=10)
            {
                $deviation = 0.05;
            }
            elseif($vehicle_age > 10 && $vehicle_age <=15)
            {
                $deviation = 0.05;
            }
        ...
        $additionData['method'] = 'premiumCalculation';
        $get_response = getWsData(config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_CV_MISC'), $idvChangedQuoteXmlRequest, 'universal_sompo', $additionData);
        ...
        if ($newresponse)
        {
            ...
            $ncb_discount = ($total_od - $anti_theft_device_discount) * $motor_applicable_ncb;
          ...
          //$final_tax = ($total_od + $total_addons - ($ncb_discount + $discount_amount + $anti_theft_device_discount + $voluntary_deductable_amount) + $liability + $lpg_cng_tp_amount + $pa_cover_driver+ $pa_owner) * 0.18 + ($tp_premium - $tppd_discount_amount) * 0.12;
            ...
            if($premium_type !== 'third_party')
            {
                if($productData->zero_dep == '0')
                {
                    ...
                    //     return [
                    //         'status' => false,
                    //         'message' => 'Premium of Zero dep is not available..!',
                    //         'request' => [
                    //             'message' => 'Premium of Zero dep is not available..!',
                    //             'vehicle_age' => $vehicle_age,
                    //             'request' => $quote_array,
            ...
            $data_response = [
                ...
                'Data' => [
                    ...
                    'is_cv_json_kit' => ($requestData->business_type == 'newbusiness') ? FALSE : TRUE,
                    // 'motor_lpg_cng_kit_value' => ($external_lpg_cng_accessories = 'True') ?? 0,
                    // 'cng_lpg_tp' => ($external_lpg_cng_accessories = 'True') ?? 0 
                ]
            ];
            ...
            {
                $data_response['Data']['antitheft_discount'] = (int) $anti_theft_device_discount;
            }
            ...
            if($external_lpg_cng_accessories == 'True')
            {
                $data_response['Data']['cng_lpg_tp'] = 0;
            }
...
function get_usgi_applicable_addons_misc($age,$vehicle_make,$interval,$is_GCV = false)
{
