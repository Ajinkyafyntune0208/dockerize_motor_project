...
function getMiscQuote($enquiryId, $requestData, $productData)
{
    ...
    if (empty($mmv_data->ic_version_code) || $mmv_data->ic_version_code == '') {
        ...
        if ($requestData->business_type == 'newbusiness') {
            ...
            $BusinessType = $requestData->previous_policy_type == 'Not sure' ? '6' : '5';
            $ISNewVehicle = 'false';
        ...
        $vehicle_register_date = explode('-', '01-' . $requestData->manufacture_year);
        $vehicle_in_90_days = 'N';

        if ($productData->product_sub_type_code == 'MISCELLANEOUS-CLASS') {
        ...
        //ic is not providing bi-fuel cover for misc-d
        // $is_bifuel_kit = 'false';
        // $BiFuelKitSi = 0;
        ...
        $premium_req_array = [
            ...
            'Cover'                    => [
                ...
                'IsBiFuelKit'                   => $is_bifuel_kit, //ic is not providing bi-fuel cover for misc-d
        ...
        if ($parent_id == 'GCV' || $parent_id == 'MISC') {
            $premium_req_array['Cover']['IsImt23LampOrTyreTubeOrHeadlightCover'] = 'true';
            ...
            if ($parent_id == 'GCV') {
                ...
                $premium_req_array['Vehicle']['MiscTypeOfVehicleID'] = miscVehiclesSubClasses($mmv_data->veh_sub_type_name);
            }
        }
        ...
        $get_response = getWsData(
            config('constants.IcConstants.reliance.misd.END_POINT_URL_RELIANCE_MOTOR_COVERAGE'),
            $premium_req_array,
            'reliance',
            [
        ...
        if ($coverage_res_data) {
            ...
            if (!isset($coverage_res_data->ErrorMessages)) {
                ...
                if (isset($coverage_res_data->LstAddonCovers)) {
                    ...
                    $get_response = getWsData(
                        config('constants.IcConstants.reliance.misd.END_POINT_URL_RELIANCE_MOTOR_PREMIUM'),
                        $premium_req_array,
                        'reliance',
                        [
                            'root_tag'      => 'PolicyDetails',
                    ...
                    if ($premium_res_data) {
                        ...
                        if (trim($response->ErrorMessages) == '') {
                            ...
                            $idvArray = idvChangedLogicmisc($requestData, $idvArray);
                            ...
                            $voluntary_excess = 0;
                            $loading_amount = 0;
                            $final_od_premium = $basic_od + $electrical_accessories + $non_electrical_accessories  + $GeogExtension_od; //+ $lpg_cng
                            $final_tp_premium = $tppd + $liabilities + $pa_paid_driver + $pa_unnamed + $ll_paid_cleaner + $GeogExtension_tp; //+ $lpg_cng_tp
                            ...
                            $data_response = [
                                ...
                                'Data' => [
                                    ...
                                    'motor_manf_date'           => join('-', $vehicle_register_date),
                                    ...
                                    'add_ons_data' =>   [
                                        'in_built'   => [
                                            // 'road_side_assistance' => 0, // removing RSA  from reliane as IC confirm that RSA is not applicable
                                        ...
                                        'in_built_premium' => 0,
                                        'additional_premium' => (int) $zero_dep_amount + (int) $imt_23
                                    ],
                            ...
                            if ($is_bifuel_kit == 'true') {
                                ...
                                $data_response['Data']['vehicle_lpg_cng_kit_value'] = 0;
                            }
...
function idvChangedLogicmisc($requestData, $idvArray)
{
    $bodyIDV = 0;
    $chassisIDV = 0;
    ...
    if ($requestData->is_chassis_idv_changed ?? 'N' == 'Y') {
        ...
        $chassisIDV = $idvArray['minChassisIDV'];
    }
...
function miscVehiclesSubClasses($subTypeName)
{
    ...
    return $misc_vehicles_sub_classes[strtoupper($subTypeName)] ?? 90;
}
