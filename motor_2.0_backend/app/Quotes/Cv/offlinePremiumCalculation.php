...
{
    // [feature] : Preparing Request for log
    $request_payload = new stdClass();
    $request_payload->transaction_id = customEncrypt($enquiryId);
    $request_payload->company_name = $productData->company_alias;
    $request_payload->fyntune_version_id = $requestData->version_id;
    ...
    $request_payload->mmv_data = $mmv_data;
    $request_payload->rto = $requestData->rto_code;
    $request_payload->claim = $requestData->is_claim;

    if ($productData->company_alias == 'shriram') {
        $versionId = $mmv_data->veh_code;
        $showroomPrice = $mmv_data->exshowroom_price;
        $segmentId = '';
        $versionName = $mmv_data->model_desc;
        $cubicCapacity = $mmv_data->veh_cc;
        $GrosssVehicleWeight = $mmv_data->veh_gvw;
        $carryingCapicity = $mmv_data->veh_seat_cap;
        $fuelType = $mmv_data->fuel;
        $seatingCapacity = (int) ($mmv_data->veh_seat_cap) + 1;
        $modelId = $mmv_data->veh_model;
        $modelName = $mmv_data->model_desc;
        $manfId = $mmv_data->veh_make;
        $manfName = $mmv_data->manf;
        if (isset($mmv_data->veh_remarks) && strtolower($mmv_data->veh_remarks) == 'obsolete') {
            return  [
                'premium_amount' => 0,
                'status' => false,
                'message' => 'Quote not available as Vehicle is Obsolete.',
            ];
        }
    }elseif($productData->company_alias == 'icici_lombard'){
        $versionId = $mmv_data->model_code;
        $showroomPrice = $mmv_data->exshowroom_price;
        $segmentId = '';
        $versionName = $mmv_data->model_name;
        $cubicCapacity = $mmv_data->cubic_capacity;
        $GrosssVehicleWeight = '';
        $carryingCapicity = $mmv_data->carrying_capacity;
        $fuelType = $mmv_data->fuel_type;
        $seatingCapacity = $mmv_data->seating_capacity;
        $modelId = $mmv_data->model_code;
        $modelName = $mmv_data->model_name;
        $manfId = $mmv_data->manf_code;
        $manfName = $mmv_data->manf_name;

        // uw guidlines for icici lombard

        $allowed_rto = DB::table('icici_rto_allowed_cities AS irac')
                        ->join('master_rto as mr', 'mr.rto_name', '=', 'irac.allowed_rto_cities')
        ...
        if(!$allowed_rto){
            return [
                'status' => false,
                'message' => 'Premium are not allowed for this RTO'
            ];
        }
        ...
        if($requestData->is_claim == 'Y'){
            return [
                'status' => false,
                'message' => 'Premium are not allowed because of the ICICI Lombard Guidlines'
            ];
        }
    ...
    $addionalPaidDriver = [];
    $ownerDriver = [];   /* owner driver pa cover */
    $unnamedPassenger = [];
    $eleAccessories = [];
    $nonEleAccessories = [];
    $lpgAndCng = [];
    $antiTheftDisc = [];
    $compulsorypaOwnDriver = [];/* cpa  adddon details */
    ...
    if (!$selectedAddons->isEmpty()) {
        ...
        $request_payload->addons_desc = $selectedAddons[0];
    ...
    $corporate_vehicle_quote_request = CorporateVehiclesQuotesRequest::where('user_product_journey_id', $enquiryId)->first();
    $request_payload->manufacture_year = date('Y', strtotime('01-'.$corporate_vehicle_quote_request->manufacture_year));
    $vehicle_in_days = (strtotime($current_date) - strtotime(date('d-m-Y', strtotime($corporate_vehicle_quote_request->manufacture_year)))) / (60 * 60 * 24);

    $manf_id = $manfId; //$get_response_data->manf_id;
    $model_id = $modelId; // $get_response_data->model_id;
    $version_id =  $versionId; //$get_response_data->version_id;
    $user_edit_showroom_price = $corporate_vehicle_quote_request->edit_idv; //$get_response_data->edit_idv;
    $edit_od_discount = $corporate_vehicle_quote_request->edit_od_discount; //$get_response_data->edit_od_discount;
    $seating_capacity = $seatingCapacity; //$get_response_data->seating_capacity;
    $product_sub_type_id = $requestData->product_sub_type_id; // $get_response_data->product_sub_type_id == '' ? $requestData->product_sub_type_id : $get_response_data->product_sub_type_id;
    $unnamed_person_cover_si = $corporate_vehicle_quote_request->unnamed_person_cover_si; //$get_response_data->unnamed_person_cover_si;
    ...
    $showroom_price = $showroomPrice; //$get_response_data->showroom_price;
    $ex_showroom_price_idv = $corporate_vehicle_quote_request->ex_showroom_price_idv; //$get_response_data->ex_showroom_price_idv;
    $vehicle_registration_no = $corporate_vehicle_quote_request->rto_code; //$get_response_data->rto_code;
    $vehicle_use_id = $request['vehicleUseId'];
    $car_rto_no = $requestData->rto_code;
    $corporate_client_id = $corporate_vehicle_quote_request->corp_id; //$get_response_data->corp_id ?? '';
    ...
    $request_payload->vehicle_age = $car_age;
    $request_payload->vehicle_register_date = $requestData->vehicle_register_date;
    $request_payload->previous_policy_expiry_date = $requestData->previous_policy_expiry_date;

    $motor_depreciation = DB::table('motor_idv')
        ->whereRaw($car_age . " BETWEEN age_min AND age_max")
        ->first();

    $vehicle_idv = $premium_type == 'third_party' || $premium_type == 'third_party_breakin' ? 0 : ((float)$showroom_price) * (1 - ($motor_depreciation->depreciation_rate / 100));

    $request_payload->motor_depriciation_rate = $motor_depreciation->depreciation_rate;
    $request_payload->is_idv_change = $requestData->is_idv_changed;
    ...
    if ($productData->company_alias === 'shriram') {
        $min_idv = ceil($vehicle_idv - ($vehicle_idv * $idvRange->min_idv));
        $max_idv = floor($vehicle_idv + ($vehicle_idv * $idvRange->max_idv));
    } elseif($productData->company_alias === 'icici_lombard') {
        $min_idv = ceil($vehicle_idv - ($vehicle_idv * $idvRange->min_idv));
        $max_idv = floor($vehicle_idv + ($vehicle_idv * $idvRange->max_idv));
    }
    ...
    if ($user_edit_showroom_price != '' && $user_edit_showroom_price != 0 && $premium_type != 'third_party' || $premium_type != 'third_party_breakin') {
    ...
    $vehicle_idv = $min_idv ;
    $idvDepreciation = (1 - $motor_depreciation->depreciation_rate / 100);
    $chassisPrice = ceil($min_idv/$idvDepreciation);

     // idv change condition
     if ($requestData->is_idv_changed == 'Y') {
        if ($max_idv != "" && $requestData->edit_idv >= floor($max_idv)) {
            $vehicle_idv = floor($max_idv);
            $chassisPrice = floor($max_idv/$idvDepreciation);
        } elseif ($min_idv != "" && $requestData->edit_idv <= ceil($min_idv)) {
            $vehicle_idv = ceil($min_idv);
            $chassisPrice = ceil($min_idv/$idvDepreciation);
        } else {
            $vehicle_idv = $requestData->edit_idv;
            $chassisPrice = ceil($requestData->edit_idv/$idvDepreciation);
        }
    }

    $request_payload->vehicle_idv = $vehicle_idv;
    $request_payload->chassis_price = $chassisPrice;

    $claim = $requestData->is_claim ?? 'N';

    if(((($eleAccessories['addonSI'] > ($vehicle_idv * 1.2)))) || ((($nonEleAccessories['addonSI'] > ($vehicle_idv * 1.2)))) ) {
        return [
            'premium_amount' => 0,
            'status'         => false,
            'message'        => 'Accessories sum assured should be maximum upto 20% of IDV.'
        ];
    }

    $request_payload->previous_ncb = $corporate_vehicle_quote_request->previous_ncb;
    $request_payload->applicable_ncb = $corporate_vehicle_quote_request->applicable_ncb;
    $ncb_discount = $corporate_vehicle_quote_request->applicable_ncb; //$get_response_data->applicable_ncb;
    $vehicle_in_90_days = 0;

    if ($claim == 'Yes' || $premium_type == 'third_party' || $premium_type == 'third_party_breakin') {
        $ncb_discount = 0;
    } elseif ($policy_type == "N") {
        $vehicle_registration_no = $policy_type . "_" . $corporate_vehicle_quote_request->rto_code; //$get_response_data->rto_code;
        $policyStartDate = date('d-m-Y');
        $ncb_discount = 0;
    } else if ($policy_type == "R") {
        $policyStartDate = $requestData->previous_policy_expiry_date; //$get_response_data->previous_policy_expiry_date;
    ...
    $rto_zone_details = getRto((isset($request['stateId']) ? $request['stateId'] : ''), (isset($request['stateCode']) ? $request['stateCode'] : ''), (isset($requestData->rto_code) ? $requestData->rto_code : $car_rto_no), (isset($request['rtoId']) ? $request['rtoId'] : ''));
    $zone_id = count($rto_zone_details) == 0 ? 0 : $rto_zone_details[0]->zone_id;

    $vehicle_cc = $cubicCapacity; // $get_response_data->cubic_capacity;
    $fuel_type = $fuelType; //$get_response_data->fuel_type;
    ...
    if(!isset($get_res_premium->rate)){
        return [
            'status' => false,
            'message' => 'Premium are not availabe for this '.$car_age.' age'
        ];
    }
    ...
    $electric_user_value = (isset($eleAccessories['addonSI']) && $eleAccessories['addonSI'] != '' || $eleAccessories['addonSI'] != '0') ? $eleAccessories['addonSI'] : 0;
    $electric_premium_value = $electric_user_value * ($lpgcng_det->electrical / 100);
    ...
    $user_id = $requestData->user_id; //$get_response_data->user_id;

    $mmv_detail = [
        'version_id'            => $versionId, //$get_response_data->version_id,
        'version_name'          => $versionName, //$get_response_data->version_name,
        'cubic_capacity'        => $cubicCapacity, //$get_response_data->cubic_capacity,
        'carrying_capicity'     => $carryingCapicity, //$get_response_data->carrying_capicity,
        'Gross_Vehicle_Weight'  => $GrosssVehicleWeight, //$get_response_data->Grosss_Vehicle_Weight,
        'showroom_price'        => $showroomPrice, //$get_response_data->showroom_price,
        'fuel_type'             => $fuelType, //$get_response_data->fuel_type,
        'seating_capacity'      => $seatingCapacity, //$get_response_data->seating_capacity,
        'model_id'              => $modelId, //$get_response_data->model_id,
        'model_name'            => $modelName, //$get_response_data->model_name,
        'manf_id'               => $manfId, //$get_response_data->manf_id,
        'manf_name'             => $manfName, //$get_response_data->manf_name,
    ];
    ...
    foreach ($master_policy_id as $master_policy_id_key => $master_policy_id_value) {
        ...
        $chk_brek_in_exp_date = implode('', explode('-', date('Y-m-d', strtotime($requestData->previous_policy_expiry_date))));
        ...
        if ($master_policy_id_value->is_premium_online == "Yes" && $master_policy_id_value->insurance_company_id == '39' && $chck_is_breakin != 'Yes') {
            ...
            if ($premium_type == 'third_party' || $premium_type == 'third_party') {
                $motor_lpg_cng_kit_value = 0;
            }
            ...
            if (!$vehicle_discount_detail->isEmpty()) {
                ...
                $vehicle_discount_details = getDealerDiscount($vehicleDiscountValues, $enquiryId, $corporate_vehicle_quote_request);
                if($vehicle_discount_details)
                $vehicle_discount_detail = $vehicle_discount_details;
                /* For Change Discount rate End*/
                $edit_od_discount = 0;
                if ($edit_od_discount > 0) {
                    if ($corporate_vehicle_quote_request->is_od_discount_applicable == 'Y' && $master_policy_id_value->insurance_company_id == '33') {
                    ...
                    if ($corporate_vehicle_quote_request->is_od_discount_applicable == 'Y' && $master_policy_id_value->insurance_company_id == '33') {
                        $ic_vehicle_discount = ($vehicle_discount_detail[0]->discount_rate * ($basic_premium + $nonelectric_premium_value + $electric_premium_value)) / 100;
                    } else {
                        $ic_vehicle_discount = ($vehicle_discount_detail[0]->discount_rate * ($basic_premium + $nonelectric_premium_value)) / 100;
                    }
                }
                $basic_premium = $premium_type == 'third_party' || $premium_type == 'third_party_breakin' ? 0 : $basic_premium - $ic_vehicle_discount;
...
// dd($total_own_damage);
            ...
            $motor_electric_accessories_value = $premium_type == 'third_party' ||$premium_type == 'third_party_breakin' ? 0 : $electric_premium_value;
            $motor_non_electric_accessories_value = $premium_type == 'third_party' ||$premium_type == 'third_party_breakin' ? 0 : $nonelectric_premium_value; //=$nonelectric_user_value
            ...
            if (!empty($antiTheftDisc) && $antiTheftDisc['status'] == true) {
                ...
                if ($productData->company_alias === 'shriram') {
                    $db_antitheft = 0; // Disabled for ola (shriram)
                }
            ...
            $cover_unnamed_passenger_value = 0;
            if (isset($unnamedPassenger['addonSI']) &&  $unnamedPassenger['addonSI'] != '' && $premium_type == 'third_party' || $premium_type == 'third_party_breakin') {
                $cover_array = [
                    'product_sub_type_id' => $product_sub_type_id,
                    'ic_id' => 0,
                ...
                if ($cover_detail != '' && isset($cover_detail[0]->base_si) > 0) {
                    ...
                    $cover_unnamed_passenger_value = $cover_si * $cover_detail[0]->cover_rate * $seatingCapacity; //$get_response_data->seating_capacity;
                }
            }
            ...
            if (isset($request['addonCover']) && $request['addonCover'] != "") {
                $cover_array = [
                    'policy_id' => $master_policy_id_value->policy_id,
                    //'registrationdate' => $db_manf_date[2] . '-' . $db_manf_date[1] . '-' . $db_manf_date[0],
                    'registrationdate' => date('Y-m-d', strtotime($corporate_vehicle_quote_request->vehicle_register_date)),
                    'vehicle_cc' => $vehicle_cc,
                    'version_id' => $version_id,
                ...
                if($compulsory_addon_cover_detail->where('seller_type', 'Dealer')->count() > 0)
                $addon_cover_detail = '';
                        ...
                        if ($current_addon_item && isset($current_addon_item->addon_cover_apply_on)) {
                            if (isset($current_addon_item->abs_rate_flag) && $current_addon_item->abs_rate_flag == 'absolute') {
                                ...
                                if (isset($current_addon_item->addon_cover_apply_on) && $current_addon_item->addon_cover_apply_on == 1) {
                                    // For shriram we need to add Elec, Non-Elec, External CNG/Bifuel Kit value to vehicle IDV - Amit 02-09-2021
                                    if ($productData->company_alias === 'shriram') {
                                        $total_idv = $vehicle_idv + ($electric_user_value + $nonelectric_user_value + $lpgAndCng['addonSI']);
                                        $addonval = (($current_addon_item->min_rate * $total_idv) / 100); // vehicle IDV
                                    }else{
            ...
            $current_addon_item = '';
            $selected_addon_inbuilt = $selected_addon_addional = [];
            $key = 0;
            foreach ($compulsory_addon_cover_detail as $addon_cover_detail_key => $addon_cover_detail_item) {
                $current_addon_item = $addon_cover_detail_item;
                if ($current_addon_item && isset($current_addon_item->addon_cover_apply_on)) {
                    if (isset($current_addon_item->abs_rate_flag) && $current_addon_item->abs_rate_flag == 'absolute') {
                        if($key == 0 && isset($addon_cover_detail_item->addon_type) && $addon_cover_detail_item->addon_type == "bunddled"){
                            $addonval = $current_addon_item->min_rate;
                        }
                        else if(isset($addon_cover_detail_item->addon_type) && $addon_cover_detail_item->addon_type == "bunddled"){
                            $addonval = 0;
                        }
                        else{
                            $addonval = $current_addon_item->min_rate;
                        ...
                        //FOR APPLY ON VEHICLE IDV
                        if (isset($current_addon_item->addon_cover_apply_on) && $current_addon_item->addon_cover_apply_on == 1 || $current_addon_item->addon_cover_apply_on == "IDV") {
                            $addonval = (($current_addon_item->min_rate * $vehicle_idv) / 100); // vehicle IDV

                        } elseif (isset($current_addon_item->addon_cover_apply_on) && $current_addon_item->addon_cover_apply_on == 3 || $current_addon_item->addon_cover_apply_on == "Ex-Showroom Price") {
                            $addonval = (($current_addon_item['min_rate'] * $request->showroom_price) / 100); // showroom price
                        }
                        if($key == 0 && isset($addon_cover_detail_item->addon_type) && $addon_cover_detail_item->addon_type == "bunddled"){
                            $addonval = $addonval;
                    ...
                    $addon_premium = $addon_premium + $addonval;
                    $addonAmount = round($addonval);
                    if(isset($addon_cover_detail_item->addon_type) &&$addon_cover_detail_item->addon_type == "bunddled"){
                        $selected_addon_inbuilt = array_merge($selected_addon_inbuilt, [$current_addon_item->addon_name => $addonAmount]);
                        $selected_addon_addional = array_merge($selected_addon_inbuilt, [$current_addon_item->addon_name => $addonAmount]);
                        // array_push($selected_addon, [$current_addon_item->addon_name => $addonAmount]);

                    }else{
                        $selected_addon = array_merge($selected_addon, [$current_addon_item->addon_name => $addonAmount]);
                        $selected_addon_addional = array_merge($selected_addon, [$current_addon_item->addon_name => $addonAmount]);
                    }
            ...
            $get_response1['seating_capacity'] = $seatingCapacity; //$get_response_data->seating_capacity;
    ...
    // policy Start date
    // $Policy_Start_Date = $requestData->policy_type == 'newbusiness' ? Carbon::today() : Carbon::createFromDate($requestData->previous_policy_expiry_date)->addDay(1);


    switch ($requestData->business_type) {
        case 'rollover':
            $business_type = 'Rollover';
            $Policy_Start_Date = Carbon::createFromDate($requestData->previous_policy_expiry_date)->addDay(1);
            $policy_End_Date = Carbon::createFromDate($requestData->previous_policy_expiry_date);
            break;
        case 'newbusiness':
            $business_type = 'New Business';
            $Policy_Start_Date = Carbon::today();
            $policy_End_Date = Carbon::today()->addYear(1)->subDay(1);
            break;
        case 'breakin':
            $business_type = 'Break-in';
            $Policy_Start_Date = Carbon::today()->addDay(3);
            $policy_End_Date = Carbon::today()->addYear(1)->addDay(3)->subDay(1);
            break;
        default:
            $business_type = '';
            $Policy_Start_Date = '';
    ...
    $request_payload->business_type     = $business_type;
    $request_payload->policy_start_date = $Policy_Start_Date->toDateString();
    $request_payload->policy_End_Date   = $policy_End_Date->toDateString();

    if (!empty($final_response)) {

        $tppd_cover_amount = ($productData->company_alias === 'shriram' && $tppd_cover == 'true') ? 150 : 0;
        $basicPremium = $premium_type == 'third_party' || $premium_type == 'third_party_breakin' ? 0 : ($final_response[0]['basic_premium'] + $final_response[0]['vehicleDiscountValues']['ic_vehicle_discount']);
        // dd($basicPremium, $premium_type,$final_response[0]['basic_premium'], $final_response[0]['vehicleDiscountValues']['ic_vehicle_discount']);
        $final_od_premium = $premium_type == 'third_party' ||$premium_type == 'third_party_breakin' ? 0 : ($final_response[0]['basic_premium'] + $final_response[0]['vehicleDiscountValues']['ic_vehicle_discount'] + $final_response[0]['motor_electric_accessories_value'] + $final_response[0]['motor_non_electric_accessories_value'] + $final_response[0]['motor_lpg_cng_kit_value']);
        ...
        $final_total_discount = ($final_response[0]['vehicleDiscountValues']['ic_vehicle_discount'] + $final_response[0]['deduction_of_ncb']) + $tppd_cover_amount;
        ...
        $data_response = array(
            ...
            'data' => [
                ...
                'min_idv' => $min_idv,
                'max_idv' => $max_idv,
                'pp_enddate' => $final_response[0]['master_policy_id']->policy_end_date ?? null,
                'addon_cover_data_get' => '',
                'policy_type' => $premium_type == 'third_party' ||$premium_type == 'third_party_breakin' ? 'Third Party' : 'Comprehensive',
                'cover_type' => '1YC',
                'vehicle_registration_no' => $requestData->rto_code,
                'voluntary_excess' => 0, //$requestData->voluntary_excess_value,
                ...
                'showroom_price' => $chassisPrice,
                'fuel_type' => $requestData->fuel_type,
                'vehicle_idv' => $vehicle_idv,
                'ncb_discount' => $premium_type == 'third_party' ||$premium_type == 'third_party_breakin' ? 0 : $requestData->applicable_ncb,
                ...
                'master_policy_id' => array(
                    ...
                    'policy_start_date' => $final_response[0]['master_policy_id']->policy_start_date ?? null,
                    'policy_end_date' => $final_response[0]['master_policy_id']->policy_end_date ?? null,
                    'sum_insured' => $final_response[0]['master_policy_id']->sum_insured ?? null,
                    'corp_client_id' => $final_response[0]['master_policy_id']->corp_client_id ?? null,
                ...
                'basic_premium' => round($basicPremium),
                ...
                'motor_lpg_cng_kit_value' => ($premium_type == 'third_party' ||$premium_type == 'third_party_breakin') ? 0 : $final_response[0]['motor_lpg_cng_kit_value'],
                'cover_unnamed_passenger_value' => ($premium_type == 'third_party' || $premium_type == 'third_party_breakin' && $productData->company_alias === 'icici_lombard') ? 0 : $final_response[0]['cover_unnamed_passenger_value'],
                ...
                'policyStartDate' => Carbon::parse($Policy_Start_Date->toDateString())->format('d-m-Y'),
                'policyEndDate' => Carbon::parse($policy_End_Date->toDateString())->format('d-m-Y'),
                'ic_of' => $final_response[0]['master_policy_id']->insurance_company_id,
                'vehicle_in_90_days' => $vehicle_in_90_days,
                'get_policy_expiry_date' => NULL,
                'get_changed_discount_quoteid' => 0,
                'tppd_discount' => $tppd_cover_amount,
    ...
    $data_response['data']['add_ons_data']['in_built'] = $selected_addon_inbuilt ?? [] ;
    $data_response['data']['add_ons_data']['additional'] = $selected_addon_addional ?? [] ;

    if (!empty($final_response[0]['selected_addon'])) {
        foreach ($final_response[0]['selected_addon'] as $addons_key => $addons) {

            if ($productData->company_alias === 'icici_lombard') {
                if (isset($addons['Road Side Assistance'])) {
                    $data_response['data']['add_ons_data']['additional']['road_side_assistance'] = (int) $addons['Road Side Assistance'];
                }

                if (isset($addons['Zero Depreciation'])) {
                    $data_response['data']['add_ons_data']['additional']['zero_depreciation'] = (int)  $addons['Zero Depreciation'];
                }
            }
    ...
    if ($productData->company_alias === 'icici_lombard') {
         array_splice($data_response['data']['applicable_addons'], array_search('zeroDepreciation', $data_response['data']['applicable_addons']), 1);
        if (isset($data_response['data']['add_ons_data']['additional']['road_side_assistance']) && $data_response['data']['add_ons_data']['additional']['road_side_assistance'] == 0) {
            array_splice($data_response['data']['add_ons_data']['additional'], array_search('road_side_assistance', $data_response['data']['add_ons_data']['in_built']), 1);
            array_splice($data_response['data']['applicable_addons'], array_search('roadSideAssistance', $data_response['data']['applicable_addons']), 1);
        }
        if (isset($data_response['data']['add_ons_data']['additional']['zero_depreciation']) && $data_response['data']['add_ons_data']['additional']['zero_depreciation'] == 0) {
            array_splice($data_response['data']['add_ons_data']['additional'], array_search('zero_depreciation', $data_response['data']['add_ons_data']['additional']), 1);
        }
    }elseif ($productData->company_alias === 'shriram') {
        if (isset($data_response['data']['add_ons_data']['additional']['zero_depreciation']) && $data_response['data']['add_ons_data']['additional']['zero_depreciation'] == 0) {
            unset($data_response['data']['add_ons_data']['additional']['zero_depreciation']);
            unset($data_response['data']['applicable_addons']['zeroDepreciation']);
            //array_splice($data_response['data']['add_ons_data']['additional'], array_search('zero_depreciation', $data_response['data']['add_ons_data']['additional']), 1);
            //array_splice($data_response['data']['applicable_addons'], array_search('zeroDepreciation', $data_response['data']['applicable_addons']), 1);
        }
        if (isset($data_response['data']['add_ons_data']['additional']['road_side_assistance']) && $data_response['data']['add_ons_data']['additional']['road_side_assistance'] == 0) {
            unset($data_response['data']['add_ons_data']['additional']['road_side_assistance']);
            unset($data_response['data']['applicable_addons']['roadSideAssistance']);
            //array_splice($data_response['data']['add_ons_data']['additional'], array_search('road_side_assistance', $data_response['data']['add_ons_data']['additional']), 1);
            //array_splice($data_response['data']['applicable_addons'], array_search('roadSideAssistance', $data_response['data']['applicable_addons']), 1);
        }
    }

    requestResponse($request_payload, $data_response);
...
function getDealerDiscount($request, $enquiryId, $corporate_vehicle_quote_request)
{
    $agent_details = CvAgentMapping::where('user_product_journey_id', $enquiryId)->first();
    
    if (isset($agent_details->seller_type) && $agent_details->seller_type != 'Dealer')
    return false;
    if($agent_details == null)
    return false;
    $dealer_discount_ids = \Illuminate\Support\Facades\DB::table('oem_discount_dealear')->where('oem_dealer_id', $agent_details->agent_id)->get('oem_discount_id')->pluck('oem_discount_id')->toArray();

    $dealer_discounts = \Illuminate\Support\Facades\DB::table('oem_discounts')->whereIn('id', $dealer_discount_ids);

    $rto_state_data = \Illuminate\Support\Facades\DB::table('master_rto')->where('rto_code', $corporate_vehicle_quote_request->rto_code)->join('master_state', 'master_rto.state_id', '=', 'master_state.state_id')->first();
    
    if($dealer_discountss = $dealer_discounts->where('rto_cluster_type', 'Pan India')->max('discount_rate')){
        $data = new stdClass();
        $data->discount_rate = (int) $dealer_discountss;
        return collect([
            $data
        ]);
    } 
    if($dealer_discountss = $dealer_discounts->where('rto_cluster_type', 'State')->where('rto_cluster_data', 'Like', '%'. $rto_state_data->state_name . '%')->max('discount_rate')){
        $data = new stdClass();
        $data->discount_rate = (int) $dealer_discountss;
        return collect([
            $data
    ...
    if($dealer_discountss = $dealer_discounts->where('rto_cluster_type', 'RTO')->where('rto_cluster_data', 'Like', '%'. $rto_state_data->rto_code . '%')->max('discount_rate')){
        $data = new stdClass();
        $data->discount_rate = (int) $dealer_discountss;
        return collect([
            $data
...
{
    $agent_details = CvAgentMapping::where('user_product_journey_id', $request['enquiry_id'])->first();
    if($agent_details && $agent_details->seller_type == 'Dealer'){
        $applicable_addon_list = DB::table('applicable_addons')->where('oem_product_id', $request['policy_id'])->get();
        $data = collect();
        foreach ($applicable_addon_list as $key => $value) {
            $applicable_addon_id = $value->id;
            
            $addon_ids = \Illuminate\Support\Facades\DB::table('applicable_addon_addon_list')->where('applicable_addon_id', $applicable_addon_id)->get(['addon_id'])->pluck('addon_id');
            $addon_list = \Illuminate\Support\Facades\DB::table('addon_lists')->whereIn('id', $addon_ids)->get();
            // dd($value);
            foreach ($addon_list as $key => $value1) {
                $data->push([
                    'min_rate' => $value->cover_rate,
                    'max_rate' => $value->cover_rate,
                    'addon_name' => $value1->name,
                    'cover_name' => $value1->id,
                    'abs_rate_flag' => $value->cover_rate_type,
                    'addon_cover_apply_on' => $value->cover_apply_on,
                    'addon_id' => $value->id,
                    'addon_type' => $value->addon_type,
                    'seller_type' => $agent_details->seller_type
                ]);
            }
        }
        // dd(collect(json_decode(json_encode($data), false)));
...
function get_addon_cover($request)
{
    ...
    // Below code commented bcs we're not considering the date(d) while calculating the months difference. - Amit Gupta
    //$policy_expiry = date('d-m-Y', strtotime($request['policy_expiry_date']));
    //$veh_age = ((date('Y', strtotime($policy_expiry)) - date('Y', strtotime($request['registrationdate']))) * 12) + (date('m', strtotime($policy_expiry)) - date('m', strtotime($request['registrationdate'])));
    $d1 = new DateTime(date('Y-m-d', strtotime($request['policy_expiry_date'])));
    $d2 = new DateTime(date('Y-m-d', strtotime($request['registrationdate'])));
    $interval = $d2->diff($d1);
    $veh_age = ($interval->y * 12) + $interval->m + ($interval->d > 0 ? 1 : 0);
...
function getAddon($addon_id, $addon_name)
{
    ...
    $getvoluntarydeductible = $getvoluntarydeductible->get()->toArray();
...
function getTariffPccv($request)
{
    ...
    $getTariffPccv = DB::table('pccv_tariff')
        ...
        ->select('*')
        ->get();
    if($getTariffPccv->isEmpty()){
        return $getTariffPccv;
    }

    return $getTariffPccv[0];
}

function requestResponse($payload, $response)
{
   create_webservice([
        'enquiryId' => customDecrypt($payload->transaction_id),
        'productName' => 'Commercial Insurance',
        'transaction_type' => 'CV Insurance',
        'section' => 'CV',
        'method' => 'Quote',
        'companyAlias' => $payload->company_name,
        'request' => $payload,
        'response' => $response
    ]);
}
