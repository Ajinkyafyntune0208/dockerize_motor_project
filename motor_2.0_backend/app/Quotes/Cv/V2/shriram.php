<?php
// namespace App\Quotes\Cv\V2\shriram;
...
function getJSONPCVGCVQuote($enquiryId, $requestData, $productData)
{
...
//     }
    ...
    
    {
        ...
        $is_not_sure = $requestData->previous_policy_type == 'Not sure';
        $break_in = 'N';   
    ...
    if($requestData->vehicle_owner_type = 'I'  ){
        ...
        $PAOwnerDriverExReason = "";
            ...
            if($no_of_conductorLL > 0){
                $no_of_cleanerLL = $no_of_conductorLL;
            }
        ...
        $vehicleClass = "CLASS_4A1"; //GCCV-PUBLIC CARRIERS OTHER THAN THREE WHEELERS
            ...
            $inputArray = [
                "objPolicyEntryETT" => [
                    ...
                    "DeTariff" => 0,
                    "IMT23YN" => "",
                    "BreakIn" => $break_in, // prev insu expiry date and today date
                    ...
                    "RSACover" => $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y", // Roadside assistance
                    "HypothecationType" => '',
        ...
        $imt23yn = 0;
            if ($is_pcv) {
                ...
                $inputArray['objPolicyEntryETT']['IMT23'] = $imt23yn;
            }
        if($is_gcv){
            $inputArray['objPolicyEntryETT']['ProdCode'] = 'MOT-PRD-003';
            $inputArray['objPolicyEntryETT']['AadharEnrollNo'] = "";
            $inputArray['objPolicyEntryETT']['AadharNo'] = "";
            $inputArray['objPolicyEntryETT']['AddonPackage'] = "";
            $inputArray['objPolicyEntryETT']['Consumables'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['DailyExpRemYN'] = "Y";
            $inputArray['objPolicyEntryETT']['DriverAgeYN'] = "";
            $inputArray['objPolicyEntryETT']['EmergencyTranHotelExpRemYN'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['Eng_Protector'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['IMT23'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? '0' : $imt_23;
            $inputArray['objPolicyEntryETT']['InBuiltCNGKitYN'] = "0";
            $inputArray['objPolicyEntryETT']['KeyReplacementYN'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['LimitOwnPremiseYN'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : $LimitOwnPremiseYN;
            $inputArray['objPolicyEntryETT']['LossOfPersonBelongYN'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['MISPDealerCode'] = $MISPDealerCode;
            $inputArray['objPolicyEntryETT']['MultiCarBenefitYN'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : "Y";
            $inputArray['objPolicyEntryETT']['tpPolAddr'] = "";
            $inputArray['objPolicyEntryETT']['tpPolComp'] = "";
            $inputArray['objPolicyEntryETT']['tpPolFmdt'] = "";
            $inputArray['objPolicyEntryETT']['tpPolNo'] = "";
            $inputArray['objPolicyEntryETT']['tpPolTodt'] = "";
            $inputArray['objPolicyEntryETT']['LLtoCleanerYN'] = $no_of_cleanerLL;
            $inputArray['objPolicyEntryETT']['LLtoConductorYN'] = $no_of_conductorLL;
            // unset($inputArray['objPolicyEntryETT']['IMT23YN']);
            unset($inputArray['objPolicyEntryETT']['PCCVVehType']);
            unset($inputArray['objPolicyEntryETT']['IMT23']);
            ...
            $inputArray['objPolicyEntryETT']['RSACover'] = $productData->product_identifier == 'BASIC_WITHOUT_ADDONS' ? 'N' : 'Y';
        }
            ...
            if ($response['MessageResult']['Result'] == 'Success')
            {
                ...
                foreach ($response['GetQuotResult']['CoverDtlList'] as $key => $value)
                {
                    ...
                    if (in_array($value['CoverDesc'], array('Minimum OD Loading', 'Minimum OD Loading - OD'))) {
                        $minimumOdLoading = $value['Premium'];
                    }

                    if (in_array($value['CoverDesc'], ['NCB Discount  - OD','NCB Discount - OD'])) {
                ...
                if ($minimumOdLoading > 0) {
                    $basic_od = $basic_od + $minimumOdLoading;
                }

                $final_od_premium = $basic_od + $non_electrical_accessories + $electrical_accessories + $lpg_cng + $geoextensionod;
                // $ncb_discount = ($final_od_premium - $other_discount) * ($requestData->applicable_ncb / 100); //We are getting NCB in response
                $final_tp_premium = $basic_tp + $lpg_cng_tp + $pa_unnamed +  $pa_paid_driver + $ll_paid_driver + $geoextensiontp + $ll_conductor + $ll_cleaner;
                $final_total_discount = $ncb_discount + $other_discount + $limited_to_own_premises + $tppd_discount + abs($anti_theft);
              ...
              if($is_gcv){
               ...
               elseif($productData->product_identifier == 'IMT-23'  && $imt_23 == 0)
                    ...
                    'Data' => [
                       ...
                       'master_policy_id' => [
                        ...
                        'add_ons_data' => [
                            ...
                            'in_built_premium'   => 0,
                            'additional_premium' => array_sum( [$rsapremium]),//, $imt_23] ),
                            'other_premium'      => 0,
                        ],
                ...
                if ($productData->zero_dep == 0 && $zero_dep_amount > 0 && $is_pcv)
                {   
                    $data_response['Data']['add_ons_data']['in_built']['zero_depreciation'] = round($zero_dep_amount);    
                    array_push($applicable_addons, "zeroDepreciation");   
                }
                
                if ($productData->zero_dep == 0 && $zero_dep_amount > 0 && $is_pcv && $veh_category == '3W')
                {   
                    $data_response['Data']['add_ons_data']['in_built']['zero_depreciation'] = round($zero_dep_amount);   
                ...
                if ($productData->zero_dep == 0 && $zero_dep_amount > 0 && $is_gcv)
                {
