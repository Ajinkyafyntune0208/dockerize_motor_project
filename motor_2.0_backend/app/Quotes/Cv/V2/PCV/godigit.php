...
function oneApigetQuote($enquiryId, $requestData, $productData)
{
    ...
    $date_difference = get_date_diff('day', $date2->format('d-m-Y'));
    ...
    if (!empty($additional['additional_covers'])) {
        ...
        //commenting as per git 34453
        // if (!empty ($cover_pa_paid_driver) && $cover_pa_paid_driver != 200000) {
        //     return  [
        //         'premium_amount' => 0,
        //         'status' => false,
        //         'message' => 'PA cover for additional paid driver Insured amount should be 2,00,000',
        //         'request' => [
        //             'message' => 'PA cover for additional paid driver Insured amount should be 2,00,000',
        //             'PA_cover_for_additional_paid_driver_amt' => $cover_pa_paid_driver
        //         ]
        //     ];
        // }
        // if (!empty($cover_pa_unnamed_passenger) && $cover_pa_unnamed_passenger != 200000) {
        //     return  [
        //         'premium_amount' => 0,
        //         'status' => false,
        //         'message' => 'Unnamed Passenger PA Cover Insured amount should be 2,00,000',
        //         'request' => [
        //             'message' => 'Unnamed Passenger PA Cover Insured amount should be 2,00,000',
        //             'Unnamed_Passenger_PA_Cover_amt' => $cover_pa_unnamed_passenger
        //         ]
        //     ];
        // }
    ...
    if(strtoupper($requestData->previous_policy_type) == 'NOT SURE'){
        $premium_req_array['motorQuickQuote']['previousInsurer']['isPreviousInsurerKnown'] = false;
        $premium_req_array['motorQuickQuote']['previousInsurer']['previousInsurerCode'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['previousPolicyNumber'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['previousPolicyExpiryDate'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['isClaimInLastYear'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['originalPreviousPolicyType'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['previousPolicyType'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['previousNoClaimBonus'] = null;
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy'] = null;
    }

    if ($mmv->vehicle_type != 'Passenger Carrying') {
        $premium_req_array['motorQuickQuote']['contract']['coverages']['isIMT23'] = $productData->product_identifier == 'IMT23' ? 'true' : 'false';
    }

    if(config('IC.GODIGIT.V2.CAR.ENVIRONMENT') == 'UAT'){
        $premium_req_array = $premium_req_array['motorQuickQuote'];
    }

    $get_response = getWsData(
        config('IC.GODIGIT.V2.CV.ENDPOINT_URL'),
        $premium_req_array,
        'godigit',
        [
            'enquiryId' => $enquiryId,
        ...
        if (isset($response->error->errorCode) && $response->error->errorCode == '0') {
            ...
            if ($requestData->is_idv_changed == 'Y') {
                if ($requestData->edit_idv >= $max_idv) {
                    $premium_req_array['motorQuickQuote']['vehicle']['vehicleIDV']['idv'] ?? $premium_req_array['vehicle']['vehicleIDV']['idv']  = $max_idv;
                } elseif ($requestData->edit_idv <= $min_idv) {
                    $premium_req_array['motorQuickQuote']['vehicle']['vehicleIDV']['idv'] ?? $premium_req_array['vehicle']['vehicleIDV']['idv'] = $min_idv;
                } else {
                    $premium_req_array['motorQuickQuote']['vehicle']['vehicleIDV']['idv'] ?? $premium_req_array['vehicle']['vehicleIDV']['idv'] = $requestData->edit_idv;
                }
            } else {
                ...
                switch ($getIdvSetting) {
                    case 'default':
                        $premium_req_array['motorQuickQuote']['vehicle']['vehicleIDV']['idv'] ?? $premium_req_array['vehicle']['vehicleIDV']['idv'] = $vehicle_idv;
                            ...
                            if ($cover == 'unnamedPaidDriver') {
                                if (isset($subcover->selection) && $subcover->selection == 1) {
                                    if (isset($subcover->netPremium)) {
                                        $cover_pa_paid_driver_SI = $subcover->insuredAmount;
            ...
            if($cng_lpg_amt || in_array($mmv->fuel_type, ['CNG', 'PETROL+CNG', 'DIESEL+CNG', 'LPG'])){
