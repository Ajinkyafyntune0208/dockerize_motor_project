...
function getQuoteMisd($enquiryId, $requestData, $productData)
{
    ...
    if (($requestData->ownership_changed ?? '') == 'Y') 
    {
        return [
            'status'            => false,
            'message'           => 'Breakin Product is not availabe For OwnershipChange change',
        ];
    ...
    // }
        ...
        $mmv = (object) array_change_key_case($mmv, CASE_LOWER);
    ...
    if($policy_holder_type == "COMPANY"){
        return [
            'status' => false,
            'message' => 'Quotes are not Available for Company Case',
    ...
    {
        if($productData->zero_dep == 0 && in_array($productData->company_alias, explode(',', config('CV_AGE_VALIDASTRION_ALLOWED_IC'))))
        {
            return [
                'premium_amount' => 0,
                'status' => false,
    ...
    [   'enquiryId' => 'GODIGIT_QQ_CV_PACKAGE_01',
        ...
        [   'insuranceProductCode' => $insurance_product_code,
            ...
            'coverages' =>
                ...
                'accessories' =>
                    ...
                    'cng' =>
                        [
                        'selection' => $is_cng_opted,
                        'insuredAmount' => $cng_lpg_amt,
                    ],
                    'electrical' =>
                        [
        ...
        [
            ...
            'previousInsurerCode' => $requestData->previous_insurer == 'Go Digit General Insurance Ltd.' ? 158 : 'NA',
    ...
    if ($productData->product_identifier == 'IMT23') {
        $premium_req_array['contract']['coverages']['isIMT23'] = 'true';
    }
    $checksum_data = checksum_encrypt($premium_req_array);
    $is_data_exits_for_checksum = get_web_service_data_via_checksum($enquiryId,'godigit',$checksum_data,'MISC');
