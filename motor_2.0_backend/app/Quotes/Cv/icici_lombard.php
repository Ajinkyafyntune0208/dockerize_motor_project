...
include_once app_path() . '/Quotes/Cv/icici_lombard_Miscd.php';
...
//        ->first();
    ...
    if($type !== 'MISC')
    {
        ...
        $mmvDetails['gvw'] = $mmvData->gvw ?? NULL;
    }
    ...
    if ($token) {
        ...
        if ($enable_idv_service == 'Y') {
            $idv_request = [
                ...
                "manufacturercode" => $mmvData->manf_code,
                "BusinessType" => $businessType,
                "rtolocationcode" => $rtoLocationCode,
                "DeliveryOrRegistrationDate" => !empty($proposerVehDet->vehicle_invoice_date) ? date('Y-m-d', strtotime($proposerVehDet->vehicle_invoice_date)) : date('Y-m-d', strtotime($proposerVehDet->vehicle_register_date)),
                "PolicyStartDate" => $policyStartDate->toDateString(),
                "vehiclemodelcode" => $mmvData->ic_version_code,
            ...
            if ($premium_type == 'comprehensive' || $premium_type == 'short_term_3' || $premium_type == 'short_term_6' || $premium_type == 'breakin') {
                ...
                $get_response = getWsData(config('constants.IcConstants.icici_lombard.CV_IDV_END_POINT_URL'), $idv_request, 'icici_lombard', $idvAdditionalRequest);
                ...
                if(!$idv_response)
                {
                    return [
                        ...
                        'webservice_id' => $get_response['webservice_id'],
                        'table' => $get_response['table'],
                        'message' => $idv_response->error ?? 'Insurer not reachable'
                    ];
                }

                if (isset($idv_response->errorMessage) || $idv_response->status === false) {
                    return [
                        'status' => false,
                        'webservice_id' => $get_response['webservice_id'],
                        'table' => $get_response['table'],
                        'message' => $idv_response->errorMessage ?? 'Insurer not reachable'
                    ];
                }
        ...
        $premiumRecalcRequest = [
            ...
            "PolicyStartDate" => $policyStartDate->toDateString(),
            "RTOLocationCode" => $rtoLocationCode, //$rtoLocationCode->rto_location_code,
            "VehicleMakeCode" =>  $type == 'MISC' ? '366' : $mmvData->manf_code,
            "VehicleModelCode" => $type == 'MISC' ? '6338' : $mmvData->ic_version_code,
            ...
            "InclusionOfIMT" => $type != 'PCV' ? true : false,
            ...
            if (!empty($premRecalculateResponse))
            {
                ...
                else
                {
                    return [
                        ...
                        'webservice_id' => $get_response['webservice_id'],
                        'table' => $get_response['table'],
                        'message' => $premiumResponse['Error'] ?? "Insurer not reachable"
                    ];
                }
            }
            else
        ...
        if ($enable_idv_service != 'Y' && $requestData->is_idv_changed != 'Y' && $allowed == 'Y') {
            if (!empty($premRecalculateResponse))
            {
                ...
                {
                    $message = 'Insurer not reachable';
                    if(isset($premiumResponse['Message']))
                    {
                        $message = $premiumResponse['Message'];
                    }
                    else if(isset($premiumResponse['message']))
                    {
                        $message = $premiumResponse['message'];
                    }
                    return [
                        ...
                        'webservice_id' => $get_response['webservice_id'],
                        'table' => $get_response['table'],
                        'message' => $message//isset($premiumResponse['Message']) ? $premiumResponse['Message'] :"Insurer not reachable"
                    ];
                }
            }
            else
        ...
        if (!empty($premRecalculateResponse)) {
            ...
            if ($response_status && isset($premiumResponse['statusMessage']) && $premiumResponse['statusMessage'] == 'SUCCESS') {
                ...
                // $bonusDiscount = (!in_array($premium_type,['third_party','third_party_breakin'])) ?($totalOdPremium - $totalDiscount) * ($requestData->applicable_ncb/100) : 0;
                ...
                $data_response = [
                    ...
                    'Data' => [
                        ...
                        'mmv_detail' => $type === 'MISC' ? [] :$mmvDetails,
                        'master_policy_id' => [
