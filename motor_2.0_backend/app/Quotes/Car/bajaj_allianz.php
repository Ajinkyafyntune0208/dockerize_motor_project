...
use App\Quotes\Car\V1\bajaj_allianz as BAJAJ_ALLIANZ_V1;
...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
            $url = config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_QUOTE_TP_URL');
    } else {
            $url = config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_QUOTE_URL');
    }
    ...
    if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
        $service_response = $response['SOAP-ENV:Body']['m:calculateMotorPremiumSigResponse'] ?? null;
    } else {
        $service_response = $response['env:Body']['m:calculateMotorPremiumSigResponse'] ?? null;
    }
    ...
    if ($requestData->is_idv_changed == 'Y' && $tp_only == 'false') {
        ...
        if($is_data_exist_for_checksum['found'] && $refer_webservice && $is_data_exist_for_checksum['status']){
            ...
            $get_response = getWsData(config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_QUOTE_URL'), $input_array, 'bajaj_allianz', $additional_data);
        }
        if (empty($get_response['response'])) {
            return [
                'webservice_id' => $get_response['webservice_id'],
    ...
    $transaction_id =  ''; // by this git we have to pass null #33408
    ...
    if (isset($service_response['premiumSummeryList_out']['typ:WeoMotPremiumSummaryUser'])) {
        ...
        if ($premium_type == 'own_damage' && $requestData->is_claim == 'Y' && $productData->zero_dep == '1' && isset($covers['od'])) {
            ...
            foreach ($covers as $key => $cover) {
                ...
                } elseif (in_array($cover['typ:paramDesc'], ['Commercial Discount','7Commercial Discount'])) {
    ...
    $totalTP = $tppd + $pa_unnamed + $ll_paid_driver + $lpg_cng_tp + $ll_paid_emp; //B
    ...
    if (!empty($ll_paid_emp) && $requestData->vehicle_owner_type == 'C') 
    {
        $data_response['Data']['other_covers']['LegalLiabilityToEmployee'] = $ll_paid_emp;
        $data_response['Data']['LegalLiabilityToEmployee'] = $ll_paid_emp;
    }        
    if($is_lpg_cng)
    {
