...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    {
        include app_path() . '/Quotes/Car/V2/hdfc_ergo.php';
        return getQuoteV2($enquiryId, $requestData, $productData);
    }
    else if (config('IC.HDFC_ERGO.V1.CAR.ENABLE') == 'Y')
    {
        include app_path() . '/Quotes/Car/V1/hdfc_ergo.php';
        return hdfc_ergo::getQuoteV1($enquiryId, $requestData, $productData);
    } 
    else {
...
function getQuoteV1($enquiryId, $requestData, $productData)
{
    //try {
        ...
        if (empty($mmv_data->ic_version_code) || $mmv_data->ic_version_code == '') {
            ...
            }elseif($car_age > 10 && $productData->zero_dep == '0' && in_array($productData->company_alias, explode(',', config('CAR_AGE_VALIDASTRION_ALLOWED_IC')))){
                return [
                    'premium_amount' => 0,
                    'status' => false,
            ...
            if ($requestData->business_type == 'newbusiness') {
                ...
                $PreviousPolicyFromDt = $PreviousPolicyType = $previous_ncb = '';
            ...
            else
            {
                ...
                if ($requestData->business_type == "breakin" && !in_array($premium_type, ['third_party', 'third_party_breakin'])) { 
                    $policy_start_date = today();
                }
                $policy_end_date = Carbon::parse($policy_start_date)->addYear(1)->subDay(1)->format('d/m/Y');
                $vehicale_registration_number = explode('-', $requestData->vehicle_registration_no);
            ...
            $is_rsa = $is_lopb = $eng_Gear_box = $zero_dep = $key_rplc_yn = '0';
            ...
            if($requestData->applicable_ncb != 0 && $productData->zero_dep == '0' && !in_array($premium_type,['third_party','third_party_breakin'])){
                ...
                if (isset($data_idv['StatusCode']) && $data_idv['StatusCode'] == 200 || in_array($premium_type, ['third_party', 'third_party_breakin'])) {
                    ...
                    $model_config_premium = [
                        ...
                        'Req_PvtCar' => [
                            ...
                            'IsRTI_Cover' => ($requestData->business_type == 'newbusiness') ? 1 : (($interval->days <= 1093 && !$without_addon && $requestData->applicable_ncb != 0) ? 1 : 0),
                            'IsCOC_Cover' => ($car_age <= 5 && !$without_addon) ? 1 : 0,
                            'IsEngGearBox_Cover' => ($car_age <= 5 && !$without_addon && $requestData->fuel_type != "ELECTRIC") ? 1 : 0,
                            ...
                            'IsLossOfPersonalBelongings_Cover' => ($car_age <= 5 && !$without_addon) ? 1 : 0,
                            'LossOfPersonalBelonging_SI' => 30000,
                            'IsTyreSecure_Cover' => ($interval->days <= 1064 && $without_addon == false) ? 1 : 0,
                    ...
                    if($requestData->applicable_ncb != 0 && $productData->zero_dep == '0' && !in_array($premium_type,['third_party','third_party_breakin'])){
                        switch($productData->product_identifier){
                            case 'Essential ZD':
                                ...
                                $model_config_premium['Req_PvtCar']['IsZeroDept_Cover'] = $zero_dep;
                                $model_config_premium['Req_PvtCar']['IsEA_Cover'] = $is_rsa;
                                $model_config_premium['Req_PvtCar']['IsEAW_Cover'] = $key_rplc_yn;
                                $model_config_premium['Req_PvtCar']['IsLossOfPersonalBelongings_Cover'] = $is_lopb;
                                break;
                            case 'Essential EGP':
                                $model_config_premium['Req_PvtCar']['planType'] = 'Essential EGP plan';
                                $model_config_premium['Req_PvtCar']['IsZeroDept_Cover'] = $zero_dep;
                                $model_config_premium['Req_PvtCar']['IsEA_Cover'] = $is_rsa;
                                $model_config_premium['Req_PvtCar']['IsEAW_Cover'] = $key_rplc_yn;
                                $model_config_premium['Req_PvtCar']['IsLossOfPersonalBelongings_Cover'] = $is_lopb;
                                $model_config_premium['Req_PvtCar']['IsEngGearBox_Cover'] = $eng_Gear_box;
                                break;
                        }
                    }
                    $additionData = [
                    ...
                    if (($arr_premium['StatusCode'] == '200')) {
                        ...
                        $idv = ($premium_type != 'third_party') ? (string)$premium_data['IDV'] : '0';
                        ...
                        if (!empty($premium_data['LossOfPersonalBelongings_Premium'])) {
                            $personal_belonging = $premium_data['LossOfPersonalBelongings_Premium'];
                        }
                        if (!empty($premium_data['Vehicle_Base_NCB_Premium'])) {
                            $ncb_protection = $premium_data['Vehicle_Base_NCB_Premium'];
                        }
                        ...
                        if (!empty($premium_data['BiFuel_Kit_OD_Premium'])) {
                            $lpg_cng = $premium_data['BiFuel_Kit_OD_Premium'];
                        }
                        if (!empty($premium_data['BiFuel_Kit_TP_Premium'])) {
                            $lpg_cng_tp = $premium_data['BiFuel_Kit_TP_Premium'];
                        }
                        ...
                        if (!empty($premium_data['Vehicle_Base_TySec_Premium'])) {
                            $tyre_secure = $premium_data['Vehicle_Base_TySec_Premium'];
                        }
                        ...
                        if (!empty($premium_data['InBuilt_BiFuel_Kit_Premium'])) {
                            $lpg_cng_tp = $premium_data['InBuilt_BiFuel_Kit_Premium'];
                        }
                        if(!empty($premium_data['NumberOfEmployees_Premium'])) {
                            $legal_liability_to_employee = $premium_data['NumberOfEmployees_Premium'];
                        }
                        if(!empty($premium_data['BatteryChargerAccessory_Premium'])) {
                            $batteryProtect = $premium_data['BatteryChargerAccessory_Premium'];
                        }

                        $final_tp_premium = $premium_data['Basic_TP_Premium'] + $pa_unnamed + $lpg_cng_tp + $pa_paid_driver + $ll_paid_driver+$GeogExtension_tp+$OwnPremises_TP+$legal_liability_to_employee;
                        ...
                        if($productData->zero_dep == '0' && in_array($productData->product_identifier,['Essential ZD','Essential EGP']) && $requestData->applicable_ncb != 0 && !in_array($premium_type,['third_party','third_party_breakin']))
						{
                            switch($productData->product_identifier)
							{
                                case 'Essential ZD':
									...
									array_push($applicable_addons, "zeroDepreciation","roadSideAssistance","keyReplace","lopb");
									break;
								case 'Essential EGP':
									$add_ons = [
										'in_built' => [
									...
									array_push($applicable_addons, "zeroDepreciation","roadSideAssistance","keyReplace","lopb","engineProtector");
									break;
							}
						}
                        ...
                        if (!empty($applicable_addons)) {
                            if ($rsapremium == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["roadSideAssistance"]);
                            }
                            if ($ncb_protection == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["ncbProtection"]);
                            }
                            if ($tyre_secure == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["tyreSecure"]);
                            }
                            if ($zero_dep_amount == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["zeroDepreciation"]);
                            }
                            if ($engine_protection == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["engineProtector"]);
                            }

                            if ($consumables_cover == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["consumables"]);
                            }

                            if ($key_rplc == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["keyReplace"]);
                            }
                            if ($rti == 0) {
                                $applicable_addons = array_diff($applicable_addons, ["returnToInvoice"]);
                            }
                        } 
                        ...
                        $data_response = [
                            ...
                            'Data' => [
                                ...
                                'net_premium' => $final_payable_amount,
                                ...
                                'final_net_premium' => $final_payable_amount,
...
function getQuoteV2($enquiryId, $requestData, $productData)
{
    ...
    if (config('IC.HDFC_ERGO.V2.CAR.ENABLE') == 'Y'){
        return  hdfc_ergo::getQuotev2($enquiryId, $requestData, $productData);
        }
    ...
    if ((empty($RegistrationNo))||($RegistrationNo == "NEW")) { 
        $RegistrationNo = $requestData->rto_code . "-XX-XXXX";
    } 
    $premium_request = [
        'ConfigurationParam' => [
        ...
        'AddOnCovers' => [
            ...
            'IsZeroDepCover' => $productData->zero_dep == 0 && $interval->y < 5? "YES" : "NO",
            'IsLossOfUse' => "NO", //$interval->y < 1 ? "YES" : "NO",
            //'IsEmergencyAssistanceCover' => $interval->y < 15 ? "YES" : "NO",
            'IsEmergencyAssistanceCover' => $premium_type != 'third_party' ? "YES" : "NO",
            'IsNoClaimBonusProtection' => $interval->y < 5 ? "YES" : "NO",
            'IsEngineAndGearboxProtectorCover' => $interval->y < 5 ? "YES" : "NO",
            ...
            'IsEmergencyAssistanceWiderCover' => $interval->y < 5 ? "YES" : "NO",
            ...
            if (isset($premium_response['Data'][0]['AddOnCovers']))
            {
                foreach ($premium_response['Data'][0]['AddOnCovers'] as $addon_cover)
                {
                    switch($addon_cover['CoverName'])
                    {
                        ...
                        case 'ZERODEP':
                            $zero_depreciation = $productData->zero_dep == 0 && $interval->y < 5 ? $addon_cover['CoverPremium'] : 0;
                            break;
                        ...
                        case 'ENGEBOX':
                            $engine_protection = $interval->y < 5 ? $addon_cover['CoverPremium'] : 0;
                            break;
                        ...
                        case 'EMERGASSISTWIDER':
                            $key_replacement = $interval->y < 5 ? $addon_cover['CoverPremium'] : 0;
                            break;
                        ...
                        // case 'LOSSUSEDOWN':
                        // case 'LOPB':
                        //     $loss_of_personal_belongings = $interval->y < 1 ? $addon_cover['CoverPremium'] : 0;
                        //     break;
