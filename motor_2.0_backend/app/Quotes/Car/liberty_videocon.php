...
use App\Quotes\Car\V1\liberty_videocon as LIBERTY_VIDEOCON_V1;

use App\Http\Controllers\Proposal\Services\Car\libertyVideoconSubmitProposal as Liberty;

function getQuote($enquiryId, $requestData, $productData)
{
    if(config('IC.LIBERTY_VIDEOCON.V1.CAR.ENABLE') == 'Y'){
        return LIBERTY_VIDEOCON_V1::getQuote($enquiryId, $requestData, $productData);
    }
    ...
    } elseif ($zero_dep_age_limit && $productData->product_identifier == 'zerodep' && in_array($productData->company_alias, explode(',', config('CAR_AGE_VALIDASTRION_ALLOWED_IC')))) {
        return [
            'status' => false,
            'premium_amount' => 0,
    ...
    if(config('LIBERTY_RTI_DISABLED') == 'Y') {
    ...
    $productcode = ($is_od ? config('constants.IcConstants.liberty_videocon.PRODUCT_CODE_OD') : ($is_liability ? config('constants.IcConstants.liberty_videocon.PRODUCT_CODE_TP') :config('constants.IcConstants.liberty_videocon.PRODUCT_CODE_PACKAGE')));#tp-3155
    ...
    $premium_request = [
        ...
        'PassengerAsstCover' => 'Yes', // by default yes by #33470
        'EngineNo' => '',
        'ChassisNo' => '',
    ...
    $is_bh = (isBhSeries($requestData->vehicle_registration_no));
    if($is_bh){
        $premium_request['isBHSeries'] = true;
        $premium_request['BHSeriesNumber'] = str_replace('-', '', $requestData->vehicle_registration_no);
        $premium_request['RegNo1'] = '';
        $premium_request['RegNo2'] = '';
        $premium_request['RegNo3'] = '';
        $premium_request['RegNo4'] = '';
    }
    ...
    if ($data['response']) {
        ...
        if ($premium_response !== null && trim($premium_response['ErrorText']) == "") {
            ...
            if(!$skip_second_call){
            $data_request = $premium_request;
            unset($data_request['QuickQuoteNumber'], $data_request['RegNo4']);
            $checksum_data = checksum_encrypt($data_request);
            $is_data_exits_for_checksum = get_web_service_data_via_checksum($enquiryId, 'liberty_videocon', $checksum_data, 'CAR');
            if($is_data_exits_for_checksum['found'] && $refer_webservice && $is_data_exits_for_checksum['status']){
                $data = $is_data_exits_for_checksum;
            }else{
                    if (!$is_liability && $requestData->is_idv_changed == 'Y') {
                        $data = getWsData(config('constants.IcConstants.liberty_videocon.END_POINT_URL_LIBERTY_VIDEOCON_PREMIUM_CALCULATION'), $premium_request, 'liberty_videocon', [
                            'enquiryId' => $enquiryId,
                            'requestMethod' => 'post',
                            'productName'  => $productData->product_name,
            ...
            switch ($productData->product_identifier) {
                case 'key_replacement':
                    ...
                    array_push($applicable_addons, "zeroDepreciation", "consumables", "engineProtector", "roadSideAssistance", "keyReplace", "returnToInvoice");
                    break;
                case 'consumable':
                    $addons_data = [
                        'in_built'   => [
                    ...
                    array_push($applicable_addons, "zeroDepreciation", "consumables", "roadSideAssistance", "returnToInvoice");
                    break;
                ...
                case 'zerodep':
                    ...
                    array_push($applicable_addons, "zeroDepreciation", "roadSideAssistance", "returnToInvoice");
                    break;
                ...
                case 'basic':
                    ...
                    array_push($applicable_addons, "roadSideAssistance", "returnToInvoice");
                    break;
            }
