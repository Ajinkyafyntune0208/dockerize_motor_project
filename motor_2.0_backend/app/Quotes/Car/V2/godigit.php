...
use App\Quotes\Car\V2\GodigitZDPlan;

function oneApiGetQuote($enquiryId, $requestData, $productData)
{
    //ZD PLAN ENABLE V2 'ONE,TWO,UNLIMITED'
    $is_zd_all_plan_enable = config('IC.GODIGIT.V2.CAR.ZERO_DEP_ALL_PLAN_ENABLE') == 'Y';
    if($is_zd_all_plan_enable)
    {
        $godigit_v2_zd_plan = new GodigitZDPlan();
        return $godigit_v2_zd_plan->getQuote($enquiryId, $requestData, $productData);
    }
        ...
        [
            ...
            'previousInsurer' => [
                ...
                'previousPolicyNumber' => null,
                 // 'previousPolicyExpiryDate' => $requestData->ownership_changed == 'Y' ? date('Y-m-d', strtotime('-91 days', time())) : (!empty($requestData->previous_policy_expiry_date) ? date('Y-m-d', strtotime($requestData->previous_policy_expiry_date)) : null), #removed ownership_changed logic by #31785
        ...
        if (isset($response->error->errorCode) && $response->error->errorCode == '0') {
            ...
            if (!empty($data['response'])) {
                $response = json_decode($data['response']);
                    if ($response->error->errorCode == '0') {
                        if(!empty($response->error->errorCode)){
                ...
                if (!empty($response->error->validationMessages[0])) {
                    ...
                    case 'addons':
                        foreach ($value as $key => $addon) {
                            switch ($key) {
                                ...
                                case 'tyreProtection':
                                    if (isset($addon->coverAvailability) && ($addon->selection == true && $addon->coverAvailability == 'AVAILABLE' || $addon->coverAvailability == 'MANDATORY')) {
                                        $tyre_protection = (str_replace('INR ', '', $addon->netPremium));
                                    }
                                    break;
                                ...
                                case 'personalBelonging':
                                    if (isset($addon->coverAvailability) && ($addon->selection == true && $addon->coverAvailability == 'AVAILABLE' || $addon->coverAvailability == 'MANDATORY')) {
                                        $personal_belonging = (str_replace('INR ', '', $addon->netPremium));
                                    }
                                    break;

                                case 'keyAndLockProtect':
                                    if (isset($addon->coverAvailability) && ($addon->selection == true && $addon->coverAvailability == 'AVAILABLE' || $addon->coverAvailability == 'MANDATORY')) {
                                        $key_and_lock_protection = (str_replace('INR ', '', $addon->netPremium));
                                    }
                                    break;
                            }
                        }
            ...
            $data_response = [
                ...
                'Data' => [
                    ...
                    'business_type' => ($requestData->business_type == 'newbusiness') ? 'New Business' : (($requestData->previous_policy_type == 'Third-party' || $requestData->previous_policy_type == 'Not sure') ? 'Break-in' : $requestData->business_type),
...
function oneApiGetBundleAddon($enquiryId, $requestData, $productData)
{
    ...
    switch ($productData->product_identifier) {
        ...
        case "BASIC_WITH_ADDONS":
            ...
            $tyreSecure = null;
            $engineProtector = null;
            $consum = null;
            $personal_Belonging = null;
            $keyAndLock_Protect = null;
            $roadSideAssistance = null;
            break;
    }
    $access_token_resp = GoDigitHelper::getToken($enquiryId, $productData);
    $access_token = $access_token_resp['token'] ?? null;

    if (empty($access_token)) {
        return [
            'status' => false,
            'message' => 'Error in token service',
        ];
    }
    ...
    // if ($premium_type == 'third_party') {
    //     unset($premium_req_array['motorQuickQuote']['contract']['coverages']['addons']);
    // }
    if ($premium_type == "own_damage") {
        $premium_req_array['motorQuickQuote']['previousInsurer']['originalPreviousPolicyType'] = "1OD_3TP";
        $premium_req_array['motorQuickQuote']['previousInsurer']['currentThirdPartyPolicy']['isCurrentThirdPartyPolicyActive'] = true;
    ...
    if ($productData->good_driver_discount == 'Yes') {
        ...
        return oneApiGetGDDQuote($enquiryId, $requestData, $productData, [
    ...
    if($is_data_exist_for_checksum['found'] && $refer_webservice && $is_data_exist_for_checksum['status']){
        ...
        $data = getWsData(
            ...
            [
                ...
                'authorization' => $access_token, //(passing Username and Password)
                'transaction_type' => 'quote',
                'integrationId' => $integrationId
            ]
        );
    ...
    if (!empty($data['response'])) {
        ...
        if (isset($response->error->errorCode) && $response->error->errorCode == '0') {
            ...
            if (!empty($data['response'])) {
                ...
                } elseif (!isset($response->contract)) {
                        ...
                        foreach ($value as $key => $addon) {
                            ...
                            if( $enable_package_validation && isset($addon->coverOfferingType) && $addon->coverOfferingType != 'INDIVIDUAL' && $productData->product_identifier == 'BASIC_WITH_ADDONS')
                            {
                                return [
                                    ...
                                    'premium_amount' => 0,
                                    'status' => false,
                                    'message' => 'Addons are not available for this product '. $productData->product_identifier
                                ];
                                break;
                            }
                            else if( $enable_package_validation && isset($addon->coverOfferingType) && $addon->coverOfferingType == 'INDIVIDUAL' && $productData->product_identifier != 'BASIC_WITH_ADDONS' && $productData->product_identifier != 'BASIC')
                            {
                                return [
                                    'webservice_id' => $data['webservice_id'],
                                    'table' => $data['table'],
                            ...
                            switch ($key) {
                                case 'partsDepreciation':
                                    if (  ($addon->coverAvailability == 'AVAILABLE' || $addon->coverAvailability == 'MANDATORY')) {
                                        $zero_depreciation = (str_replace('INR ', '', $addon->netPremium));
                                    }
                                    break;
            ...
            if ($productData->product_identifier == 'BASIC_WITH_ADDONS') {
                array_splice($applicable_addons, array_search('zeroDepreciation', $applicable_addons), 1);
            }
...
function oneApiGetGDDQuote($enquiryId, $requestData, $productData, $additionalData)
{
    ...
    $premium_response = getWsData(config('IC.GODIGIT.V2.CAR.END_POINT_URL'), $premium_req_array, 'godigit', [
        'enquiryId' => $enquiryId,
        'requestMethod' => 'post',
        'productName'  => $productData->product_name,
        'company'  => 'godigit',
        ...
        'integrationId' => $integrationId,
        'headers' => [
            // 'Authorization' => 'Basic ' . base64_encode(config('constants.IcConstants.godigit.GODIGIT_WEB_USER_ID') . ':' . config('constants.IcConstants.godigit.GODIGIT_PASSWORD'))
            'authorization' => $access_token, //(passing Username and Password)
        ]
    ]);
    ...
    if ($premium_response['response']) {
        ...
        if (isset($premium_request_data['error']['errorCode']) && $premium_request_data['error']['errorCode'] == 0) {
            ...
            if ($premium_type != 'third_party') {
                $vehicle_idv = ($premium_request_data['vehicle']['vehicleIDV']['idv']);
                ...
                $default_idv = ($premium_request_data['vehicle']['vehicleIDV']['defaultIdv']);
            ...
            foreach ($premium_request_data['contract']['coverages'] as $key1 => $coverage) {
                if ($coverage['name'] == 'Motor Own Damage -  Add Ons') {
            ...
            $get_premium_response_data = getWsData(config('IC.GODIGIT.V2.CAR.END_POINT_URL'), $premium_request_data, 'godigit', [
                'enquiryId' => $enquiryId,
                'requestMethod' => 'post',
                'productName'  => $productData->product_name,
                'company'  => 'godigit',
                ...
                if (isset($premium_response_data['error']['errorCode']) && $premium_response_data['error']['errorCode'] == 0) {
                    ...
                    foreach ($premium_response_data['contract']['coverages'] as $coverage) {
                        if ($coverage['coverType'] == 'THIRD_PARTY') {
                            ...
                            $basic_od = ((float) str_replace('INR ', '', $coverage['netPremium']));
                    ...
                    $ncb_discount = ($basic_od * $requestData->applicable_ncb / 100);
