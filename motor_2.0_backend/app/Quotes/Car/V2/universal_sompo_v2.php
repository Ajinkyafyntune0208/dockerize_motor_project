...
use Carbon\Carbon;
use App\Models\UniversalSompoCarAddonConfiguration;
...
function getQuoteV2($enquiryId, $requestData, $productData)
{
    ...
    // blocking newbussiness tp policy generation
    ...
    if(config('APPLICABLE_ADDONS_WITH_AGE_UNIVERSAL_SAMPO_CAR') == 'Y')
    {
    $usgi_applicable_addons = get_usgi_applicable_addons_v2($car_age,$vehicle_make_name,$premium_type);
    }
    else{
    $usgi_applicable_addons = get_usgi_without_applicable_addons($premium_type);
    }
   
    if ($usgi_applicable_addons['zero_dep'] == 'False' && $productData->zero_dep == '0' && in_array($productData->company_alias, explode(',', config('CAR_AGE_VALIDASTRION_ALLOWED_IC')))) {
        return [
            'premium_amount' => 0,
            'status' => false,
    ...
    // try {
        ...
        // if(in_array($premium_type ,['own_damage','third_party'])) //commented-git:#35577
        // {
        //     $cpa = 'False';
        //     $cpa_tenure = '0';
        // }
                ...
                'Risks' => [
                    ...
                    'VehicleMakeCode' => [
                        ...
                        '@attributes' => [
                            ...
                            'Value' => $mmv_data->usgi_make_code ?? NULL
                        ],
                    ],
                    'VehicleModelCode' => [
                        '@value' => '',
        ...
        $comprehensive_product_response = getWsData(config('IC.UNIVERSAL_SOMPO.V2.CAR.END_POINT_URL'), $xmlQuoteRequest, 'universal_sompo', $additionData);
        ...
        if ($response) 
        {
            ...
            $get_newresponse = getWsData(config('IC.UNIVERSAL_SOMPO.V2.CAR.END_POINT_URL'), $idvChangedQuoteXmlRequest, 'universal_sompo', $additionData);
        }
        else{
            return [
                'status' => false,
        ...
        if ($get_newresponse) 
        {
            ...
            $total_addons = $rsa + $consumable + $eng_prot + $key_replacement + $zero_dep_amount + $return_to_invoice + $tyre_secure;
            ...
            $applicable_addons = [
                'zeroDepreciation', 'roadSideAssistance', 'keyReplace', 'engineProtector', 'consumables', 'tyreSecure', 'returnToInvoice'
            ];
            ...
            {
                $data_response['Data']['antitheft_discount'] = ($anti_theft_device_discount);
            }

            if($tppd_cover_flag == 'True')
            {
                $data_response['Data']['tppd_discount'] = ($tppd_discount_amount);
            }
...
function get_usgi_applicable_addons_v2($age,$vehicle_make,$premium_type)
{
    ...
    if($age <= 15 && !in_array($premium_type,['third_party','third_party_breakin']))
    {
        $addon_data = UniversalSompoCarAddonConfiguration::
        select('zero_dep', 'engine_protector', 'return_to_invoice', 'road_side_assistance', 'key_replacement', 'consumable', 'tyre_secure', 'loss_of_belongings')
        ->where('vehicle_make', $vehicle_make)
        ->where('age_range', $age_range)->first();
        if($addon_data)
        {

            $is_zero_dep_applicable = $addon_data->zero_dep;
            $is_key_replacement_applicable = $addon_data->key_replacement;
            $is_engine_protector_applicable = $addon_data->engine_protector;
            $is_consumable_applicable = $addon_data->consumable;
            $is_tyre_secure_applicable = $addon_data->tyre_secure;
            $is_rti_applicable = $addon_data->return_to_invoice;
            $is_lob_applicable = $addon_data->loss_of_belongings;
            $is_rsa_applicable = $addon_data->road_side_assistance;
            $is_ncb_protector_applicable = $addon_data->ncb_protector;
        }
        else
        {
            $is_zero_dep_applicable = $is_key_replacement_applicable = $is_engine_protector_applicable = $is_consumable_applicable = $is_tyre_secure_applicable = $is_rti_applicable = $is_lob_applicable = $is_rsa_applicable = $is_ncb_protector_applicable = 'False';
        }
    }
    else
    {
...
function get_usgi_without_applicable_addons($premium_type)
{
