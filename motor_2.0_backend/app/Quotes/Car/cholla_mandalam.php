...
function getQuote($enquiryId, $requestData, $productData)
{
    if(config('IC.CHOLLA_MANDALAM.V1.CAR.ENABLED') == 'Y'){
        include_once app_path() . '/Quotes/Car/V1/cholla_mandalam.php';
    ...
    if(empty($rto_data)){
        ...
        $product_id = ($is_package ? config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_PRODUCT_ID'): ($is_liability ? config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_PRODUCT_ID_TP') : config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_PRODUCT_ID_OD')));
        ...
        $cpa['period'] = '1';
        //Addons
        $is_addon['zero_dep']                      = (($car_age <= 5) && !$is_liability && ($productData->zero_dep == 0)) ? true : false;
        $is_addon['rsa']                           = (($car_age <= 5) && !$is_liability) ? true : false;
        $is_addon['key_replacement']               = (($car_age <= 5) && !$is_liability) ? true : false;
        $is_addon['consumable']                    = (($car_age <= 5) && !$is_liability) ? true : false;
        $is_addon['loss_of_belongings']            = (($car_age <= 5) && !$is_liability) ? true : false;
        $is_addon['engine_protect']                = (($car_age <= 5) && !$is_liability) ? true : false;
        ...
        $quote_array = [
            ...
            'IMDShortcode_Dev' => config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_IMDSHORTCODE_DEV'),
            'product_id' => $product_id,
            'user_code' => config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_USER_CODE'),
        ...
        if(config('constants.IcConstants.cholla_madalam.IS_TP_IMDSHORTCODE_DEV') == 'Y' && in_array($premium_type, ['third_party', 'third_party_breakin']))
        {
            $quote_array['IMDShortcode_Dev'] = config('constants.IcConstants.cholla_madalam.CHOLLA_MANDALAM_MOTOR_TP_IMDSHORTCODE_DEV');
        }

        $quote_url = ($is_package ? config('constants.IcConstants.cholla_madalam.END_POINT_URL_CHOLLA_MANDALAM_MOTOR_QUOTE') : ($is_liability ? config('constants.IcConstants.cholla_madalam.END_POINT_URL_CHOLLA_MANDALAM_MOTOR_QUOTE_TP') : config('constants.IcConstants.cholla_madalam.END_POINT_URL_CHOLLA_MANDALAM_MOTOR_QUOTE_OD')));
        ...
        $error_message = $quote_respone['Message'] ?? $quote_respone['Status'] ?? $quote_respone['message'];
        ...
        if (empty($quote_respone['data'])) {
            return [
                'webservice_id' => $data['webservice_id'],
                'table' => $data['table'],
                'status'   => false,
                'message'  => $quote_respone['description'] ?? $quote_respone['message'] ?? 'Error In Premium Calculation',
                'error'        => $quote_respone['description'] ?? $quote_respone['message'] ?? 'Error In Premium Calculation',
                'product_identifier' => $masterProduct->product_identifier,
            ];
        }
