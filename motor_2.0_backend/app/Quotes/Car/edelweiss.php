...
include_once app_path() . '/Quotes/Car/V1/edelweiss.php';
        ...
        }else
        {
            ...
            if(empty($get_token_data)){
                $get_token_data = cache()->remember($token_cache_name, 60 * 45, function() use ($enquiryId, $productData){     
                    $token_data = getWsData(config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_TOKEN_GENERATION'),'', 'edelweiss',
                    [
                        'enquiryId' => $enquiryId,
                        'requestMethod' =>'post',
                ...
                if ($zero_addon_age == false && $productData->zero_dep == 0 && in_array($productData->company_alias, explode(',', config('CAR_AGE_VALIDASTRION_ALLOWED_IC'))))
                {
                ...
                if(isset($token_data['access_token']))
                {
                    ...
                    //Changes 35085
                    if($requestData->vehicle_owner_type == 'C'){
                        $service_request_data['caTenure'] = '1';
                    }
                    if(!$is_liability){
                    ...
                    if(!$is_od && $requestData->vehicle_owner_type == 'I'){
                        array_push($service_request_data['contractDetails'],
                    ...
                    if(!$is_od && $requestData->vehicle_owner_type == 'C'){
                        array_push($service_request_data['contractDetails'], $tp_block); //Git 35147 
                    }
                    ...
                    if($is_data_exits_for_checksum['found'] && $refer_webservice && $is_data_exits_for_checksum['status']){
                        ...
                        $get_quote_data = getWsData(config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_QUOTE_GENERATION'),$service_request_data, 'edelweiss',
                        [
                            'enquiryId' => $enquiryId,
                            'requestMethod' =>'post',
                            'productName'  => $productData->product_name,
