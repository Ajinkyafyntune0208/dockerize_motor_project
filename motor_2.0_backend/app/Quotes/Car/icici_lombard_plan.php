...
function getQuotePlan($enquiryId, $requestData, $productData)
{
    try {
        ...
        if (!empty($token)) {
            ...
            foreach ( $allPackagesWithAddons as $package_key => $package_value) {
                ...
                foreach ($package_value as $all_addon_key => $all_addon_value) {
                ...
                if ($premium_type != 'third_party' && $enable_idv_service == 'Y') {
                    ...
                    $additionPremData = [
                        ...
                        'type' => 'idvService',
                        'section' => 'car',
                        'productName' => $productData->product_name . " " . $package_key,
                ...
                $additionPremData = [
                    ...
                    'section' => 'car',
                    'productName' => $productData->product_name . " premiumCalculation " . $package_key,
                    'token' => $access_token,
                    'enquiryId' => $enquiryId,
                    'transaction_type' => 'quote',
                    'headers' => [
                ...
                if($productData->good_driver_discount=="Yes")
                {
                    ...
                    $randomNumber=1;
                    $payud_Arr['1']['PAYUDetails']['InitialPlan']="5000";
                    $payud_Arr['1']['PAYUDetails']['OdometerCaptureDate']="2023-02-01T00:00:00";
                    $payud_Arr['1']['PAYUDetails']['OdometerReading']="34535";
                    $payud_Arr['2']['PAYUDetails']['InitialPlan']="7500";
                    $payud_Arr['2']['PAYUDetails']['OdometerCaptureDate']="2023-02-01T00:00:00";
                    $payud_Arr['2']['PAYUDetails']['OdometerReading']="54535";
                    $payud_initialplan=$payud_Arr[$randomNumber]['PAYUDetails']['InitialPlan'];
                    $payud_odometercapturedate=$payud_Arr[$randomNumber]['PAYUDetails']['OdometerCaptureDate'];
                    $payud_odometerreading=$payud_Arr[$randomNumber]['PAYUDetails']['OdometerReading'];
                ...
                if ($enable_idv_service != 'Y' && $requestData->is_idv_changed != 'Y' && $premium_type != 'third_party') {
                    if ($data['response']) {
                        ...
                        if (isset($dataResponse['status']) && $dataResponse['status'] == 'Success') {
                            ...
                            $additionPremData['productName'] = $productData->product_name . " premiumCalculationidv " . $package_key;
                            $data = getWsData($url, $model_config_premium, 'icici_lombard', $additionPremData);
                    ...
                    if (isset($arr_premium['status']) && strtolower($arr_premium['status']) == 'success') {
                        ...
                        if ($premium_type != 'third_party') {
                            ...
                            if ($requestData->is_idv_changed == 'Y') {
                                ...
                                $additionPremData = [
                                    ...
                                    'section' => 'car',
                                    'productName' => $productData->product_name . "premiumCalculationIdvChange " . $package_key,
                                    'token' => $access_token,
                                    'enquiryId' => $enquiryId,
                                    'transaction_type' => 'quote',
                                ];
                        ...
                        $data_response =
                            ...
                            'Data' => [
                                ...
                                'product_name' => $productData->product_sub_type_name . ' - ' . ucwords(str_replace('_', ' ', strtolower($productData->product_identifier)))." - ".$package_key,
                                'mmv_detail' => $mmv_data,
                                'master_policy_id' => [
...
function thirdPartyResponse(
    ...
    $total_od = $od_premium + $elect_acc + $non_elec_acc + $lpg_cng_od; #breaking loading amount remove from here
    $total_tp = $tp_premium + $llpd_amt + $unnamed_pa_amt + $lpg_cng_tp;
    $total_discount = $ncb_discount + $automobile_assoc + $anti_theft + $voluntary_deductible + $tppd_discount;
    $basePremium = $total_od + $total_tp - $total_discount;
