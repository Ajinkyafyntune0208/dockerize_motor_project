...
include app_path() . '/Quotes/Car/V2/universal_sompo_v2.php';
...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    //Version 2 should be enabled.
        return [
            'premium_amount' => 0,
            'status' => false,
            'message' => 'Kindly Enable V2 Config.',
        ];
    ...
    try {
        ...
        $is_data_exist_for_checksum = get_web_service_data_via_checksum($enquiryId, 'universal_sompo', $checksum_data, 'CAR');
        ...
        if($is_data_exist_for_checksum['found'] && $refer_webservice && $is_data_exist_for_checksum['status']){
            $comprehensive_product_response = $is_data_exist_for_checksum;
        }else{
            $comprehensive_product_response = getWsData(config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_CAR'), $xmlQuoteRequest, 'universal_sompo', $additionData);
        }
        ...
        if ($response) 
        {
            ...
            update_quote_web_servicerequestresponse($comprehensive_product_response['table'], $comprehensive_product_response['webservice_id'], "Success", "Success");
            ...
            if($is_data_exist_for_checksum['found'] && $refer_webservice && $is_data_exist_for_checksum['status']){
                $get_newresponse = $is_data_exist_for_checksum;
            }else{
                $get_newresponse = getWsData(config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_CAR'), $idvChangedQuoteXmlRequest, 'universal_sompo', $additionData);
            }
        }
        else{
            return [
        ...
        if ($get_newresponse) 
        {
            ...
            if($premium_type !== 'third_party')
            {
                ...
                if($usgi_applicable_addons['rsa'] == 'True')
                {
...
function get_usgi_applicable_addons($age,$vehicle_make)
{
    ...
    if($age <=15)
    {
       ...
       $is_ncb_protector_applicable = $addon_data['ncb_protector'] ?? 'False';
    }
    else 
    {
