...
include_once app_path() . '/Quotes/Car/V1/iffco_tokio.php';
function getQuote($enquiryId, $requestData, $productData) {
    if (config('IC.IFFCO_TOKIO.V1.CAR.ENABLE') == 'Y') {
     
        $iffcoTokio = new iffco_tokio();
        return $iffcoTokio->getQuoteV1($enquiryId, $requestData, $productData);
    } 
    ...
    //     ];
    // }else
    if($vehicle_age > 5 && $zero_dep && in_array($productData->company_alias, explode(',', config('CAR_AGE_VALIDASTRION_ALLOWED_IC'))) ) {
        return [
            'premium_amount' => 0,
            'status' => false,
    ...
    if(!$tp_only) {
        ...
        if (
            isset($arr_idv['ns1:erorMessage']['@attributes']['nil'])
            && $arr_idv['ns1:erorMessage']['@attributes']['nil'] === 'true'
        ) {
            $arr_idv['ns1:erorMessage'] = null; // Treat as null
        }

        if (!empty($arr_idv['ns1:erorMessage'])) {
            return [
                'webservice_id' => $data['webservice_id'],
                'table' => $data['table'],
                'premium_amount' => 0,
                'status' => false,
                'message' => $arr_idv['ns1:erorMessage']
            ];
        }
    ...
    if($data['response']) {
        ...
        if ($requestData->business_type == 'rollover' || $requestData->business_type == 'breakin') {
            ...
            $checkResponse = checkResponse($premium_data, $data, $ns);
            ...
            $discount_amount = !empty($premium_data[$ns.'discountLoadingAmt']) && is_scalar($premium_data[$ns.'discountLoadingAmt']) && is_numeric($premium_data[$ns.'discountLoadingAmt']) ? abs($premium_data[$ns.'discountLoadingAmt']) : 0;
        ...
        if ($premium_type != 'third_party' && $productData->zero_dep == 0 && $vehicle_age <= 5) {
            $add_ons_data['in_built']['zeroDepreciation'] = $dep_value;
            $add_ons_data['additional']['roadSideAssistance'] = $towing_related_cover;
            $add_ons_data['additional']['consumables'] = $consumable_value;
            $add_ons_data['additional']['ncb_protection'] = $ncb_protection_value;
        }
        ...
        if ($premium_data) {
            $data_response = [
                ...
                'Data' => [
                    ...
                    'aai_discount' => abs($aai_discount), //$automobile_association,
                    'voluntary_excess' => abs($voluntary_deductible), //$voluntary_excess,
            ...
            if($is_lpg_cng)
            {
                $data_response['Data']['cng_lpg_tp'] = $cng_tp_premium;
                $data_response['Data']['motor_lpg_cng_kit_value'] = $cng_od_premium;
