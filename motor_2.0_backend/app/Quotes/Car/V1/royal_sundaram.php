<?php
    ...
    function getQuoteV1($enquiryId, $requestData, $productData)
    {
        ...
        2. ZERO_DEP (With zero_dep - DepreciationWaiver,AggravationCover,TyreCoverClause,KeyReplacement,LossOfBaggage,InvoicePrice,WindShield,NCBProtector,consumabeCover)

        3. BASIC_ADDON (Except zero_dep - AggravationCover,TyreCoverClause,KeyReplacement,LossOfBaggage,InvoicePrice,WindShield,NCBProtector,consumabeCover)
        */
    ...
    //         ];
    // }
        // if ($vehicle_age > 10 && $productData->zero_dep == 0) {
        //     return [
        //         'status' => false,
        //         'premium' => '0',
    ...
    if ($requestData->fuel_type == 'PETROL' ) {
        ...
        if ($external_fuel_kit_amount < 100000 && $electrical_accessories_value < 100000 && $non_electrical_accessories_value < 100000) {
            ...
            {
                $data = getWsData(config('IC.ROYAL_SUNDARAM.V1.CAR.END_POINT_URL'), $premium_request, 'royal_sundaram', [
                    'enquiryId' => $enquiryId,
                    'requestMethod' =>'post',
            ...
            if ($data['response']) {
                ...
                if (isset($premium_response['PREMIUMDETAILS']['Status']['StatusCode']) && $premium_response['PREMIUMDETAILS']['Status']['StatusCode'] == 'S-0002') {
                    ...
                    $non_electrical_accessories_amt = $non_electrical_accessories == 'Yes' && !$tp_only ? round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['NON_ELECTRICAL_ACCESSORIES']) : 0;
                    ...
                    if($requestData->vehicle_owner_type == 'C'){
                        $employe_amount  = round($premium_response['PREMIUMDETAILS']['DATA']['LIABILITY']['TO_EMPLOYESES']);
                    }
                    ...
                    if (!$tp_only) {
                        if ($productData->zero_dep == 1) {
                            $addons_data = [
                                ...
                                'additional' => [
                                    ...
                                    'ncb_protection' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['NCB_PROTECTOR']),
                                    'key_replace' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['KEY_REPLACEMENT']),
                                    'tyre_secure' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['TYRE_COVER']),
                                    'consumables' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['CONSUMABLE_COVER']),
                                    // 'return_to_invoice' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['INVOICE_PRICE_INSURANCE']),
                                    'return_to_invoice' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['VEHICLE_REPLACEMENT_COVER']),
                                    'lopb' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['LOSS_OF_BAGGAGE']),
                                    'road_side_assistance' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ROADSIDE_ASSISTANCE_COVER']),
                                    'windShield' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['WIND_SHIELD_GLASS'] ?? 0)
                                ],
                                'other'=>[]
                            ];
                        } else {
                            $addons_data = [
                                ...
                                'additional' => [
                                    ...
                                    'return_to_invoice' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['VEHICLE_REPLACEMENT_COVER']),
                                    'lopb' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['LOSS_OF_BAGGAGE']),
                                    'road_side_assistance' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ROADSIDE_ASSISTANCE_COVER'] ?? 0),
                                    'windShield' => round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['WIND_SHIELD_GLASS'] ?? 0)
                                ],
                                'other'=>[]
                            ];
                        ...
                        if($productData->product_identifier == 'roadSideAssistancePlan2'){
                            $addons_data['additional']['road_side_assistance_2'] = round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['ROADSIDE_ASSISTANCE_COVER'] ?? 0);
                            unset($addons_data['additional']['road_side_assistance']);
                        };
                        if($hdn_invoice_price=='true' && round($premium_response['PREMIUMDETAILS']['DATA']['OD_PREMIUM']['VEHICLE_REPLACEMENT_COVER']) > 0){
                            $addons_data['other'] = [
                                'fullInvoicePrice'      =>0,
                        ...
                        $applicable_addons = [
                            'zeroDepreciation', 'keyReplace', 'engineProtector', 'ncbProtection', 'tyreSecure', 'returnToInvoice', 'lopb','roadSideAssistance', 'windShield' 
                        ];
                        if ($productData->product_identifier == 'roadSideAssistancePlan2') {
                    ...
                    if(config('IC.ROYAL_SUNDARAM.V1.CAR.ENABLE_CONSUMABLE_AS_BUILT_IN') == 'Y')
                    {
