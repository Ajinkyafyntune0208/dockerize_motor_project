...
function getQuote($enquiryId, $requestData, $productData)
{
    ...
    {
        include_once app_path() . '/Quotes/Car/V1/future_generali.php';
        return getQuoteV1($enquiryId, $requestData, $productData);
    }
    if(config('IC.FUTURE_GENERALI.V2.CAR.ENABLED') == 'Y')
    {
        include_once app_path() . '/Quotes/Car/V2/future_generali.php';
   ...
   $vehicleDate = !empty($requestData->vehicle_invoice_date) ? $requestData->vehicle_invoice_date : $requestData->vehicle_register_date;
    ...
    //     ];
    // } //https://github.com/Fyntune/motor_2.0_backend/issues/30367  no validation for third party quotes
    $with_zd = ($productData->zero_dep == '0') ? 'Y' : 'N';
    $fg_applicable_addons = futureGeneraliProposal::get_fg_applicable_addons($car_age,$with_zd);


    // removed all age validation as per #32029 
       ...
       // }
        ...
        $quote_array = [
            ...
            'Risk'         => [
                ...
                'Addon'             =>(((config('IS_ADDON_ENABLE_FG')=='N') || empty($addon)) ? ['CoverCode' => ''] : $addon ),
                'PreviousTPInsDtls' =>[
        ...
        if($is_data_exits_for_checksum['found'] && $refer_webservice && $is_data_exits_for_checksum['status']){
            ...
            $data = getWsData(config('constants.IcConstants.future_generali.END_POINT_URL_FUTURE_GENERALI'), $quote_array, 'future_generali', $additional_data);
        }
        ...
        if ($data['response'])
        {
            ...
            if (isset($quote_output['s:Body']['CreatePolicyResponse']['CreatePolicyResult']['Root']['Policy']))
            {
                ...
                if ($quote_output['Status'] == 'Fail') {
                    ...
                    {
                        $idv = ((float) $quote_output['VehicleIDV']);
                    ...
                    if ($data['response']) {
                        ...
                        else
                        {
                            ...
                            if (isset($quote_output['Root'])) { 
                                $quote_output = $quote_output['Root'];
                            }
                            ...
                            if(isset($quote_output['ValidationError']))
                            {
                                return $return_data = [
                                                ...
                                                'premium_amount' => 0,
                                                'status'         => false,
                                                'message'        =>  $quote_output['ValidationError'] ?? "Something went wrong" // changes done for #31415
                                              ];
                            }
                            elseif(isset($quote_output['ErrorMessage']))
                            {
                    ...
                    $vehicleIDV = $quote_output['VehicleIDV'] ?? 0; 
                    $total_idv = ($premium_type == 'third_party') ? 0 : (is_numeric($vehicleIDV) ? ($vehicleIDV):0);
                    $total_od_premium = 0;
                    $total_tp_premium = 0;
                    $od_premium = 0;
                    $tp_premium = 0;
                    ...
                    $data_response = [
                        ...
                        'Data' => [
                            ...
                            'motor_electric_accessories_value' =>($electrical_amount),
                            'motor_non_electric_accessories_value' => ($non_electrical_amount),
                    ...
                    {
                        $data_response['Data']['motor_lpg_cng_kit_value'] = ($lpg_cng_amount);
                    ...
                    if (!empty($other_addons['LegalLiabilityToEmployee'])) {
                        $data_response['Data']['LegalLiabilityToEmployee'] = $other_addons['LegalLiabilityToEmployee'];
                    }
