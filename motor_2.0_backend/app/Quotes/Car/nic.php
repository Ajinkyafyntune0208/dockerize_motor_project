...
use App\Http\Controllers\Proposal\Services\Car\NicSubmitProposal as NIC;

include_once app_path() . '/Helpers/CarWebServiceHelper.php';
function getQuote($enquiryId, $requestData, $productData)
{
    if(config('IC.NIC.V2.CAR.ENABLE') == 'Y'){
        include_once app_path() . '/Quotes/Car/V2/nic.php';
...
foreach ($addons as $key => $value) {
    ...
    if (in_array('Loss of Personal Belongings', $value)) {
        ...
        $addon['Loss_of_belonging'] = ((!$is_liability && ($vehicle_age < 5)) ? true : false);
        ...
        else
        {
            ...
            $tp_end_date        = date('d-m-Y', strtotime($tp_start_date.'+3Y'));
        }
        ...
        if(!$is_new)
        {
            if($age <= 6)
            {
                $mmv->idv = $mmvdata['idv_6mon'];
            }
            else if($age > 6 && $age <= 12)
            {
                $mmv->idv = $mmvdata['idv_6monto1yr'];
            }
            else
            {
                for ($i=1; $i < 10; $i++) {
        ...
        // else
        // {
        //     $customer_id = '9520977405';//$customer_data['customer_id'];
        // }
        $customer_id = '9520977405'; //$customer_data['customer_id'];
        ...
        $quote_request_data['od_package'] = NIC::create_od_request($policy_date, $is_applicable, $registration_year, $is_new, $mmv_data, $is_liability,$amount);
        ...
        $quote_request_data['mmv'] = NIC::create_mmv_details_request($is_applicable, $addon, $mmv, $rto_data, $registration_year, $motor_manf_date);
        ...
        $quote_request_data['key_replace']       =  NIC::create_lock_and_key_protect_request($policy_date, $registration_date, $mmv_data, $is_new);
        $quote_request_data['consumable']        = NIC::create_Consumables_Protect_request($policy_date, $registration_date, $mmv_data, $is_new);
        $quote_request_data['Loss_of_belonging'] = NIC::create_Loss_Of_Belonging_request($policy_date, $registration_date, $mmv_data, $is_new);
        ...
        if ($get_response['response']) {
            ...
            $quote_request_data['Loss_of_belonging']['siOfCoverType'] = $idv;
            $quote_request_data['consumable']['siOfCoverType'] = $idv;
            $quote_request_data['key_replace']['siOfCoverType'] = $idv;
            ...
            if($addon['engine_protect'])
            {
                array_push(
                    $idv_quote_array['policySOABO']['insuredSOABOList'][0]['policyCtSOABOList'], 
                    $quote_request_data['engine_protect']);
            }

            if ($addon['key_replace']) {
                array_push(
                    $idv_quote_array['policySOABO']['insuredSOABOList'][0]['policyCtSOABOList'], 
                    $quote_request_data['key_replace']);
            }
            if ($addon['consumable']) {
                array_push(
                    $idv_quote_array['policySOABO']['insuredSOABOList'][0]['policyCtSOABOList'], 
                    $quote_request_data['consumable']);
            }
            if ($addon['Loss_of_belonging']) {
                array_push(
                    $idv_quote_array['policySOABO']['insuredSOABOList'][0]['policyCtSOABOList'], 
                    $quote_request_data['Loss_of_belonging']);
            }
            // return $quote_array;
            ...
            $PA_To_PaidDriver       = $response['policyCtAcceSOABOList']['PA To PaidDriver Conductor Cleaner']?? 0;
            $lpg_cng_amount         = //$response['policyCtAcceSOABOList']['Non Electrical Accessories']?? 0;
            $lpg_cng_tp_amount      = '0';
            ...
            if (!$addon['key_replace']) {
                array_splice($applicable_addons, array_search('keyReplacement', $applicable_addons), 1);
            }
            if (!$addon['consumable']) {
                array_splice($applicable_addons, array_search('Consumable', $applicable_addons), 1);
            }
            if (!$addon['Loss_of_belonging']) {
                array_splice($applicable_addons, array_search('Loss_Of_Belonging', $applicable_addons), 1);
            }
            ...
            $data_response = [
                ...
                'premium_data' => [
                    ...
                    'covers'                    => [
                        ...
                        'pa_paid_driver'            => $PA_To_PaidDriver,
