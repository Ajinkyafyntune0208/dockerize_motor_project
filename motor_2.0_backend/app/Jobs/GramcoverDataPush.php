...
class GramcoverDataPush implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // $token_response = httpRequest('gramcover_token');
        // $token_response = $token_response['response'];
        // if (isset($token_response['status_code']) && $token_response['status_code'] == 3001) {
        //     $token = $token_response['result']['token'];
        // } else {
        //     return response()->json([
        //         'status' => false,
        //         'msg' => $token_response['result'] ?? $token_response
        //     ]);
        // }
        $token='';
        $user_product_journey_ids = \App\Models\GramcoverPostDataApi::where(['status' => 'success'])->get(['user_product_journey_id'])->pluck('user_product_journey_id')->toArray();
        $reports = \App\Models\UserProductJourney::with([
            ...
            'sub_product',
            'addons'
        ])->whereNotIn('user_product_journey_id', $user_product_journey_ids)
        ...
        if (empty($this->enquiryId)) {
           ...
           $reports = $reports->where('user_product_journey_id', $this->enquiryId);
           $tokendata= CvAgentMapping::where('user_product_journey_id', $this->enquiryId)
        ...
        foreach ($reports as $report) {          
            ...
            })->where([ 'user_product_journey_id' => $report->user_product_journey_id])->get(['user_product_journey_id'])->pluck('user_product_journey_id')->toArray();
            if($user_product_journey_ids != null && !empty($user_product_journey_ids)){
                continue;
            }
            ...
            $temp['trace_id'] = customEncrypt($report->user_product_journey_id) ?? "";
            ...
            if ($report->agent_details) {
                foreach ($report->agent_details as $key => $agent_detail) {
                    if (in_array($agent_detail->seller_type, ['E', 'P'])) {
            ...
            if ($temp['product_type'] == 'CAR') {
                ...
                } else if ($temp['policy_type'] == 'Own Damage' && ($temp['business_type'] == 'rollover' || $temp['business_type'] == 'breakin')) {
                    $temp['tp_start_date'] = $report->user_proposal->tp_start_date ?? "";
                    $temp['tp_end_date'] = $report->user_proposal->tp_end_date ?? "";
