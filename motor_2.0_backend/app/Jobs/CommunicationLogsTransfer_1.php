...
class CommunicationLogsTransfer_1 implements ShouldQueue
{
    ...
    {
        DB::table('communication_logs_1')->whereDate('created_at', Carbon::today()->subDay(12))
        ->chunkbyId(100, function ($logs_data) 
        {
            foreach ($logs_data as $key => $value)
            {
                ...
                if($inserted_data)
                {
                    DB::table('communication_logs_1')->where('id',$last_id)->delete();
                }
            }
        });
