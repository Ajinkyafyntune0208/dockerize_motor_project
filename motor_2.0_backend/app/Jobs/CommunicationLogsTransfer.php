...
class CommunicationLogsTransfer implements ShouldQueue
{
    ...
    {
        DB::table('communication_logs')->whereDate('created_at', Carbon::today()->subDay(1))
        ->chunkbyId(100, function ($logs_data) 
        {
            foreach ($logs_data as $key => $value)
            {
                ...
                if($inserted_data)
                {
                    DB::table('communication_logs')->where('id',$last_id)->delete();
                }
            }
        });
