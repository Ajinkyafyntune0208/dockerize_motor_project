...
class AbiblDataMigrationJobOld implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        set_time_limit(0);
        $files = Storage::allFiles('abil-data-migration-uploads-old');
        foreach ($files as $key => $file) {
            ...
            Storage::makeDirectory('abibl-data-migration-data-old');
            foreach ($data as $key => $value) {
                $data = array_merge($header, $value);
                $new_file = Storage::path('abibl-data-migration-data-old/' . \Illuminate\Support\Str::uuid()->toString() . '.csv');
