...
class CommunicationLogsTransfer_4 implements ShouldQueue
{
    ...
    {
        DB::table('communication_logs_4')->whereDate('created_at', Carbon::today()->subDay(1))
        ->chunkbyId(100, function ($logs_data) 
        {
            foreach ($logs_data as $key => $value)
            {
                ...
                if($inserted_data)
                {
                    DB::table('communication_logs_4')->where('id',$last_id)->delete();
                }
            }
        });
