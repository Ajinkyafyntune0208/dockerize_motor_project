...
class DataPushResReqLogController extends Controller
{
    public function index(Request $req){
        ...
        if (!auth()->user()->can('datapushlog.list')) {
            return [
                'status' => false,
        ...
        $enquiryId = acceptBothEncryptDecryptTraceId($enqid);

        if ( $enqid != null || $df != null || $dt != null 
        ) {
            $logs = DatapushReqResModel::
                when(!empty($enqid), function ($query) use($enquiryId) {
        ...
        return view ('datapush_logs.index',compact('logs'));
    }
    public function datapushView(Request $req){
        ...
        return view('datapush_logs.show', compact('log'));
    }
