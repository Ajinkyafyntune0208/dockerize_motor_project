...
class TraceJourneyIdController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!auth()->user()->can('trace_journey_id.list')){
            ...
            try {
                ...
                switch ($request->type) {
                    case 'rcNumber':
                        ...
                        $traceIdDetails = UserProposal::selectRaw("CONCAT(DATE_FORMAT(j.created_on,'%Y%m%d'),LPAD(j.user_product_journey_id,8,0)) AS `TraceID`,
                                 user_proposal.vehicale_registration_number AS `RCNumber`,
                                 payment.status AS `status`,
                                 br.breakin_number as breakinNumber,
                                 policy.policy_number AS `policyNumber`,
                                 policy.pdf_url AS `pdfURL`")
                            ->join('user_product_journey as j', 'j.user_product_journey_id', '=', 'user_proposal.user_product_journey_id')
                            ->leftJoin('payment_request_response as payment', function ($join) {
                                $join->on('user_proposal.user_product_journey_id', '=', 'payment.user_product_journey_id')
                                    ->where('payment.active', '=', '1');
                            })
                            ->leftJoin('cv_breakin_status as br', 'br.user_proposal_id', '=', 'user_proposal.user_proposal_id')
                            ->leftJoin('policy_details as policy', 'payment.user_proposal_id', '=', 'policy.proposal_id')
                            // ->whereRaw("REPLACE(user_proposal.vehicale_registration_number,'-','') = ? ", [$userInput])
                    ...
                    case 'engineNo':
                        $traceIdDetails = UserProposal::selectRaw("CONCAT(DATE_FORMAT(j.created_on,'%Y%m%d'),LPAD(j.user_product_journey_id,8,0)) AS `TraceID`,
                            ...
                            ->leftJoin('cv_breakin_status as br', 'br.user_proposal_id', '=', 'user_proposal.user_proposal_id')
                            ->leftJoin('policy_details as policy', 'payment.user_proposal_id', '=', 'policy.proposal_id')
                            // ->whereRaw("UPPER(user_proposal.engine_number) = ? ", [strtoupper($userInput)])
                            ->where("user_proposal.engine_number", '=', $userInput)
                            ->orderBy('j.user_product_journey_id')
                            ->get();
                        break;
                    case 'chassisNo':
                        $traceIdDetails = UserProposal::selectRaw("CONCAT(DATE_FORMAT(j.created_on,'%Y%m%d'),LPAD(j.user_product_journey_id,8,0)) AS `TraceID`,
                            ...
                            ->leftJoin('cv_breakin_status as br', 'br.user_proposal_id', '=', 'user_proposal.user_proposal_id')
                            ->leftJoin('policy_details as policy', 'payment.user_proposal_id', '=', 'policy.proposal_id')
                            // ->whereRaw("UPPER(user_proposal.chassis_number) = ? ", [strtoupper($userInput)])
                            ->where("user_proposal.chassis_number", '=', $userInput)
                            ->orderBy('j.user_product_journey_id')
                            ->get();
                        break;
                }
        ...
        return view('admin.trace-journey-id.index', compact('traceIdDetails', 'data', 'options'));
    }
