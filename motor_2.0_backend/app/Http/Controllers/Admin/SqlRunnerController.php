    ...
    public function index(Request $request)
    {
        ...
        if (!empty($request->sql_query)){
            ...
            if ($records['status'] == false){
                ...
                if ($records['is_validation_error'] == true ){

                    return redirect()->route('admin.sql_runner')->withInput()->with([
                        'errors'=> $records['message'],
                        'class' => 'warning',
                        'records' => []
                    ]);

                } else if ($records['is_syntax_error'] == true){
        ...
        if ($request->has('errorbol') &&$request->errorbol == true){
            return view('sql_runner.index');
        } 
        else if($request->download == 'rundownload' && !empty($request->sql_query)){
            return $this->runAndDownload($records,$headings);
        }
        else if (isset($records) && isset($headings) ) {
            return view('sql_runner.index',compact('headings','records'));
        }
