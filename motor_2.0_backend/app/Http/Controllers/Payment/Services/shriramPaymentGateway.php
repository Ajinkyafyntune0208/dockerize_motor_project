...
use App\Http\Controllers\Payment\Services\shriramgcvpaymentgateway;
use App\Http\Controllers\Payment\Services\V1\GCV\shriramPaymentGateway as GCVPaymentGateway; 
use App\Http\Controllers\Payment\Services\V1\PCV\shriramPaymentGateway as PCVPaymentGateway; 
...
// includeFile(app_path('Helpers/CvWebServiceHelper.php')); getting error in ola uat
...
class shriramPaymentGateway
{
    ...
    public static function make($request)
    {
       ...
       {
            $shriramgcvpayment = new shriramgcvpaymentgateway();
            return $shriramgcvpayment->make($request);
        }
        ...
        $return_data = [
            'form_action' => ($service_type == 'XML' && in_array($master_policy->product_sub_type_id, [6, 9]))
            ? config('constants.cv.shriram.SHRIRAM_XML_PAYMENT_URL_PCV')
            : config('constants.IcConstants.shriram.SHRIRAM_PAYMENT_BEFORE_URL'),
            ...
            'form_data' => [
                ...
                'application' => config('constants.brokerName'),
                'createdBy' => config('constants.IcConstants.shriram.SHRIRAM_USERNAME'),
    ...
    {       
        ...
        // START : Payment condition based on JSON kit     
        ...
        if ($service_type == 'JSON' && get_parent_code($productData->product_sub_type_id) == 'GCV') 
        {
            $shriramgcvpayment = new shriramgcvpaymentgateway();
            return $shriramgcvpayment->JSONGcvConfirm($request, $user_proposal);
        }         
        else if ($service_type == 'JSON') 
        {
            return self::JSONConfirm($request, $user_proposal);
        }
        // END : Payment condition based on JSON kit
        ...
        $input_array = [
            'soap:Header' => [
                'AuthHeader' => [
                    ...
                    'Username' => $service_type == 'XML' ? config('constants.cv.shriram.SHRIRAM_XML_AUTH_USERNAME_PCV') : config('constants.IcConstants.shriram.SHRIRAM_USERNAME'),
                    'Password' => $service_type == 'XML' ? config('constants.cv.shriram.SHRIRAM_XML_AUTH_PASSWORD_PCV') : config('constants.IcConstants.shriram.SHRIRAM_PASSWORD'),
                ],
            ],
            'soap:Body' => [
                'PolicyApprove' => [
        ...
        $get_response = getWsData(config('constants.IcConstants.shriram.SHRIRAM_POLICY_APPROVED_URL'), $input_array, 'shriram', $additional_data);
    ...
    static function JSONConfirm($request, $user_proposal)
    {
        $data['user_product_journey_id'] = $user_proposal->user_product_journey_id;
        $data['ic_id'] = $user_proposal->ic_id;
        ...
        if ($rehit_payment_api_response)
        {
            ...
            {   
                ...
                foreach ($rehit_payment_api_res['Response'] as $transaction) {
                ...
                if ($successfulTransaction) {
                    ...
                    $request->PolicyNumber = $successfulTransaction['PolicyNo'];
                    $request->PolicyURL =  $successfulTransaction['PolicyURL'];
                }
        ...
        if ($request->Status != 'Success') { // Paytm will send 'ResponseMsg' only if the payment is unsuccessful/fail
            $data['stage'] = STAGE_NAMES['PAYMENT_FAILED'];
            updateJourneyStage($data);
            return redirect(paymentSuccessFailureCallbackUrl($user_proposal->user_product_journey_id, 'CV', 'FAILURE'));
        }
    ...
    static public function generatePdf($request)
    {
        ...
        if (policyProductType($request['master_policy_id'])->parent_id == 4  &&   config('IC.SHRIRAM.V1.GCV.ENABLE') == 'Y') {

            $shriramgcvpayment = new GCVPaymentGateway();
            return $shriramgcvpayment->generateJSONPDF($request);
        }
      elseif  (policyProductType($request['master_policy_id'])->parent_id == 4  &&  config('constants.cv.shriram.SHRIRAM_CV_REQUEST_TYPE') == 'JSON') {
      
            $shriramgcvpayment = new shriramgcvpaymentgateway();
            return $shriramgcvpayment->generateJSONPDF($request);
         } 
          elseif (policyProductType($request['master_policy_id'])->parent_id == 8  &&  config('IC.SHRIRAM.V1.PCV.ENABLE') == 'Y') {

            $shrirampcvpayment = new PCVPaymentGateway();
            return $shrirampcvpayment->generateJSONPDF($request);
        }
       
        if(config('constants.cv.shriram.SHRIRAM_CV_REQUEST_TYPE') == 'JSON'){
    ...
    static public function generateJSONPDF($request){
        $user_product_journey_id = customDecrypt($request->enquiryId);
        $service_type = config('constants.cv.shriram.SHRIRAM_CV_REQUEST_TYPE');
        if (policyProductType($request['master_policy_id'])->parent_id == 4 && $service_type == 'JSON' && config('IC.SHRIRAM.V1.GCV.ENABLE') == 'Y') {

            $shriramgcvpayment = new GCVPaymentGateway();
            return $shriramgcvpayment->generateJSONPDF($request);
        }
       elseif (policyProductType($request['master_policy_id'])->parent_id == 4 && $service_type == 'JSON') {
            $shriramgcvpayment = new shriramgcvpaymentgateway();
            return $shriramgcvpayment->generateJSONPDF($request);
        }
        elseif (policyProductType($request['master_policy_id'])->parent_id == 8  &&  config('IC.SHRIRAM.V1.PCV.ENABLE') == 'Y') {
    ...
    $response = json_decode($payment_response->response);
        ...
        $get_response = getWsData(config('constants.cv.iffco.SHRIRAM_POLICY_PDF_URL'), $Policy_payment_api_request, 'shriram', [
            'enquiryId' => $user_product_journey_id,
            'headers' => [
                'Username' => config('constants.IcConstants.shriram.SHRIRAM_POLICY_USERNAME'),
                'Password' => config('constants.IcConstants.shriram.SHRIRAM_POLICY_PASSWORD'),
                'Accept' => 'application/json',
                'Content-Type' => 'application/json',
            ],
            'requestMethod' => 'post',
