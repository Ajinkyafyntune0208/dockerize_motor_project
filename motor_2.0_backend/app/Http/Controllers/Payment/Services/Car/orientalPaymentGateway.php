...
class orientalPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        $return_data = [
            'form_action'       => config('constants.IcConstants.oriental.car.END_POINT_URL_PAYMENT_GATEWAY_CAR'),
            'form_method'       => 'POST',
        ...
        DB::table('payment_request_response')->insert([
            ...
            'payment_url'               => config('constants.IcConstants.oriental.car.END_POINT_URL_PAYMENT_GATEWAY_CAR'),
            'return_url'                => route(
                'car.payment-confirm',
                [
                    'oriental',
    ...
    public static  function  create_checksum($enquiryId ,$request)
    {
        ...
        $new_pg_transaction_id = strtoupper(config('constants.IcConstants.oriental.car.CAR_PAYMENT_IDENTIFIER')).date('Ymd').time().rand(10,99);

        $str_arr = [
            config('constants.IcConstants.oriental.car.CAR_PAYMENT_MERCHANT_ID'),
            ...
            'NA',
            'R',
            config('constants.IcConstants.oriental.car.CAR_PAYMENT_USER_ID'),
            'NA',
            'NA',
            'F',
            $data->proposal_no,
            config('constants.IcConstants.oriental.car.CAR_PAYMENT_IDENTIFIER'),
            customDecrypt(customEncrypt($enquiryId)),
            $data->email,
            $data->chassis_number,
            $data->first_name.' '.$data->last_name,
            'NA',
            route('car.payment-confirm', [
                    'oriental',
        ...
        $checksum = strtoupper(hash_hmac('sha256', $msg_desc, config('constants.IcConstants.oriental.car.CAR_PAYMENT_CHECKSUM_KEY')));
    ...
    static public function PolicyPDFService($proposal, $product_data)
    {
        try
        {
            ...
            {
                $pdf_doc_url = config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'oriental/'.md5($proposal->user_proposal_id).'.pdf';
