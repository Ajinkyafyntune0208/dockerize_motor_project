...
use App\Http\Controllers\Proposal\Services\Car\iciciLombardSubmitProposal;
...
class iciciLombardPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        $enquiry = customDecrypt($request->enquiryId);
        $request_Data = getQuotation($enquiry);
        ...
        $proposal = UserProposal::where('user_product_journey_id', ($request['userProductJourneyId']))->first();
        ...
        if (
            $isBreakinCase &&
            !empty($request_Data->previous_policy_expiry_date) &&
            strtoupper($request_Data->previous_policy_expiry_date) != 'NEW' &&
            config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_INSPECTION_WAIVED_OFF') == 'Y'
        ) {
            $date1 = new DateTime($request_Data->previous_policy_expiry_date);
            $date2 = new DateTime();
        ...
        $breakinDetails = CvBreakinStatus::where('user_proposal_id', trim($proposalData['user_proposal_id']))->first();

        if($proposalData->is_breakin_case == 'Y' && config('PROPOSAL_RESUBMIT_FOR_BREAKIN') == 'Y' && $breakinDetails->breakin_status == STAGE_NAMES['INSPECTION_APPROVED'])
        {   
            $submitProposalResponse = self::proposalSubmit($request);
            $proposalData = UserProposal::where('user_product_journey_id', $request['userProductJourneyId'])->first();
        ...
        $payemntTokenRequest = [
            ...
            "ReturnURL" => route('car.payment-confirm', 'icici_lombard'),
        ...
        if (!empty($premRecalculateResponse)) {
            ...
            PaymentRequestResponse::insert([
                ...
                'amount'                  => $proposalData->final_payable_amount,
                'return_url'              => route('car.payment-confirm', 'icici_lombard'),
                'active'                  => 1
            ]);
    ...
    public static function confirm($request)
    {
        if(count($request->all()) == 1 && isset($request->enquiry_id)){
            ...
            //return redirect(config('constants.motorConstant.CAR_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($request->enquiry_id)]));
        }
        ...
        if(!$pg_status) {
            ...
            return redirect(paymentSuccessFailureCallbackUrl($enquiryId,'CAR','FAILURE'));
            //return redirect(config('constants.motorConstant.CAR_PAYMENT_FAILURE_CALLBACK_URL') . '?' . http_build_query(['enquiry_id' => customEncrypt($userDetails[0]->user_product_journey_id)]));
        }
            ...
            if ($response['Status'] == '0' || $response['Status'] == 'True')
            {
                ...
                if (!empty($token) && isset($tokenResponse['access_token'])) 
                {
                    ...
                    $pos_data = DB::table('cv_agent_mappings')
                        ...
                        ->where('seller_type','P')
                        ->first();
                    $is_pos_enabled = ($requestData->is_renewal == 'Y' && $requestData->rollover_renewal == 'N') ? 'N' : $is_pos_enabled;                        
                            ...
                            if ($generatedPolicy['paymentTagResponse'] != 'null') 
                            {
                                if(strtolower($generatedPolicy['paymentTagResponse']['paymentTagResponseList'][0]['status']) == 'success')
                                {
                                    ...
                                    if (!empty($tokenforPdfGen) && isset($generatedtoken['access_token'])) 
                                    {
                                        ...
                                        if (preg_match("/^%PDF-/", $pdfGenerationResponse)) 
                                        {
                                            ...
                                            $proposal_pdf = Storage::put(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($userDetails[0]->user_proposal_id) . '.pdf', $pdfGenerationResponse);
                                           

                                            PolicyDetails::updateOrCreate(['proposal_id' => $userDetails[0]->user_proposal_id],
                                                [
                                                    'pdf_url' => config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($userDetails[0]->user_proposal_id) . '.pdf',
                                                ]);
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                         /* return response()->json([
                                            'status' => false,
                                            'msg' => "Error in policy Generation",
                                        ]); */
                                        $enquiryId = $userDetails[0]->user_product_journey_id;
                                        return redirect(paymentSuccessFailureCallbackUrl($enquiryId,'CAR','SUCCESS'));
                                        //return redirect(config('constants.motorConstant.CAR_PAYMENT_SUCCESS_CALLBACK_URL') . '?' . http_build_query(['enquiry_id' => customEncrypt($userDetails[0]->user_product_journey_id)]));
                    ...
                    else 
                    {
                        /* return response()->json([
                            'status' => false,
                            'msg' => "Insurer not reachable. No response received from policy number generation service",
                        ]); */
                ...
                else 
                {
                    /* return response()->json([
                        'status' => false,
                        'msg' => "Insurer not reachable.No response received from Token Generation service",
                    ]); */
    ...
    static public function retry_pdf($user_proposal)
    {
        ...
        if (preg_match("/^%PDF-/", $pdfGenerationResponse)) 
        {
            Storage::put(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($user_proposal->user_proposal_id) . '.pdf', $pdfGenerationResponse);

            PolicyDetails::where('proposal_id', $user_proposal->user_proposal_id)
               ->update([
                'pdf_url' => config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($user_proposal->user_proposal_id) . '.pdf',
              ]);
              updateJourneyStage([
          ...
          return response()->json([
                ...
                'data' => [
                    'pdf_link' => file_url(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($user_proposal->user_proposal_id) . '.pdf'),
    ...
    public static function generatePdf($request)
    {
        ...
        /* else if((isset($policy_details->response['AuthCode']) && empty($policy_details->response['AuthCode'])) || (isset($policy_details->response['PGTransactionId']) && empty($policy_details->response['PGTransactionId'])))
        {
            $payment_status_data = self::update_pg_response($user_product_journey_id,$policy_details->unique_proposal_id);
            if($payment_status_data['status'] == 'false')
            {
                            ...
                            if ($generatedPolicy['paymentTagResponse'] != 'null') 
                            {
                                if(strtolower($generatedPolicy['paymentTagResponse']['paymentTagResponseList'][0]['status']) == 'success')
                                {
                                    ...
                                    if (!empty($tokenforPdfGen) && isset($generatedtoken['access_token'])) 
                                    {
                                        ...
                                        if (preg_match("/^%PDF-/", $pdfGenerationResponse)) 
                                        {
                                            $pdf_name = config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'icici_lombard/' . md5($proposal->user_proposal_id) . '.pdf';
    ...
    static public function update_pg_response($enquiry_id,$correlation_id)
    {
        ...
        /* $tokenUrl = config('constants.IcConstants.icici_lombard.TRANSACTION_ENQUIRY_END_POINT_URL_ICICI_LOMBARD_MOTOR') . $orderid->order_id;
        
        $result = getWsData($tokenUrl, '', 'icici_lombard', $additionalData);
        $response = json_decode($result, true);
        if ($response['Status'] == '0' || $response['Status'] == 'True')
        {
    ...
    static public function checkTransationStatus($request)
    {
        $tokenUrl = config('constants.IcConstants.icici_lombard.TRANSACTION_ENQUIRY_END_POINT_URL_ICICI_LOMBARD_MOTOR') . $request->order_id;
    ...
    static public function policyGeneration($request)
    {
        $enquiryId = $request->enquiryId;
        $tokenParam = [
            'grant_type'    => 'password',
        ...
        if (!empty($token) && isset($tokenResponse['access_token'])) 
        {
            ...
            $paymentEntryRequestArr = [
                'CorrelationId' => $request->CorrelationId, //$request->TransactionId,   #Proposal Page corelation id
                // 'DealId' => config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_DEAL_ID'),
                //'DealId' => $ICICI_LOMBARD_DEAL_ID,
                'isTaggingRequired' => true,
                'isMappingRequired' => true,
    ...
    static public function pdfGeneration($request)
    {
        $enquiryId = $request->enquiryId;
        ...
        if (!empty($tokenforPdfGen) && isset($generatedtoken['access_token'])) 
        {
            ...
            $pdfGenerationResponse = getWsData($policyPdfUrl, $policypdfdata, 'icici_lombard', $additionData);
            return $pdfGenerationResponse['response'];
        }
    }
    static public function proposalSubmit($request)
    {


        $user_product_journey_id = $request['userProductJourneyId'];
        $enquiryId = $request['userProductJourneyId'];
        $proposal_request = UserProposal::where('user_product_journey_id', $user_product_journey_id)->first();
        $deal_id = config('constants.IcConstants.icici_lombard.CALCULATE_PREMIUM_DEAL_ID_ICICI_LOMBARD_MOTOR_BREAKIN');
        $proposalAdditionDetailsData =  json_decode($proposal_request->additional_details_data, true);
        
        $proposalServiceRequest = $proposalAdditionDetailsData;
        ...
        $IRDALicenceNumber = $CertificateNumber = $PanCardNo = $AadhaarNo = $ProductCode = '';
        $pos_data = DB::table('cv_agent_mappings')
            ->where('user_product_journey_id',$proposal_request->user_product_journey_id)
            ->where('user_proposal_id',$proposal_request->user_proposal_id)
            ->where('seller_type','P')
            ->first();
        ...
        if($is_pos_enabled == 'Y' && isset($pos_data->seller_type) && $pos_data->seller_type == 'P')
        {
            ...
            $ProductCode = in_array($premium_type, ['breakin', 'own_damage_breakin']) ? '2311' : '2319'; #config('constants.IcConstants.icici_lombard.PRODUCT_CODE_ICICI_LOMBARD_MOTOR');
        }
        elseif($pos_testing_mode === 'Y')
        {
            $IsPos = 'Y';
        ...
        if(in_array($premium_type, ['breakin', 'own_damage_breakin'])){
            $proposalServiceRequest['PolicyStartDate'] = Carbon::parse($policyStartDate)->format('d-M-Y');
            $proposalServiceRequest['PolicyEndDate'] = Carbon::parse($policyEndDate)->format('Y-m-d');
        }
        // $$proposalServiceRequest['CorrelationId'] = $corelationId;

        $get_response = getWsData($url, $proposalServiceRequest, 'icici_lombard', $additionPremData);
            ...
            if ($arr_premium['status'] == 'Success') {
                ...
                if(in_array($premium_type, ['breakin', 'own_damage_breakin'])){
                    ...
                    ->update([
                        ...
                        'final_payable_amount' => $arr_premium['finalPremium'],
                ...
                //BREAKIN INSPECTION CALLING:
                if (!empty($updateProposal)) {
                $breakin_id = DB::table('cv_breakin_status')
                    ->where('user_proposal_id', $proposal_request['user_proposal_id'])
                    ->pluck('breakin_number')
                    ->first();

                $breakinDetails = UserProposal::where('cv_breakin_status.breakin_number', '=', $breakin_id)
                ...
                $updated_data['breakinId'] = $breakin_id;
                $updated_data['correlationId'] = $proposalServiceRequest['CorrelationId'];
                $updated_data['token'] = $access_token;
                $breakinDetails->created_at =  date('Y-m-d');
                    $check_inspection = iciciLombardBreakInClearStatusApi($updated_data, $breakinDetails, $additionData);
                    if ($check_inspection['response']) {
                        if (isset($check_inspection['vehicleInspectionStatus']) && $check_inspection['vehicleInspectionStatus'] == "FAIL") {
                            $return_data = [
                                'status' => false,
                                'message' => $check_inspection['message']
                            ];
                        }
                    } else {
                        $return_data = [
                            'status' => false,
                            'message' => "No response received from breakin clear status service"
                        ];
                    }
                }
            //END OF BREAKIN INSPECTION CALLING:
