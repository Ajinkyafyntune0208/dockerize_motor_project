...
class GoDigitPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        if ($proposal) {
            ...
            $integrationId = config('IC.GODIGIT.V2.CAR.PAYMENT_INTEGRATION_ID');
                ...
                if ($breakinDetails->breakin_status_final == STAGE_NAMES['INSPECTION_APPROVED']) {
                    $pg_request =  [
                        "motorNewPaymentGenService" => [
                            "paymentMode" => "EB",//as confirmed with ic for all cases we have pass that default value
                    ...
                    if (!empty($data)) {
                        ...
                        if ((isset($check->timestamp) && isset($check->message)) || (empty($check->paymentLink))) {
                            return [
                                'status' => false,
    ...
    public static function confirm($request)
    {
        ...
        $product = getProductDataByIc ($request_data['policy_id'] ?? $request['policyId']);
        ...
        $appno = [
            "motorPolicyPDF(usingapplicationid)" => [
                ...
                "headerParam" => [
                    'Authorization' => config('IC.GODIGIT.V2.CAR.PG_AUTHORIZATION'),
                ],
            ]
        ];
        ...
        $integrationId = config('IC.GODIGIT.V2.CAR.PDF_INTEGRATION_ID');
        $posData = CvAgentMapping::where([
            'user_product_journey_id' => $enquiryId,
            'seller_type' => 'P'
        ])
