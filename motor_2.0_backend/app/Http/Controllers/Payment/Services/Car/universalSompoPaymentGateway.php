...
use App\Http\Controllers\Proposal\Services\Car\universalSompoSubmitProposal;
...
class universalSompoPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        if ($proposal) {
            ...
            PaymentRequestResponse::insert([
                ...
                'return_url'                => route('car.payment-confirm', ['universal_sompo']),
    ...
    public static function rehitpdf($enquiryId, $policy_no, $pdf_url)
    {
        if ($policy_no && $enquiryId) 
        {
            ...
            try
            {
                ...
                Storage::put(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf', $pdf_data);
                PolicyDetails::where('proposal_id', $proposal->user_proposal_id)
                ->update([
                    'pdf_url' => config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf',
                ]);
                updateJourneyStage([
                    'user_product_journey_id' => $proposal->user_product_journey_id,
                    'stage' => STAGE_NAMES['POLICY_ISSUED']
              ...
              return [
                    ...
                    'data' => [
                        'pdfUrl' => file_url(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf'),
                    ]];
    ...
    static public function generatePdf($request)
    {
        ...
        if(empty($policy_details->response) || ($policy_details->policy_number == "NA"))
        {
    ...
    static public function update_pg_response($enquiry_id)
    {
        ...
        $payment_response = PaymentRequestResponse::where('user_product_journey_id', $user_product_journey_id)
            ...
            ->pluck('proposal_no')
            ->toArray();
        $proposal_numbers = array_unique(array_merge($payment_response, [$proposal_no]));
        foreach ($proposal_numbers as $proposal_no) {
