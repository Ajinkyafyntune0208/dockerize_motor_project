...
class futureGeneraliPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        $return_data = [
            'form_action' => config('constants.IcConstants.future_generali.PAYMENT_GATEWAY_LINK_FUTURE_GENERALI_MOTOR'),
            'form_method' => 'POST',
            'payment_type' => config('constants.IcConstants.future_generali.PAYMENT_TYPE', 0),
            'form_data' => [
                'PaymentOption' => config('constants.IcConstants.future_generali.PAYMENT_OPTION', 3),
        ...
        PaymentRequestResponse::insert([
            ...
            'ic_id'                     => '28',
            'payment_url'               => config('constants.IcConstants.future_generali.PAYMENT_GATEWAY_LINK_FUTURE_GENERALI_MOTOR'),
    ...
    public static function confirm($request)
    {
        ...
        if(($request->Response ?? '') == 'Success' && !empty($enqId))
        {
            ...
            try{
                ...
                //$return_url = config('constants.motorConstant.CAR_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($enqId)]);
            }catch(\Exception $e)
            {
        ...
        if(!isset($request->TID) || empty($request->TID))
        {
            ...
            //return redirect(config('constants.motorConstant.CAR_PAYMENT_SUCCESS_CALLBACK_URL'). '?' . http_build_query(['enquiry_id' => customEncrypt($enqId)]));
        }
        ...
        if ($Response == 'UserCancelled')
        {
            ...
            //return redirect(config('constants.motorConstant.CAR_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($enqId)]));
        }
        ...
        if ($Response == 'Success' && $Premium !== 0)
        {
            ...
            if(isset($policy_response['status']) && $policy_response['policy_no'] != '' && $policy_response['policy_no'] != 'NULL')
            {
                ...
                {
                    $UserID = config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_USER_ID');
                    $Password = config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_PASSWORD');
                }else
                {
                    $UserID = config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_USER_ID_CORPORATE');
                    $Password = config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_PASSWORD_CORPORATE');
                }
                ...
                $get_response = getWsData(config('constants.IcConstants.future_generali.Pdf_LINK_FUTURE_GENERALI_MOTOR'), $pdf_array, 'future_generali', $additional_data);
                ...
                if ($data)
                {
                    ...
                    if (isset($pdf_data))
                    {
                        ...
                        $pdf_name = config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'future_generali/' . $proposal->user_product_journey_id . '.pdf';
    ...
    static public  function generatePolicyNumber($pg_response, $user_product_journey_id)
    {
        ...
        if($premium_type == 'comprehensive' && $requestData->business_type == "breakin")
        {
            $premium_type = 'comprehensive';
            $is_breakin = 'Y';
        }
        ...
        if ($Response == 'Success' && $Premium !== 0)
        {
             if ($requestData->is_renewal == 'Y' && $requestData->rollover_renewal != 'Y') {
                ...
                $url = config('constants.IcConstants.future_generali.FG_RENEWAL_CAR_CREATE_POLICY_DETAILS');
    ...
    static public function ReconService($request)
    {
        ...
        if($incomplete_transactions)
        {
            ...
            $get_response = getWsData(config('constants.IcConstants.future_generali.FUTURE_GENERALI_MOTOR_CHECK_TRN_STATUS'), $quote_array, 'future_generali', $additional_data);
    ...
    static public function payment_status_check($enquiry_id)
    {
        ...
        foreach ($get_payment_details as $value) {
            ...
            $get_response = getWsData(config('constants.IcConstants.future_generali.FUTURE_GENERALI_MOTOR_CHECK_TRN_STATUS'), $recorn_payment_status_check, 'future_generali', $additional_data);
