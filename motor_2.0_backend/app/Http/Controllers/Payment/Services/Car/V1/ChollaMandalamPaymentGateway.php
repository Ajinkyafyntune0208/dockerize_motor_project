...
;
        ...
        $return_data = [
            'form_action' => config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_GATEWAY_ID_CHOLLA_MANDALAM'),
            'form_method' => 'POST',
        ...
        PaymentRequestResponse::insert([
            ...
            'payment_url' => config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_GATEWAY_ID_CHOLLA_MANDALAM'),
    ...
    public static  function  create_checksum($enquiryId ,$request)
    {
        ...
        $new_pg_transaction_id = strtoupper(config('IC.CHOLLA_MANDALAM.V1.CAR.BROKER_NAME')). substr(strtoupper(md5(mt_rand())), 0, 7) . date('His', strtotime(now()));

        $str_arr = [
            config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_MERCHANT_ID_CHOLLA_MANDALAM'),
            $new_pg_transaction_id,
            'NA',
            (env('APP_ENV') == 'local') ? config('IC.CHOLLA_MANDALAM.V1.CAR.STATIC_PAYMENT_AMOUNT_CHOLLA_MANDALAM') :$data->final_payable_amount,
            'NA',
            'NA',
            'NA',
            'INR',
            'NA',
            'R',
            config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_SECURITY_ID_CHOLLA_MANDALAM'),
            'NA',
            'NA',
            'F',
            $data->proposal_no,
        ...
        $checksum = strtoupper(hash_hmac('sha256', $msg_desc, config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_CHECKSUM_CHOLLA_MANDALAM')));
 ...
 static public function check_payment_status($enquiry_id, $section)
    {
        ...
        $api_merchant_id = config('IC.CHOLLA_MANDALAM.V1.CAR.QUERY_API_MERCHANT_ID_CHOLLA_MANDALAM');
        $payment_checksum = config('IC.CHOLLA_MANDALAM.V1.CAR.PAYMENT_CHECKSUM_CHOLLA_MANDALAM');
        foreach ($transactiondata as $value) {
            ...
            $get_response = getWsData(
                Config('IC.CHOLLA_MANDALAM.V1.CAR.QUERY_API_URL_CHOLLA_MANDALAM'),
                $query_api_request,
                'cholla_mandalam',
                $additional_payment_data
            );
