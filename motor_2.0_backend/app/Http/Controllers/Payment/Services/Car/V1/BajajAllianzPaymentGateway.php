...
class BajajAllianzPaymentGateway extends Controller
{
    /**
     * @param $request
        ...
        if(((strtoupper($policy_no) != 'NULL') && (in_array($status, ['Y', 'success'])))) {
            ...
            PolicyDetails::updateOrCreate(
                ...
                [
                    ...
                    'ic_pdf_url' => ($productData->product_identifier == 'tp_only' && $bajaj_new_tp_url == 'Y') ? config('IC.BAJAJ_ALLIANZ.V1.CAR.END_POINT_URL_POLICY_PDF_DOWNLOAD_TP') 
                                : config('IC.BAJAJ_ALLIANZ.V1.CAR.END_POINT_URL_POLICY_PDF_DOWNLOAD'),
            ...
            $password = ($productData->product_identifier == 'tp_only' && $bajaj_new_tp_url == 'Y') ? config("IC.BAJAJ_ALLIANZ.V1.CAR.PASSWORD_TP") : config("IC.BAJAJ_ALLIANZ.V1.CAR.PASSWORD");
            ...
            if ($productData->product_identifier == 'tp_only' && $bajaj_new_tp_url == 'Y') {
                $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.CAR.END_POINT_URL_POLICY_PDF_DOWNLOAD_TP'), $issue_array, 'bajaj_allianz', $additional_data);
            } else {
                $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.CAR.END_POINT_URL_POLICY_PDF_DOWNLOAD'), $issue_array, 'bajaj_allianz', $additional_data);
            }
    ...
    public static function generatePdf($request) {
        ...
        if($user_proposal['policy_no'] == NULL) {
            ...
            $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.CAR.CHECK_PG_TRANS_STATUS'), $status_array, 'bajaj_allianz', $additional_data);
            $response = $get_response['response'];
            ...
            if ($status_data['pTransStatus'] == 'Y' && !empty($status_data['pPolicyRef'])) {
                ...
                PolicyDetails::updateOrCreate(
                    ...
                    [
                        'policy_number' => $policy_no,
                        'ic_pdf_url' => config('IC.BAJAJ_ALLIANZ.V1.CAR.END_POINT_URL_POLICY_PDF_DOWNLOAD'),
                        'idv' => $user_proposal['idv'] ,
                        'policy_start_date' => $user_proposal['policy_start_date'] ,
                        'ncb' => $user_proposal['ncb_discount'] ,
                        'premium' => $user_proposal['final_payable_amount'] ,
    ...
    {
        $userId = config("IC.BAJAJ_ALLIANZ.V1.CAR.USERNAME");

        $is_pos_enabled = config('constants.motorConstant.IS_POS_ENABLED');

        if(config('constants.posTesting.IS_POS_TESTING_MODE_ENABLE_BAJAJ') == 'Y') {
            $userId = config("IC.BAJAJ_ALLIANZ.V1.CAR.USERNAME_POS");
        }
    ...
    { 
        ...
        $additional_data = [
         ...
         $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.CAR.BREAKIN_POLICY_ISSUE_URL'), $input_array, 'bajaj_allianz', $additional_data);
         $response = $get_response['response'];
