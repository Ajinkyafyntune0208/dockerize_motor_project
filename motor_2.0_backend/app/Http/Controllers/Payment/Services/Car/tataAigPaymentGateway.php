...
use App\Http\Controllers\Payment\Services\Car\tataAigV2PaymentGateway as TATA_AIG_V2;
...
class tataAigPaymentGateway
{
    ...
    public static function confirm($request)
    {
            if(config('constants.IcConstants.tata_aig_v2.IS_TATA_AIG_V2_CAR_ENABLED') == 'Y')
            {
            return TATA_AIG_V2::confirm($request);
            }
            ...
            $ic_pdf_url = config('constants.IcConstants.tata_aig.END_POINT_POLICY_GENERATION_URL') . '?' . http_build_query(['polno' => $response['data']['policyno'], 'src' => 'app', 'key' => $response['data']['rnd_str']]);
            ...
            try{
                $proposal_pdf = Storage::put(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'tata_aig/' . md5($user_proposal->user_proposal_id) . '.pdf', file_get_contents($ic_pdf_url));
            }
            catch(Exception $e)
            {
                $enquiryId = $user_proposal->user_product_journey_id;
            ...
            PolicyDetails::updateOrCreate(
                ...
                [
                    'policy_number' => $response['data']['policyno'],
                    'pdf_url' => config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'tata_aig/' . md5($user_proposal->user_proposal_id) . '.pdf',
                    'ic_pdf_url' => $ic_pdf_url
                ]
            );
    ...
    static public function create_pdf($proposal)
    {
        ...
        $ic_pdf_url = config(
            'constants.IcConstants.tata_aig.END_POINT_POLICY_GENERATION_URL')
        . '?'
        . http_build_query(
            [
        ...
        $pdf_url = config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'tata_aig/'. md5($proposal->user_proposal_id).'.pdf';
        ...
        return [
            ...
            'data' => [
                'policy_number' => $additional_details['tata_aig']['policy_no'],
                'pdf_link'      =>  file_url(config('constants.motorConstant.CAR_PROPOSAL_PDF_URL') . 'tata_aig/'. md5($proposal->user_proposal_id).'.pdf')
            ]
        ];
    }
