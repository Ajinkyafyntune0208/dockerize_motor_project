...
class edelweissPaymentGateway
{
    ...
    public static  function  create_checksum($enquiryId, $request)
    {
        ...
        $bill_desk_request =
            [
                'MerchantID'        => config('constants.IcConstants.edelweiss.MOTOR_MERCHANT_ID'), //'EDGENBKAGR',
                ...
                'ItemCode'          => 'NA',
                'TypeField1'        => 'R',
                'UserID'            => config('constants.IcConstants.edelweiss.MOTOR_USER_ID'), //'EDGNMIBLPC-NA',
                'Filler4'           => 'NA',
                'Filler5'           => 'NA',
                'TypeField2'        => 'F',
        ...
        $checksum = hash_hmac('sha256', $bill_desk_msg, config('constants.IcConstants.edelweiss.MOTOR_CHECKSUM_KEY'), false);
        ...
        $payment_link = config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_PAYMENT_GATEWAY') . '?msg=' . $bill_desk_request;
    ...
    public static function confirm($request)
    {
        ...
        if ($request_data != null && isset($_REQUEST['msg'])) {
            if ($AuthStatus == '0300') {
                ...
                $get_response = getWsData(
                    config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_POLICY_GENERATION'),
                    $request_policy_gen,
                    'edelweiss',
                    [
                        'enquiryId' => $request_data['enquiry_id'],
                ...
                if (isset($response_policy_gen['issuePolicyObject']['issuepolicy']['policynrTt'])) {
                    ...
                    $payment_request_data = [
                        'itPay' => [
                            'itemList' => [
                                [
                                    'policyId'        => $policy_no,
                                    'bpId'            => config('constants.IcConstants.edelweiss.MOTOR_BPID'), //'1000012208',
                                    'paymentType'     => '39',
                    ...
                    $get_response = getWsData(
                        config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_PAYMENT_REQUEST'),
                        $payment_request_data,
                        'edelweiss',
                        [
                            'enquiryId' => $request_data['enquiry_id'],
                    ...
                    if ($payment_response_data) {
                        ...
                        $get_response = getWsData(
                            config('constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_PDF_SERVICE'),
                            $edelweiss_pdf_request,
                            'edelweiss',
                            [
                                'enquiryId' => $request_data['enquiry_id'],
                                ...
                                'headers' => [
                                    ...
                                    "x-api-key" => config('constants.IcConstants.edelweiss.EDELWEISS_X_API_KEY'),
                                ]
                            ]
                        );
    ...
    static public function generatePdf($request)
    {
        ...
        $password = config('constants.IcConstants.edelweiss.EDELWEISS_TOKEN_PASSWORD');
        if (empty($proposal->policy_no) || empty($policy_details->policy_number) ) {
    ...
    {
        if (config('constants.IcConstants.edelweiss.ENABLE_PAYMENT_LOT_CREATION') != "Y") {
            return false;
        }
        ...
        $instrumentNo   = explode('|', $paymentDetails->response);
        $instrumentNo = $instrumentNo[2] ?? null;

        if (empty($instrumentNo)) {
            return [
                'status' => false,
                'msg'    => 'Instrument number not found'
            ];
        }
