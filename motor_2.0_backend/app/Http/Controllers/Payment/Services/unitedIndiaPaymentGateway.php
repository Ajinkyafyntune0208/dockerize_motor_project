...
class unitedIndiaPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        if (config('constants.IcConstants.united_india.CV.PAYMENT_GATEWAY_TYPE') == 'paytm') {
            if (
                config('PAYMENT_GATEWAY.PAYTM.ENABLE') == 'Y' &&
                in_array('united_india', explode(',', config('PAYMENT_GATEWAY.PAYTM.IC_LIST')))
            ) {
                $paytmPaymentGateway = new PaymentGatewayController('united_india', 'cv', $enquiryId);
        ...
        $return_data = [
            'form_action'       => config('constants.IcConstants.united_india.CV.END_POINT_URL_PAYMENT_GATEWAY'),
            'form_method'       => 'POST',
    ...
    public static function paytmMakePayment($request, $proposal, $quote_log_id)
    {
        ...
        $paytmParams["body"] = array(
            "requestType" => "Payment",
            "mid" => config('constants.IcConstants.united_india.CV.PAYTM.MID'),
            "websiteName" => config('constants.IcConstants.united_india.CV.PAYTM.WEBSITENAME'),
            "orderId" => "ORDERID_" . Str::random(15),
            "callbackUrl" => route('cv.payment-confirm', [
                'united_india',
                'user_proposal_id'      => $proposal->user_proposal_id,
                'policy_id'             => $request->policyId
            ]),
        ...
        $checksum = PaytmChecksum::generateSignature(json_encode($paytmParams["body"], JSON_UNESCAPED_SLASHES), config('constants.IcConstants.united_india.CV.PAYTM.MERCHANT_KEY'));
        ...
        $get_response = getWsData(config('constants.IcConstants.united_india.CV.PAYTM.TOKENGENERATION_URL') . 'mid=' . $paytmParams['body']['mid'] . '&orderId=' . $paytmParams['body']['orderId'], $post_data, 'united_india', [
    ...
    public static  function  create_checksum($enquiryId, $request)
    {
        ...
        $new_pg_transaction_id = strtoupper(config('constants.IcConstants.united_india.CV.PAYMENT_MERCHANT_ID')) . date('Ymd') . time() . rand(10, 99);

        $str_arr = [
            config('constants.IcConstants.united_india.CV.PAYMENT_MERCHANT_ID'),
            ...
            'NA',
            'R',
            config('constants.IcConstants.united_india.CV.PAYMENT_SECURITY_ID'),
            'NA',
            'NA',
            'F',
            $data->proposal_no,
            ...
            'NA',
            route('cv.payment-confirm', ['united_india', 'enquiry_id' => $enquiryId, 'policy_id' => $request['policyId']]),

        ];

        $msg_desc = implode('|', $str_arr);
        $checksum = strtoupper(hash_hmac('sha256', $msg_desc, config('constants.IcConstants.united_india.CV.PAYMENT_CHECKSUM_KEY')));
    ...
    public static function confirm($request)
    {
        ...
        if (config('constants.IcConstants.united_india.CV.PAYMENT_GATEWAY_TYPE') == 'paytm') {
            if (
                ...
                $paytmPaymentGateway = new PaymentGatewayController('united_india', 'bike');
        ...
        if ($request_data != null && isset($_REQUEST['msg'])) {
            ...
            if ($response[14] == '0300') {
                ...
                if (!$payment_service['status']) {
                    return redirect(paymentSuccessFailureCallbackUrl($request_data['enquiry_id'], 'CV', 'SUCCESS'));
                }
    ...
    public static function paytmConfirmPayment($request)
    {
        ...
        $url = config('constants.IcConstants.united_india.CV.PAYTM.CHECK_STATUS_URL'); #https://securegw-stage.paytm.in/v3/order/status 
        ...
        if (isset($response_data['body']['resultInfo']['resultStatus']) && $response_data['body']['resultInfo']['resultStatus'] == 'TXN_SUCCESS') {
            ...
            updateJourneyStage($data);
            return redirect(
                config('constants.motorConstant.CV_PAYMENT_FAILURE_CALLBACK_URL')
                    . '?'
    ...
    public static function payment_info_service($proposal_data, $proposal)
    {
        ...
        $payent_info_array = [
            'HEADER' => [
                ...
                'TXT_BANK_CODE'             => '',
                'TXT_BANK_NAME'             => '',
                'TXT_MERCHANT_ID'           => config('constants.IcConstants.united_india.CV.PAYMENT_GATEWAY_TYPE') == 'paytm' ? config('constants.IcConstants.united_india.CV.PAYTM.MID') : config('constants.IcConstants.united_india.CV.PAYMENT_MERCHANT_ID'),
                'TXT_TRANSACTION_ID'        => $proposal_data['transaction_id'],
                'TXT_UTR_NUMBER'            => $proposal_data['utr_number'],
                'TXT_MERCHANT_KEY'          => config('constants.IcConstants.united_india.CV.PAYTM.MERCHANT_KEY'),
                'TXT_ORDER_ID'              => $proposal_data['pg_transaction_id']
            ]
        ];
        ...
        $get_response = getWsData(config('constants.IcConstants.united_india.CV.END_POINT_URL_SERVICE'), $payent_info_array, 'united_india', $additional_data);
    ...
    static public function generatePdf($request)
    {
        ...
        if (config('constants.IcConstants.united_india.CV.PAYMENT_GATEWAY_TYPE') == 'paytm') {
            if (
                ...
                $paytmPaymentGateway = new PaymentGatewayController('united_india', 'cv', $user_product_journey_id);
    ...
    static public function generatePolicy($proposal, $requestArray)
    {
        ...
        $transaction_id = json_decode($payment_details['response'] , true);
        ...
        $proposal_data = [
            ...
            'utr_number'                => $transaction_id['transactionid'],
        ];
    ...
    static public function create_pdf($proposal, $policy_data)
    {
        ...
        $pdf_url = config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'united_india/' . md5($proposal->user_proposal_id) . '.pdf';
    ...
    public static function check_payment_status($enquiry_id)
    {
        ...
        foreach ($get_payment_details as $value) {
            ...
            if (
                ...
                $merchantKey = config('constants.IcConstants.united_india.CV.PAYTM.MERCHANT_KEY');
            }
