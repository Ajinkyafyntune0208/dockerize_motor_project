        ...
        if ($proposal) {
            ...
            if ($token) {
                ...
                if (isset($token_data['access_token'])) {
                    ...
                    if ($parent_code == 'PCV') {
                        $proposal_status_url = config('constants.IcConstants.magma.END_POINT_URL_MAGMA_PCV_PROPOSAL_STATUS');
                    } else {
                        $proposal_status_url = config('constants.IcConstants.magma.END_POINT_URL_MAGMA_GCV_PROPOSAL_STATUS');
                    }

                    $get_response = getWsData($proposal_status_url, $proposal_status_requset, 'magma', [
                        'section'          => $productData->product_sub_type_code,
                    ...
                    if ($proposal_status) {
                        ...
                        if ($proposal_status['ServiceResult'] == 'Success' && $proposal_status['OutputResult']['PaymentStatus'] == 'Pending For Payment') { */
                            $payment_request = [
                                ...
                                'SuccessURL' => route('cv.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
                                'FailureURL' => route('cv.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
                            ...
                            if ($parent_code == 'PCV') {
                                $payment_gateway_url = config('constants.IcConstants.magma.END_POINT_URL_MAGMA_PCV_GETPAYMENTURL');
                            } else {
                                $payment_gateway_url = config('constants.IcConstants.magma.END_POINT_URL_MAGMA_GCV_GETPAYMENTURL');
                            }

                            $get_response = getWsData($payment_gateway_url, $payment_request, 'magma', [
                                'section'          => $productData->product_sub_type_code,
                            ...
                            if ($payment_data) {
                                ...
                                if ($payment_result['ServiceResult'] == 'Success') {
                                    ...
                                    DB::table('payment_request_response')->insert([
                                        ...
                                        'payment_url'             => $pg_url,
                                        'return_url'              => route('cv.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
            ...
            if (isset($request['PolicyNo']) && ($request['PolicyNo'] != '' || $request['PolicyNo'] != NULL)) {
                ...
                if ($token) {
                    ...
                    if (isset($token_data['access_token'])) {
                        ...
                        if ($parent_code == 'PCV') {
                            $pdf_service_url = config('constants.IcConstants.magma.END_POINT_URL_POLICY_PDF_MAGMA_PCV');
                        } else {
                            $pdf_service_url = config('constants.IcConstants.magma.END_POINT_URL_POLICY_PDF_MAGMA_GCV');
                        }

                        $get_response = getWsData($pdf_service_url, $pdf_request_data, 'magma', [
                            'section'          => $productData->product_sub_type_code,
                            'method'           => 'Policy PDF',
                            'requestMethod'    => 'post',
                        ...
                        if ($pdf_response) {
                            ...
                            if ($pdf_response['ServiceResult'] == 'Success') {
                                Storage::put(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'magma/' . md5($user_proposal->user_proposal_id) . '.pdf', base64_decode($pdf_response['OutputResult']['PolicyBase64String']));
                                ...
                                PolicyDetails::updateOrCreate(
                                    ['proposal_id' => $user_proposal->user_proposal_id], [
                                        ...
                                        'pdf_url'       => config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'magma/' . md5($user_proposal->user_proposal_id) . '.pdf',
                                        'status'        => 'SUCCESS'
                                    ]
                                );
                            }
                ...
                if ($token) {
                    ...
                    if (isset($token_data['access_token'])) {
                        ...
                        if ($pdf_response) {
                            ...
                            if ($pdf_response['ServiceResult'] == 'Success') {
                                Storage::put(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'magma/' . md5($policy_details->user_proposal_id) . '.pdf', base64_decode($pdf_response['OutputResult']['PolicyBase64String']));
                                ...
                                PolicyDetails::updateOrCreate(
                                    ['proposal_id' => $policy_details->user_proposal_id], [
                                        'policy_number' => $pdf_response['OutputResult']['PolicyNumber'],
                                        'pdf_url'       => config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'magma/' . md5($policy_details->user_proposal_id) . '.pdf',
                                        'status'        => 'SUCCESS'
                                    ]
                                );

                                $pdf_url = file_url(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'magma/'. md5($policy_details->user_proposal_id). '.pdf');
