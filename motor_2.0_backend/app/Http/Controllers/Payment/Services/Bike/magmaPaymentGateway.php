        ...
        if ($proposal)
        {
            ...
            if ($token)
            {
                ...
                if (isset($token_data['access_token']))
                {
                    ...
                    $get_response = getWsData(config('constants.IcConstants.magma.END_POINT_URL_MAGMA_BIKE_PROPOSAL_STATUS'), $proposal_status_requset, 'magma', [
                        'section'          => $productData->product_sub_type_code,
                    ...
                    if ($proposal_status)
                    {
                        ...
                        { */
                            $get_payment_url = [
                                ...
                                'SuccessURL' => route('bike.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
                                'FailureURL' => route('bike.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
                            ...
                            $get_response = getWsData(config('constants.IcConstants.magma.END_POINT_URL_MAGMA_BIKE_GETPAYMENTURL'), $get_payment_url, 'magma', [
                                'section'          => $productData->product_sub_type_code,
                            ...
                            if ($payment_data)
                            {
                                ...
                                if ($payment_result['ServiceResult'] == 'Success')
                                {
                                    ...
                                    DB::table('payment_request_response')->insert([
                                        ...
                                        'payment_url'             => $pg_url,
                                        'return_url'              => route('bike.payment-confirm', ['magma', 'user_proposal_id' => $proposal->user_proposal_id]),
            ...
            if (isset($request['PolicyNo']) && ($request['PolicyNo'] != '' || $request['PolicyNo'] != NULL))
            {
                ...
                if ($token)
                {
                    ...
                    if (isset($token_data['access_token']))
                    {
                        ...
                        $get_response = getWsData(config('constants.IcConstants.magma.END_POINT_URL_POLICY_PDF_MAGMA_BIKE'), $pdf_request_data, 'magma', [
                            'section'          => $productData->product_sub_type_code,
                            'method'           => 'Policy PDF',
                            'requestMethod'    => 'post',
                        ...
                        if ($pdf_response)
                        {
                            ...
                            {
                                Storage::put(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'magma/' . md5($user_proposal->user_proposal_id) . '.pdf', base64_decode($pdf_response['OutputResult']['PolicyBase64String']));
                                ...
                                PolicyDetails::updateOrCreate(
                                    ['proposal_id' => $user_proposal->user_proposal_id], [
                                        ...
                                        'pdf_url'       => config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'magma/' . md5($user_proposal->user_proposal_id) . '.pdf',
                                        'status'        => 'SUCCESS'
                                    ]
                                );
                            }
                ...
                if ($token)
                {
                    ...
                    if (isset($token_data['access_token']))
                    {
                        ...
                        if ($pdf_response)
                        {
                            ...
                            if ($pdf_response['ServiceResult'] == 'Success')
                            {
                                Storage::put(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'magma/' . md5($policy_details->user_proposal_id) . '.pdf', base64_decode($pdf_response['OutputResult']['PolicyBase64String']));
                                ...
                                PolicyDetails::updateOrCreate(
                                    ['proposal_id' => $policy_details->user_proposal_id], [
                                        'policy_number' => $pdf_response['OutputResult']['PolicyNumber'],
                                        'pdf_url'       => config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'magma/' . md5($policy_details->user_proposal_id) . '.pdf',
                                        'status'        => 'SUCCESS'
                                    ]
                                );

                                $pdf_url = file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'magma/'. md5($policy_details->user_proposal_id). '.pdf');
