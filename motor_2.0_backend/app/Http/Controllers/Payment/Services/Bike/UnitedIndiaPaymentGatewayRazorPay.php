...
class UnitedIndiaPaymentGatewayRazorPay
{
    ...
    public static function make($request)
    {
        ...
        $get_order_id = PaymentRequestResponse::where('user_product_journey_id', $enquiryId)
            ->select('order_id')
            ->get();
        ...
        if (!empty($get_order_id)) {
            ...
            foreach ($get_order_id as $value) {
                ...
                $request_params = [
                    ...
                    'enquiryId' => $enquiryId,
                    'Razorpay_key_id' => config('constants.IcConstants.UNITED_INDIA_BIKE_ROZER_PAY_KEY_ID'),
                    'Razorpay_secret_id' => config('constants.IcConstants.UNITED_INDIA_BIKE_ROZER_PAY_SECREAT_KEY'),
                ];
        ...
        if ((isset($decoded_data['status']) && $decoded_data['status'])) {     #successful part
            ...
            try {
                ...
                $return_data = [
                    ...
                    'clientKey' => config('constants.IcConstants.UNITED_INDIA_BIKE_ROZER_PAY_KEY_ID'),
                    'orderId' => $order_id,
                    "amount" => $create_order_response['amount_due'],
                    'returnUrl' =>  route('bike.payment-confirm', ['united_india'])
                ];
    ...
    public static function confirm($request)
    {
        ...
        if ($decoded_data['status'] && !empty($decoded_data['data'])) {                        #checking tranasaction stage
            ...
            if ($decoded_data['data']['error_code'] == null && $decoded_data['data']['status'] == 'captured' && $decoded_data['data']['captured'] == true) {
                ...
                return response()->json([
                    'status' => true,
                    'redirectUrl' => paymentSuccessFailureCallbackUrl($enquiry_id,'BIKE','SUCCESS'),
                ]);
            } else {
                ...
                return response()->json([
                    'status' => false,
                    'redirectUrl' => paymentSuccessFailureCallbackUrl($enquiry_id,'BIKE','FAILURE'),
                ]);
            }
        }
        return response()->json([
            ...
            if (is_null($policy_details->policy_number) || $policy_details->policy_number == '') {
                ...
                if (!empty($get_order_id)) {
                    ...
                    foreach ($get_order_id as $value) {
                        ...
                        $store_data_encrypt = [
                            'enquiry_id'        => $user_product_journey_id,
                            'product'           => 'Private Bike Insurance',
                            'section'           => 'BIKE',
                            'method_name'       => 'Fetch Payment Status',
                            'company'           => 'united_india',
                            'method'            => 'post',
