...
class newIndiaPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        PaymentRequestResponse::insert([
            ...
            'return_url'                => route('bike.payment-confirm', ['new_india', 'enquiry_id' => $enquiryId]),
    ...
    { 
        ...
        $additional = SelectedAddons::where('user_product_journey_id', $requestData->user_product_journey_id)
                                ->select('addons', 'accessories','compulsory_personal_accident','additional_covers', 'voluntary_insurer_discounts', 'discounts')
                                ->first();
        ...
        if (!empty($additional['additional_covers']) && $productData->zero_dep != 0) {
        ...
        $reg = explode("-",$proposal->vehicale_registration_number);
         ...
         }else
         {
            ...
            {
                $policy_start_date = date('d/m/Y', strtotime(date('Y-m-d', strtotime('+1 day', strtotime($requestData->previous_policy_expiry_date == 'New' ? date('Y-m-d')  : $requestData->previous_policy_expiry_date)))));
            }
        ...
        else
        {
            $policy_holder_array = [
                ...
                if (!empty($additional['accessories'])) {
                    ...
                    $properties = [
                        ...
                        [
                            'typ:value' => $type_of_fuel,#$type_of_fuel == "Cng" ? 'Y' : 'N',
                        ...
                        [
                            'typ:value' => ($requestData->business_type == 'newbusiness' ? '' : strtr($existing_policy_expiry_date, ['-' => '/'])),
                    ...
                    if($requestData->business_type == 'newbusiness'){
                        $proposal_array['typ:term'] = '5';
                        $proposal_array['typ:polExpirydate'] = date('d/m/Y', strtotime(date('Y-m-d', strtotime('+5 year -1 day', strtotime(strtr($policy_start_date, ['/' => '-'])))))); 
                    }
                    $proposal_array = trim_array($proposal_array);
                    $get_response = getWsData(config('constants.IcConstants.new_india.END_POINT_URL_NEW_INDIA_BIKE'),$proposal_array, 'new_india',
                    [
            ...
            if ($data)
            {
                ...
                {
                if ($SaveQuote_ApproveProposalResponseElement['PRetCode'] != '1')
                {
                    return [
                            'status'  => 'false',
                            'message' => isset($SaveQuote_ApproveProposalResponseElement['PRetErr']) ? $SaveQuote_ApproveProposalResponseElement['PRetErr'] : '',
                        ];
                }
                else
                {
                    ...
                    $pg_transaction_no = generate_random_number(floor((rand(0, 99999)) + 1), config('constants.IcConstants.NEW_INDIA.BROKER_NAME'), 5);
                    UserProposal::where('user_proposal_id' , $proposal->user_proposal_id)
                                            ->update([
                                        'unique_proposal_id' => $quoteNo,
                                        'final_payable_amount' => $netPremium,
                                        'proposal_no' => $quoteNo,
                    ...
                    if ($netPremium != 0)
                    {
                           ...
                           $str_arr = [
                                ...
                                'NA',
                                'NA',
                                route('bike.payment-confirm', ['new_india']),
                           ];
    ...
    { 
        ...
        if (((end($response_array)) == $final_checksum) && ($response_array[14] == '0300')) 
        {
            ...
            $get_response = getWsData(config('constants.IcConstants.new_india.END_POINT_URL_NEW_INDIA_BIKE'),$request, 'new_india',
            [
            ...
            if($policy_no != '' && $policy_no != NULL && !empty($policy_no))
            {
                ...
                $get_response = getWsData(config('constants.IcConstants.new_india.END_POINT_URL_NEW_INDIA_BIKE'),$document_array, 'new_india',
                [
                ...
                if(isset($fetch_array_response['Body']['fetchDocumentNameResponseElement']['docs'][0]) && !empty($proposal->policy_no))
                {
                    ...
                    if(!empty($document_id))
                    {
                        ...
                        $data = httpRequestNormal($doc, 'GET', [], [], [], [], false)['response'];
                    ...
                    if ($data != false && $policy_no !='') 
                    {
                       Storage::put(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'new_india/' . $proposal->user_proposal_id . '.pdf', $data);
                       PolicyDetails::where('proposal_id', $proposal->user_proposal_id)
                            ->update([
                                'pdf_url' => config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'new_india/' . $proposal->user_proposal_id. '.pdf',
                                'ic_pdf_url' => $doc
                            ]);
            ...
            else
            {
                return [
                    'status'  => 'false',
                    'message' => 'Issue in policy number generation',
                ];
                return redirect(paymentSuccessFailureCallbackUrl($proposal->user_product_journey_id,'BIKE','SUCCESS'));
    ...
    public static function generatePdf($request)
    {
        ...
        if($policy_details->pdf_url == '')
        {
            ...
            if($proposal->policy_no =='' || $proposal->policy_no =='NULL')
            {
                $pdf_response_data = [
                'status' => false,
                ...
                if ($data != false) 
                {
                   ...
                   $pdf_response_data = [
                            ...
                            'data' => [
                                ...
                                'pdf_link'      => file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'new_india/' . $proposal->user_proposal_id . '.pdf')
                            ]
                        ];
