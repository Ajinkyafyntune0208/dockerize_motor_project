...
class unitedIndiaPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        if(config('constants.IcConstants.united_india.car.PAYMENT_GATEWAY_TYPE') == 'paytm') {
            if (
                ...
                $paytmPaymentGateway = new PaymentGatewayController('united_india', 'bike', $enquiryId);
        ...
        $return_data = [
            'form_action'       => config('constants.IcConstants.united_india.bike.BIKE_END_POINT_URL_PAYMENT_GATEWAY'),
            'form_method'       => 'POST',
        ...
        DB::table('payment_request_response')->insert([
            ...
            'payment_url'               => config('constants.IcConstants.united_india.bike.BIKE_END_POINT_URL_PAYMENT_GATEWAY'),
            'return_url'                => route(
    ...
    public static  function  create_checksum($enquiryId ,$request)
    {
        ...
        $new_pg_transaction_id = strtoupper(config('constants.IcConstants.united_india.bike.BIKE_PAYMENT_MERCHANT_ID')).date('Ymd').time().rand(10,99);

        $str_arr = [
            config('constants.IcConstants.united_india.bike.BIKE_PAYMENT_MERCHANT_ID'),
            ...
            'NA',
            'R',
            config('constants.IcConstants.united_india.bike.BIKE_PAYMENT_SECURITY_ID'),
            'NA',
            'NA',
            'F',
            $data->proposal_no,
            ...
            'NA',
            route('bike.payment-confirm', ['united_india','enquiry_id' => $enquiryId,'policy_id' => $request['policyId']]),

        ];

        $msg_desc = implode('|', $str_arr);
        $checksum = strtoupper(hash_hmac('sha256', $msg_desc, config('constants.IcConstants.united_india.bike.BIKE_PAYMENT_CHECKSUM_KEY')));
    ...
    public static function confirm($request)
    {
        ...
        if($request_data!=null && isset($_REQUEST['msg'])) {
            ...
            if ($response[14] == '0300')
            {
                ...
                if(!$payment_service['status']){
                    /* return redirect(
                        config('constants.motorConstant.BIKE_PAYMENT_SUCCESS_CALLBACK_URL')
                        . '?'
                        . http_build_query(['enquiry_id' => customEncrypt($request_data['enquiry_id']) ])
                    ); */
                    return redirect(paymentSuccessFailureCallbackUrl($request_data['enquiry_id'],'BIKE','SUCCESS'));
                }
                if (!empty($payment_service['pdf_schedule'])) {
            ...
            else
            {
                ...
                updateJourneyStage($data);
                /* return redirect(
                    config('constants.motorConstant.BIKE_PAYMENT_FAILURE_CALLBACK_URL')
                    . '?'
    ...
    public static function payment_info_service($proposal_data, $proposal){
        ...
        $payent_info_array = [
            'HEADER' => [
                ...
                'TXT_BANK_CODE'             => '',
                'TXT_BANK_NAME'             => '',
                'TXT_MERCHANT_ID'           => config('constants.IcConstants.united_india.car.PAYMENT_GATEWAY_TYPE') == 'paytm' ? config('constants.IcConstants.united_india.bike.PAYTM.MID') :config('constants.IcConstants.united_india.bike.BIKE_PAYMENT_MERCHANT_ID'),
        ...
        $get_response = getWsData(config('constants.IcConstants.united_india.bike.BIKE_END_POINT_URL_SERVICE'), $payent_info_array, 'united_india', $additional_data);
        ...
        if ($response) {
            ...
            $error_message = isset($header['TXT_ERR_MSG']) ? $header['TXT_ERR_MSG'] : [];
    ...
    static public function generatePdf($request)
    {
        ...
        if(config('constants.IcConstants.united_india.car.PAYMENT_GATEWAY_TYPE') == 'paytm') {
            if (
                ...
                $paytmPaymentGateway = new PaymentGatewayController('united_india', 'bike', $user_product_journey_id);
        ...
        else
        {
            $pdf_response_data = [
                ...
                'data'   => [
                    ...
                    'pdf_link'      => file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'united_india/'. md5($proposal->user_proposal_id). '.pdf'),
                    'policy_number' => $policy_details->policy_number
                ]
            ];
        }
    ...
    static public function create_pdf($proposal, $policy_data){
        ...
        $pdf_url = config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'united_india/'. md5($proposal->user_proposal_id). '.pdf';
