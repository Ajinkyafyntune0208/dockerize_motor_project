...
class ReliancePaymentGateway
{
    public static function make($request) {
        ...
        $params = [
            ...
            'PaymentType' => '1',
            'Responseurl' => route('bike.payment-confirm', ['reliance']),
        ];
        ...
        $params['subscription-key'] = config('IC.RELIANCE.V1.BIKE.OCP_APIM_SUBSCRIPTION_KEY');

        $payment_url = config('IC.RELIANCE.V1.BIKE.PAYMENT_GATEWAY_LINK'). '?' . http_build_query($params);
        ...
        DB::table('payment_request_response')->insert([
            ...
            'return_url' => route('bike.payment-confirm', ['reliance']),
    ...
    public static function confirm($request) {
        ...
        if (!empty($policy_no) && $bool_status == 1) {
    ...
    static public function generatePdf($request)
    {
        ...
        foreach ($policy_details as $policy_detail) {
            ...
            {
                if(!Storage::exists(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf'))
                {
                ...
                if(!empty($policy_detail->pdf_url))
                {
                    return response()->json([
                        ...
                        'data' => [
                            ...
                            'pdf_link' => file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf'),//$policy_detail->pdf_url,
    ...
    public static function proposalStatusService($enquiryId, $policy_detail)
    {
        ...
        $get_response = getWsData(
            config('IC.RELIANCE.V1.BIKE.END_POINT_URL_PROPOSAL_STATUS'),
            $proposalStatusRequest,
            'reliance',
            [
    ...
    {            
        ...
        if (config('IC.RELIANCE.V1.BIKE.POLICY_DWLD_LINK_NEW_API_ENABLE') == 'Y') {
            $ic_pdf_url =  config('IC.RELIANCE.V1.BIKE.POLICY_DWLD_LINK_NEW');
            $pdf_Request = [
                'PolicyNumber' => $policy_number,
                'SourceSystemID' => config('IC.RELIANCE.V1.BIKE.SOURCE_SYSTEM_ID'),
                'SecureAuthToken' => config('IC.RELIANCE.V1.BIKE.SECURE_AUTH_TOKEN'),
                'EndorsementNo' => '',
            ];
            ...
            $ic_pdf_url =  config('IC.RELIANCE.V1.BIKE.POLICY_DWLD_LINK') . '?PolicyNo=' . $policy_number . '&ProductCode=' . $policy_detail->product_code;
        }
        ...
        if (config('constants.motorConstant.RELIANCE_GENERATE_POLICY_PDF') == 'Y') {
            try{
                ...
                if (config('IC.RELIANCE.V1.BIKE.POLICY_DWLD_LINK_NEW_API_ENABLE') == 'Y') {
                    ...
                    if (!empty($pdf_data['DownloadLink'])) {
                        $pdf_data = httpRequestNormal($pdf_data['DownloadLink'], 'GET', [], [], [], [], false);
                    ...
                    if (isset($pdf_data['status']) && $pdf_data['status'] == 200) {
                        Storage::put(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'reliance/' . md5($policy_detail->user_proposal_id) . '.pdf', $pdf_data['response']);
                    }
                }  
        ...
        PolicyDetails::updateOrCreate(
            ...
            [
                'policy_number' => $policy_number,
                'ic_pdf_url' => $ic_pdf_url,
                'pdf_url' => config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf',
                'status' => 'SUCCESS'
            ]
        );

        $pdf_url = file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf');
