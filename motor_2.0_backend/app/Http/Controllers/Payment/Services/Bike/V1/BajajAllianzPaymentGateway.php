...
class BajajAllianzPaymentGateway extends Controller
{
        ...
        if(((strtoupper($policy_no) != 'NULL') && (in_array($status, ['Y', 'success'])))) { 
            ...
            PolicyDetails::updateOrCreate(
                ...
                [
                    ...
                    'ic_pdf_url' => ($productData->premium_type_code == 'third_party' && $bajaj_new_tp_url == 'Y') ? config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_POLICY_PDF_DOWNLOAD_TP') 
                    : config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_POLICY_PDF_DOWNLOAD'),
            ...
            if ($productData->premium_type_code == 'third_party' && $bajaj_new_tp_url == 'Y') {
                $password = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PASSWORD_TP");
            } else {
                $password = config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_PASS");
            }
            ...
            if ($productData->premium_type_code == 'third_party' && $bajaj_new_tp_url == 'Y') {
                $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_POLICY_PDF_DOWNLOAD_TP'), $issue_array, 'bajaj_allianz', $additional_data);
            } else {
                $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_POLICY_PDF_DOWNLOAD'), $issue_array, 'bajaj_allianz', $additional_data);
            }
    ...
    public static function generatePdf($request) {
        ...
        if($user_proposal['policy_no'] == NULL) {
            ...
            foreach ($order_id as $proposal_no) {
                ...
                $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.CHECK_PG_TRANS_STATUS'), $status_array, 'bajaj_allianz', $additional_data);
            }
            $response = $get_response['response'];
            ...
            if ($status_data['pTransStatus'] == 'Y' && !empty($status_data['pPolicyRef'])) {
                ...
                PolicyDetails::updateOrCreate(
                    ...
                    [
                        'policy_number' => $policy_no,
                        'ic_pdf_url' => config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_POLICY_PDF_DOWNLOAD'),
                        'idv' => $user_proposal['idv'] ,
                        'policy_start_date' => $user_proposal['policy_start_date'] ,
                        'ncb' => $user_proposal['ncb_discount'] ,
                        'premium' => $user_proposal['final_payable_amount'] ,
    ...
    {
        $userId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_NAME");

        $is_pos_enabled = config('constants.motorConstant.IS_POS_ENABLED');

        if(config('constants.posTesting.IS_POS_TESTING_MODE_ENABLE_BAJAJ') == 'Y') {
            $userId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_NAME_POS");
        }
