    ...
    public static function confirm($request)
    {
        ...
        if(($request->Response ?? '') == 'Success' && !empty($enqId))
        {
            ...
            try{
                ...
                //$return_url = config('constants.motorConstant.BIKE_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($enqId)]);
            }catch(\Exception $e)
            {
        ...
        if((!isset($request->TID) || empty($request->TID)))
        {
            ...
            //return redirect(config('constants.motorConstant.BIKE_PAYMENT_SUCCESS_CALLBACK_URL'). '?' . http_build_query(['enquiry_id' => customEncrypt($enqId)]));
        }
        ...
        if ($Response == 'UserCancelled') 
        {
            ...
            //return redirect(config('constants.motorConstant.BIKE_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($enqId)]));
        }
        ...
        if ($Response == 'Success' && $Premium !== 0) 
        {
            ...
            if($policy_response['status'] == 'true' && $policy_response['policy_no'] != '' && $policy_response['policy_no'] != 'NULL')
            {
                ...
                $pdf_array = [
                                ...
                                'tem:UserID'       => config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_USER_ID'), //'webagg02',
                                'tem:Password'     => config('constants.IcConstants.future_generali.FUTURE_GENERALI_PDF_PASSWORD') //'webagg02@123',
                            ];
                $additional_data = 
                [
                'requestMethod' => 'post',
                ...
                if ($data) 
                {
                    ...
                    if (isset($pdf_data)) 
                    {
                        ...
                        $pdf_name = config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'future_generali/' . $proposal->user_product_journey_id . '.pdf';
    ...
    static public  function generatePolicyNumber($pg_response, $user_product_journey_id)
    {
        ...
        if ($Response == 'Success' && $Premium !== 0) 
        {
            if ($requestData->is_renewal == 'Y' && $requestData->rollover_renewal != 'Y' && app('env') == 'local') {
                ...
                $url = config('constants.IcConstants.future_generali.FG_RENEWAL_BIKE_CREATE_POLICY_DETAILS');
    ...
    static public function generatePdf($request)
    {
        ...
        else 
        {
            $pdf_response_data = [
                ...
                'data'   => [
                    'policy_number' => $policy_details->policy_number,
                    'pdf_link' => file_url(config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'future_generali/'. $proposal->user_product_journey_id . '.pdf'),
                ]
            ];
        }
