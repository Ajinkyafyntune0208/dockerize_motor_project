...
use App\Http\Controllers\Proposal\Services\Bike\V2\tataAigSubmitProposals as TATA_AIG;
...
class tataAigPaymentGateway
{
    ...
    public static function generatePaymentService($enquiryId, $proposal, $masterProduct)
    {
        ...
        $paymentRequstUrl = config('IC.TATA_AIG.V2.BIKE.END_POINT_URL_PAYMENT') .'?'.
        http_build_query([
            'product' => 'motor'
        ]);
...
//            );
    ...
    public static function verifyPaymentService($enquiryId, $proposal, $productData, $token_response)
    {
        ...
        foreach ($order_id_data as $payment_id) {
            ...
            $verifyPaymentRequstUrl = config('IC.TATA_AIG.V2.BIKE.END_POINT_URL_PAYMENT_VERIFY') .'?'.
            http_build_query([
                'product' => 'motor'
            ]);
    ...
    static public function policyDownloadService($enquiryId, $proposal, $productData, $verifyPaymentServiceResponse, $token_response)
    {
        ...
        $ic_pdf_url = config(
            'IC.TATA_AIG.V2.BIKE.END_POINT_URL_POLICY_DOWNLOAD')
        . $verifyPaymentServiceResponse['data']['encrypted_policy_id'];

        $pdf_url = config('constants.motorConstant.BIKE_PROPOSAL_PDF_URL') . 'tata_aig_v2/'. md5($proposal->user_proposal_id).'.pdf';
