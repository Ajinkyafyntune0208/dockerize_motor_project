...
class UnitedIndiaPaymentGatewayBillDesk
{
    ...
    public static function make($request)
    {
        ...
        $url = config('IC.BIKE.BILL_DESK_API.V2.CREATE_ORDER_URL');

        $order_id = config('constants.motorConstant.SMS_FOLDER') . time() . rand(111, 9999);

        $sharedSecretKey = config('IC.BIKE.BILL_DESK_API.V2.SHARED_SECRET_KEY');

        $merchant_id = config('IC.BIKE.BILL_DESK_API.V2.MERCHANT_ID');

        $return_url = route('bike.payment-confirm', ['united_india']);
        ...
        if (!empty($get_order_id)) {
            ...
            foreach ($get_order_id as $value) {
                ...
                $status_url = config('IC.UNITED_INIDA.BIKE.PAYMENT.CHECK_BILL_DESK_API.V2_STATUS_URL');
    ...
    public static function confirm($request)
    {
        if (isset($request->transaction_response) && !empty($request->transaction_response)) {
            ...
            if ($decoded['auth_status'] == "0300" || $decoded['auth_status'] == "0002") {
                ...
                updateJourneyStage($data);
                return redirect(paymentSuccessFailureCallbackUrl($enquiry_id, 'BIKE', 'FAILURE'));
            }
            return redirect(paymentSuccessFailureCallbackUrl($enquiry_id, 'BIKE', 'FAILURE'));
        }
