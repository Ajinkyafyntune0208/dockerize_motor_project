...
use App\Http\Controllers\Proposal\Services\tataAigV2SubmitProposal as TATA_AIG;
...
class tataAigV2PaymentGateway
{
    ...
    public static function generatePaymentService($enquiryId, $proposal, $masterProduct)
    {
        ...
        if (in_array($productData->premium_type_code, ["short_term_3","short_term_6","short_term_3_breakin"])) {
        ...
        $paymentRequstUrl = config('constants.IcConstants.tata_aig_v2.TATA_AIG_V2_END_POINT_URL_PAYMENT') .'?'.
        http_build_query([
            'product' => 'motor'
        ]);
...
//constants.motorConstant.Cv_PAYMENT_FAILURE_CALLBACK_URL

        if(empty($request->proposal_no))
        {
        ...
        if(($verifyPaymentServiceResponse['data']['payment_status'] ?? '') != 'Success')
        {
            DB::table('payment_request_response')
            ->where('user_product_journey_id', $proposal->user_product_journey_id)
            ->where('user_proposal_id', $proposal->user_proposal_id)
...
//                . '?'
//                . http_build_query(['enquiry_id' => customEncrypt($enquiryId) ])
//            );
        }
...
//        );
    ...
    static public function policyDownloadService($enquiryId, $proposal, $productData, $verifyPaymentServiceResponse, $token_response)
    {
        ...
        // echo "<pre>";print_r([$verifyPaymentServiceResponse]);echo "</pre>";die();

        $ic_pdf_url = config(
            'constants.IcConstants.tata_aig_v2.TATA_AIG_V2_END_POINT_URL_POLICY_DOWNLOAD')
        . $verifyPaymentServiceResponse['data']['encrypted_policy_id'];

        $pdf_url = config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'tata_aig_v2/'. md5($proposal->user_proposal_id).'.pdf';
    ...
    static public function generatePdf($request)
    {
        ...
        $proposal_additional_details_data = json_decode($proposal->additional_details_data);
        if (empty($proposal->additional_details_data) || empty($proposal_additional_details_data->tata_aig_v2)) {
            $pdf_response_data = [
                'status' => false,
                'msg'    => empty($proposal->additional_details_data) ? 'additional_details_data is empty' : 'tata_aig_v2 is empty'
            ];
            return response()->json($pdf_response_data);
        }
