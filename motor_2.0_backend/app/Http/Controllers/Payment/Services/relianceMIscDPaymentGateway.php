...
class relianceMIscDPaymentGateway
{
    /**
     * @param $request
    ...
    public static function make($request)
    {
        ...
        $premium_req_data = RelianceMiscdSubmitProposal::getPremiumRequestData([
            'enquiryId' => $enquiryId,
            ...
            if (!empty($policy_no)) {
                ...
                if (config('constants.motorConstant.RELIANCE_GENERATE_POLICY_PDF') == 'Y') {
                    ...
                    if (config('constants.IcConstants.reliance.POLICY_DWLD_LINK_RELIANCE_NEW_API_ENABLE') == 'Y') {
                        ...
                        if (!empty($pdf_data['DownloadLink'])) {
                            Storage::put(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'reliance/' . md5($user_proposal->user_proposal_id) . '.pdf', file_get_contents($pdf_data['DownloadLink']));
                        }
                    } else {
                    ...
                    // PolicyDetails::where('proposal_id', $user_proposal->user_proposal_id)
                    //     ->update([
                    //         'pdf_url' => config('constants.motorConstant.MISC_PROPOSAL_PDF_URL') . 'reliance/' .  md5($user_proposal->user_proposal_id) . '.pdf'
                    //     ]);
                    // if (isset($pdf_data['status']) && $pdf_data['status'] == 200) {
                    //     Storage::put(config('constants.motorConstant.MISC_PROPOSAL_PDF_URL') . 'reliance/' . md5($user_proposal->user_proposal_id) . '.pdf', $pdf_data['response']);
                    // }
                    // sleep(2); // DELAY OF 2 SECONDS POLICY WAS NOT GENERATING
                    // updateJourneyStage([
                    //     'user_product_journey_id' => $user_proposal->user_product_journey_id,
                ...
                if(!empty($policy_detail->pdf_url))
                {
                    ...
                    return response()->json([
                        ...
                        'data' => [
                            ...
                            'pdf_link' => Storage::url(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf'),//$policy_detail->pdf_url,
    ...
    {            
        ...
        $pdf_url = Storage::url(config('constants.motorConstant.CV_PROPOSAL_PDF_URL') . 'reliance/'. md5($policy_detail->user_proposal_id). '.pdf');
