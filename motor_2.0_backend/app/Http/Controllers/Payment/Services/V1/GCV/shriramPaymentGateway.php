...
function  makeV1Gcv($request)
{
    ...
    $service_type = config('IC.SHRIRAM.V1.GCV.REQUEST_TYPE'); //constants.cv.shriram.SHRIRAM_CV_REQUEST_TYPE
   
    $return_data = [
       
        'form_action' => config('IC.SHRIRAM.V1.GCV.PAYMENT_URL'), //constants.cv.shriram.SHRIRAM_XML_PAYMENT_URL_PCV
        'form_method' => config('IC.SHRIRAM.V1.GCV.PAYMENT_BEFORE_URL_METHOD'), //constants.IcConstants.shriram.SHRIRAM_PAYMENT_BEFORE_URL_METHOD
        'payment_type' => 0, // form-submit
        'form_data' => [
            'DbFrom' => 'NOVA',
            'QuoteId' => $user_proposal->pol_sys_id,
            'amount' => $user_proposal->final_payable_amount,
            'application' =>  config('IC.SHRIRAM.V1.GCV.APPLICATION_NAME'),//"HeroIns",//config('constants.brokerName'), //SHRIRAM_GCV_APPLICATION_NAME
            'createdBy' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'), //"NiveshIns",//config('constants.IcConstants.shriram.SHRIRAMGCV_USERNAME'),
            'description' => $user_proposal->proposal_no.'-'.$user_proposal->pol_sys_id,
            'isForWeb' => 'true',
            'paymentFrom' => config('IC.SHRIRAM.V1.GCV.PAYMENT_TYPE'), //constants.IcConstants.shriram.SHRIRAM_CV_PAYMENT_TYPE
...
public static function confirm($request)
{
    ...
    if ( policyProductType($request['policyId'])->parent_id == 4  &&  config('IC.SHRIRAM.V1.GCV.REQUEST_TYPE') == 'JSON') {
      
        return self::JSONGcvConfirm($request, $user_proposal);
    }
    ...
    $input_array = [
        'soap:Header' => [
            'AuthHeader' => [
                ...
                'Username' =>  config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'),//constants.IcConstants.shriram.SHRIRAM_USERNAME
                'Password' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_PASSWORD'), //constants.IcConstants.shriram.SHRIRAM_PASSWORD
            ],
        ],
 ...
 //constants.IcConstants.shriram.SHRIRAM_POLICY_APPROVED_URL
    $get_response = getWsData(config('IC.SHRIRAM.V1.GCV.POLICY_APPROVED_URL'), $input_array, 'shriram', $additional_data);
    ...
    if ($response['soap:Body']['PolicyApproveResponse']['PolicyApproveResult']['Err_Code'] == 0) {
        ...
        // $folder = config('constants.motorConstant.CV_PROPOSAL_PDF_URL');
...
static function JSONGcvConfirm($request)
{
    ...
    $get_response = getWsData(config('IC.SHRIRAM.V1.GCV.PAYMENT_STATUS_CHECK_URL'), $rehit_payment_api_request, 'shriram', [
        'enquiryId' => $data['user_product_journey_id'],
        'headers' => [
            'Username' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'), //constants.IcConstants.shriram.SHRIRAMGCV_USERNAME
            'Password' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_PASSWORD'), //constants.IcConstants.shriram.SHRIRAM_PASSWORD
...
$response = json_decode($payment_response->response);
    ...
    $get_response = getWsData(config('constants.cv.iffco.SHRIRAM_POLICY_PDF_URL'), $Policy_payment_api_request, 'shriram', [
        ...
        'headers' => [
            'Username' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'),//constants.IcConstants.shriram.SHRIRAM_POLICY_USERNAME
            'Password' => config('constants.IcConstants.shriram.SHRIRAM_POLICY_PASSWORD'), //constants.IcConstants.shriram.SHRIRAM_POLICY_PASSWORD
            'Accept' => 'application/json',
            'Content-Type' => 'application/json',
        ],
        'requestMethod' => 'post',
    ...
    {
        //constants.cv.shriram.SHRIRAM_CV_REQUEST_TYPE
        if(strtolower(config('IC.SHRIRAM.V1.GCV.REQUEST_TYPE')) == 'json'){
    ...
    static public function generateJSONPDF($request){
        ...
        if ( ! isset($response->PolicyURL))
        {
            ...
            $get_response = getWsData(config('IC.SHRIRAM.V1.GCV.PAYMENT_STATUS_CHECK_URL'), $rehit_payment_api_request, 'shriram', [
                ...
                'headers' => [
                    'Username' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'), //constants.IcConstants.shriram.SHRIRAMGCV_USERNAME
                    'Password' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_PASSWORD'), //constants.IcConstants.shriram.SHRIRAMGCV_PASSWORD
                    'Accept' => 'application/json',
                    'Content-Type' => 'application/json',
                ],
                'requestMethod' => 'post',
