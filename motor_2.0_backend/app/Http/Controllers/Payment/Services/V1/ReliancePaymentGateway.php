...
use App\Http\Controllers\Proposal\Services\V1\RelianceSubmitProposal;
...
class ReliancePaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        $proposal_submit_url = config('IC.RELIANCE.V1.CV.END_POINT_URL_PROPOSAL');

        if (in_array($premium_type, ['breakin', 'short_term_3_breakin', 'short_term_6_breakin']) && $user_proposal->is_inspection_done == 'Y') {
            $proposal_submit_url = config('IC.RELIANCE.V1.CV.END_POINT_URL_PROPOSAL_POST_INSPECTION');
        }
        ...
        $total_tp_amount = (int)$tp_liability + (int)$ll_paid_driver + (int)$ll_paid_cleaner + (int)$external_lpg_cng_tp + (int)$pa_to_paid_driver + (int)$cpa_premium - (int)$tppd_discount_amt + (int)$GeogExtension_tp;
        ...
        if ($proposal_resp->status == '1') {
           ...
           $carrying_capacity = $mmv_data->carrying_capacity ?? '';
            ...
            {
                if ($mmv_data->wheels == 3) {
                $params['userID'] = config('IC.RELIANCE.V1.CV.3W_TP_USERID');
                }
            }
            ...
            $params['subscription-key'] = config('IC.RELIANCE.V1.CV.OCP_APIM_SUBSCRIPTION_KEY');
    
            $payment_url = config('IC.RELIANCE.V1.CV.PAYMENT_GATEWAY_LINK'). '?' . http_build_query($params);
    ...
    public static function confirm($request)
    {
        ...
        if($bool_status)
        {
            ...
            if (config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK_NEW_API_ENABLE') == 'Y') {
                $pdf_url =  config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK_NEW');
                $pdf_Request = [
                    'PolicyNumber' => $policy_no,
                    'SourceSystemID' => config('IC.RELIANCE.V1.CV.SOURCE_SYSTEM_ID'),
                    'SecureAuthToken' => config('IC.RELIANCE.V1.CV.SECURE_AUTH_TOKEN'),
                    'EndorsementNo' => '',
                ];
                ...
                $pdf_url = config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK') . '?PolicyNo=' . $policy_no . '&ProductCode=' . $user_proposal->product_code;
            }
    ...
    public static function proposalStatusService($enquiryId, $policy_detail)
    {
        ...
        $get_response = getWsData(
            config('IC.RELIANCE.V1.CV.END_POINT_URL_PROPOSAL_STATUS'),
            $proposalStatusRequest,
            'reliance',
            [
    ...
    {            
        ...
        if (config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK_NEW_API_ENABLE') == 'Y') {
            $ic_pdf_url =  config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK_NEW');
            $pdf_Request = [
            ...
            $ic_pdf_url =  config('IC.RELIANCE.V1.CV.POLICY_DWLD_LINK') . '?PolicyNo=' . $policy_number . '&ProductCode=' . $policy_detail->product_code;
        }
        if (config('constants.motorConstant.GENERATE_POLICY_PDF') == 'Y') {
            try{                
