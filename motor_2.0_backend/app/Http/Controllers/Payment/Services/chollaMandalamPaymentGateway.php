...
;
        ...
        $return_data = [
            'form_action' => config('constants.IcConstants.cholla_madalam.cv.PAYMENT_GATEWAY_ID_CHOLLA_MANDALAM'),
            'form_method' => 'POST',
        ...
        PaymentRequestResponse::insert([
            ...
            'payment_url' => config('constants.IcConstants.cholla_madalam.cv.PAYMENT_GATEWAY_ID_CHOLLA_MANDALAM'),
            'return_url' => route('cv.payment-confirm', ['cholla_mandalam', 'user_proposal_id' => $proposal->user_proposal_id, 'policy_id' => $request->policyId]),
    ...
    public static  function  create_checksum($enquiryId ,$request)
    {
        ...
        // $new_pg_transaction_id = strtoupper(config('constants.IcConstants.cholla_madalam.cv.BROKER_NAME')).customEncrypt($enquiryId) . date('His', strtotime(now()));
        $new_pg_transaction_id = strtoupper(config('constants.IcConstants.cholla_madalam.cv.BROKER_NAME')). substr(strtoupper(md5(mt_rand())), 0, 7) . date('His', strtotime(now()));

        $str_arr = [
            config('constants.IcConstants.cholla_madalam.cv.PAYMENT_MERCHANT_ID_CHOLLA_MANDALAM'),
            $new_pg_transaction_id,
            'NA',
            (env('APP_ENV') == 'local') ? config('constants.IcConstants.cholla_madalam.cv.STATIC_PAYMENT_AMOUNT_CHOLLA_MANDALAM') :$data->final_payable_amount,
            'NA',
            'NA',
            'NA',
            'INR',
            'NA',
            'R',
            config('constants.IcConstants.cholla_madalam.cv.PAYMENT_SECURITY_ID_CHOLLA_MANDALAM'),
            'NA',
            'NA',
            'F',
            $data->proposal_no,
            ...
            'NA',
            route('cv.payment-confirm', ['cholla_mandalam','enquiry_id' => $enquiryId,'policy_id' => $request['policyId']]),

        ];

        $msg_desc = implode('|', $str_arr);
        $checksum = strtoupper(hash_hmac('sha256', $msg_desc, config('constants.IcConstants.cholla_madalam.cv.PAYMENT_CHECKSUM_CHOLLA_MANDALAM')));
    ...
    public static function confirm($request)
    {
        ...
        if(empty($proposal))
        {
            return redirect(config('constants.IcConstants.cholla_madalam.cv.CV_PAYMENT_FAILURE_CALLBACK_URL'));
        }
        if($request_data!=null && isset($_REQUEST['msg'])) 
        {
            ...
            if ($response[14] == '0300')
            {
                ...
                $pdf_data = $cholla_model->gcv_retry_pdf(base64_encode($response[1]), $request_data['policy_id'], $proposal->user_product_journey_id);
                ...
                if ($pdf_data['status'] || $pdf_data['status']== true) 
                {
                    ...
                    //return redirect(config('constants.motorConstant.CV_PAYMENT_SUCCESS_CALLBACK_URL') . '?' . http_build_query(['enquiry_id' => customEncrypt($request_data['enquiry_id'])]));
                } else {
            ...
            return redirect(paymentSuccessFailureCallbackUrl($enquiryId,'CV','FAILURE'));
            //return redirect(Config('constants.motorConstant.CV_PAYMENT_FAILURE_CALLBACK_URL') . '?' . http_build_query(['enquiry_id' => customEncrypt($request_data['enquiry_id'])]));
    ...
    static public function generatePdf($request){
        ...
        {
        $pdf_data = $cholla_model->gcv_retry_pdf(base64_encode($policy_details->order_id),$request->master_policy_id, $user_product_journey_id);
    ...
    {
        $cholla_model = new ChollaMandalamCvModel();
        $response = $cholla_model->update_transaction_summary((object)$request->all(),'CV');
    ...
    static public function check_payment_status($enquiry_id, $section)
    {
        ...
        $api_merchant_id = config('constants.IcConstants.cholla_madalam.cv.QUERY_API_MERCHANT_ID_CHOLLA_MANDALAM');
        $payment_checksum = config('constants.IcConstants.cholla_madalam.cv.PAYMENT_CHECKSUM_CHOLLA_MANDALAM');
