...
use App\Http\Controllers\Proposal\Services\universalSompoSubmitProposal;
use App\Http\Controllers\Proposal\Services\V2\MISCD\universalSompoSubmitProposalMiscd;
...
class universalSompoPaymentGateway
{
    ...
    public static function make($request)
    {
        ...
        if ($proposal) {
            ...
            if($premium_type === 'breakin'){
                if($is_MISC && config('IC.UNIVERSAL_SOMPO.V2.MISCD.ENABLED') == 'N'){
                    $breakin_proposal = universalSompoSubmitProposalMiscd::submit($proposal, $request, 'skip');
                }else{
                    $breakin_proposal = universalSompoSubmitProposal::submit($proposal, $request, 'skip');
                }
                if($breakin_proposal['status'] !== true){
                    return [
                        'status' => false,
                        'message' => 'Something went wrong in proposal submission'
                    ];
                }
            ...
            if ($is_MISC == 3 && config('IC.UNIVERSAL_SOMPO.V2.MISCD.ENABLED') == 'Y') {
                $paymentUrl = config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_PAYMENT') . '?PosPolicyNo=' . $proposal->proposal_no . '&FinalPremium=' . $proposal->final_payable_amount . '&Src=WA' . '&SubSrc=' . config('constants.IcConstants.universal_sompo.AUTH_CODE_SOMPO_MISCD');
            } elseif ($is_MISC == 3) {
                $paymentUrl = config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_GET_POLICY_STATUS') . '?PosPolicyNo=' . $proposal->proposal_no . '&WACode=' . config('constants.IcConstants.universal_sompo.AUTH_CODE_SOMPO_CV_MISCD');
            } else {
                $paymentUrl =  config('constants.IcConstants.universal_sompo.UNIVERSAL_SOMPO_PAYMENT_END_POINT_URL') . 'PosPolicyNo=' . $proposal->proposal_no . '&FinalPremium=' . $proposal->final_payable_amount . '&Src=WA' . '&SubSrc=' . config('constants.IcConstants.universal_sompo.AUTH_CODE_SOMPO_CV');
            }
            ...
            PaymentRequestResponse::insert([
                ...
                'return_url'                => route('cv.payment-confirm', ['universal_sompo']),
            ...
            $output =self::update_pg_response($request['userProductJourneyId'] ,$request);
    ...
    {
        //payment_url?MSG=SOMPOGINS|50843213|403993715524959103|USGI/WEBAG/0073406/00/000|1043.00|NA|2021-12-16%2018:55:06|1001|Payment%20successfully.|http://124.124.15.42/WAPDFUAT/WAPDFGenerat.aspx?QuoteID=HymbBimCCIrOXN65aL466g==|NA|NA|NA|NA|NA
        ...
        if(empty($pos_policy_no))
        {
            return redirect(config('constants.motorConstant.CV_PAYMENT_FAILURE_CALLBACK_URL')); 
        }
        $proposal = UserProposal::where('proposal_no',$pos_policy_no)
                    ->first();
        $enquiryId = $proposal->user_product_journey_id;
        
        $quote_log = QuoteLog::where('user_product_journey_id', $enquiryId)->first();
        $quote_log_product = $quote_log->master_policy_id;
        $is_MISC = policyProductType($quote_log_product)->parent_id;
        //PDF URL CHANGES
        ...
        if ($payment_status == 'Payment successfully.' || $status_code == '1001') 
        {
            ...
            if ($is_MISC == 3 && config('IC.UNIVERSAL_SOMPO.V2.MISCD.ENABLED') == 'Y'){
                $policy_start_date = $proposal->policy_start_date ?? null;
        ...
        else
        {
            ...
            //return redirect(config('constants.motorConstant.CV_PAYMENT_FAILURE_CALLBACK_URL').'?'.http_build_query(['enquiry_id' => customEncrypt($enquiryId)]));
    ...
    public static function rehitpdf($enquiryId, $policy_no, $pdf_url)
    {
        if ($policy_no && $enquiryId) 
        {
            ...
            try
            {
                ...
                $user_journey_data = UserProductJourney::find($proposal->user_product_journey_id);
                ...
                Storage::put($folder . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf', $pdf_data);
                PolicyDetails::where('proposal_id', $proposal->user_proposal_id)
                ->update([
                    'pdf_url' => $folder . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf',
                ]);
                updateJourneyStage([
                    'user_product_journey_id' => $proposal->user_product_journey_id,
                    'stage' => STAGE_NAMES['POLICY_ISSUED']
              ...
              return [
                    ...
                    'data' => [
                        'pdfUrl' => file_url($folder . 'universal_sompo/' . $proposal->user_proposal_id . '.pdf'),
                    ]];
        ...
        if($policy_details->policy_number != '' && $policy_details->policy_number != 'NA' && $policy_details->pdf_url != '')
        {
            $pdf_response_data = [
                ...
                'data'   => [
                    ...
                    'pdf_link'      => $policy_details->pdf_url
                ]
            ];
            return  response()->json($pdf_response_data);
        }
    ...
    static public function update_pg_response($enquiry_id ,$request)
    {
        ...
        if ($is_MISC == 3 && config('IC.UNIVERSAL_SOMPO.V2.MISCD.ENABLED') == 'Y') {
            $url = config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_GET_POLICY_STATUS_MISCD') . '?PosPolicyNo=' . $proposal_no . '&WACode=' . config('constants.IcConstants.universal_sompo.AUTH_CODE_SOMPO_MISCD');
        } elseif ($is_MISC == 3) {
            $url = config('constants.IcConstants.universal_sompo.END_POINT_URL_UNIVERSAL_SOMPO_GET_POLICY_STATUS') . '?PosPolicyNo=' . $proposal_no . '&WACode=' . config('constants.IcConstants.universal_sompo.AUTH_CODE_SOMPO_CV_MISCD');
        } else {
