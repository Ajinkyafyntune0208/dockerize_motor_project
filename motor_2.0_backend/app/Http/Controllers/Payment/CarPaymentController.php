...
use App\Http\Controllers\Payment\Services\Car\V2\GoDigitPaymentGateway as goDigitOneapiPaymentGateway;
use App\Http\Controllers\Payment\Services\Car\V2\tataAigPaymentGateway as tataAigPaymentGatewayRenewal;
use App\Http\Controllers\Payment\Services\Car\V2\NicPaymentGateway as NicPaymentGatewayV2;
...
use App\Http\Controllers\Payment\Services\Car\V1\hdfcErgoPaymentGateway as hdfcErgoPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\EdelweissPaymentGateway as EdelweissPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\reliancePaymentGateway as reliancePaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\ShriramPaymentGateway as shriramPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\BajajAllianzPaymentGateway as BajajAllianzPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\FutureGeneraliPaymentGateway as FutureGeneraliPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V2\FutureGeneraliPaymentGateway as FutureGeneraliPaymentGatewayV2;
use App\Http\Controllers\Payment\Services\Car\V1\ChollaMandalamPaymentGateway as ChollaMandalamPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\IffcoTokioPaymentGateway as Iffco_tokioPaymentGatewayV1;
use App\Http\Controllers\Payment\Services\Car\V1\LibertyVideoconPaymentGateway as LibertyVideoconPaymentGatewayV1;
...
class CarPaymentController extends Controller
{
    public function makePayment(Request $request)
    {
        ...
        {
            $isSectionMissmatched = isSectionMissmatched($request, 'car', $user_proposal->vehicale_registration_number);
        ...
        if ($request->companyAlias == 'sbi') {
            ...
            if ($user_proposal->is_ckyc_verified != 'Y'  && ($user_proposal->proposer_ckyc_details?->is_document_upload != 'Y')) {

                return response()->json([
                    'status' => false,
                    'message' => 'Something went wrong. Please upload the Documents and try again.'
                ]);
            }
            if ($user_proposal->proposer_ckyc_details?->is_document_upload != 'N' && $user_proposal->proposer_ckyc_details?->ovd_status != 'Y') {

                return response()->json([
                    'status' => false,
                    'message' => 'It seems your Documents Verification is not complete.'
                ]);
            }
        }
        ...
        // 1. policy start date should be equal or greater than current date
        // 2. policy start date should not be greater than 61 days

        //Breakin payment end date handling before Payment 24/04/2023
        if($user_proposal->is_breakin_case == 'Y')
        {
            $breakin_data = \App\Models\CvBreakinStatus::where('user_proposal_id', $user_proposal->user_proposal_id)->first();
            if(!empty($breakin_data))
            {
                $breakin_payment_validate = (config('ENABLE_BREAKIN_PAYMENT_ENDDATE_VALIDATION_ON_MAKEPAYMENT') == 'Y') ? true : false;
                if(!empty($breakin_data->payment_end_date) && $breakin_payment_validate)
                {
                    if(\Illuminate\Support\Carbon::now() <=  $breakin_data->payment_end_date)
                    {
                    ...
                    }else
                    {
                        return response()->json([
                            'status' => false,
                            'msg' => 'This breakin proposal is expired with Payment end date ' . $breakin_data->payment_end_date .'. Please initiate a new breakin proposal.',
                            'paymentEndDate' => $user_proposal->policy_start_date
                        ]);
        ...
        //Breakin payment end date code end
        ...
        switch ($request->companyAlias) {
            case 'shriram':
                ...
                break;
            case 'godigit':
                if ($request->is_renewal == 'Y' && $CorporateVehiclesQuotesRequest['is_renewal'] == 'Y' && config('IC.GODIGIT.V2.CAR.RENEWAL.ENABLE')) {
                    $data = goDigitOneapiPaymentGateway::make($request);
                } elseif (config('IC.GODIGIT.V2.CAR.ENABLE') == 'Y') {
                    $data = goDigitOneapiPaymentGateway::make($request);
                } else {
            ...
            case 'liberty_videocon':
                if(config('IC.LIBERTY_VIDEOCON.V1.CAR.ENABLE') == 'Y'){
                    $data = LibertyVideoconPaymentGatewayV1::make($request);
                } else {
            ...
            case 'reliance':
             ...
             case 'future_generali':
                ...
                elseif(config('IC.FUTURE_GENERALI.V2.CAR.ENABLED') == 'Y')
                {
                    ...
                    $data = FutureGeneraliPaymentGatewayV2::make($request);
                }
                else
                {
            ...
            case 'tata_aig':
                if($CorporateVehiclesQuotesRequest['is_renewal'] == 'Y' && config('IC.TATA.V2.CAR.RENEWAL.ENABLE') == 'Y')
                {    
                    $data = tataAigPaymentGatewayRenewal::make($request);
                }
                else{
            ...
            case 'iffco_tokio':
                ...
                if (config('IC.IFFCO_TOKIO.V1.CAR.ENABLE') == 'Y') {
                    ...
                    $data = Iffco_tokioPaymentGatewayV1::make($request);
                } 
                else{
            ...
            case 'nic':
                ...
                {
                    $data = NicPaymentGatewayV2::make($request);
                } 
                else
                {
                    $data = nicPaymentGateway::make($request);
                }
            break;
            case 'tata_aig_v2':
                if($CorporateVehiclesQuotesRequest['is_renewal'] == 'Y' && config('IC.TATA.V2.CAR.RENEWAL.ENABLE') == 'Y')
                {  
                    ...
                    $data = tataAigV2PaymentGateway::make($request);
                }
    ...
    public function confirm(Request $request, $ic_name)
    {
        ...
        switch ($ic_name) {
            case 'shriram':
                ...
                break;
            case 'godigit':
                if ($request->is_renewal == 'Y' && config('IC.GODIGIT.V2.CAR.RENEWAL.ENABLE')) {
            ...
            case 'liberty_videocon':
                if(config('IC.LIBERTY_VIDEOCON.V1.CAR.ENABLE') == 'Y'){
                    return LibertyVideoconPaymentGatewayV1::confirm($request);
                } else {
            ...
            case 'future_generali':
                if(config('IC.FUTURE_GENERALI.V1.CAR.ENABLED') == 'Y')
                {
                    return futureGeneraliPaymentGatewayV1::confirm($request);
                }
                elseif(config('IC.FUTURE_GENERALI.V2.CAR.ENABLED') == 'Y')
                {
                    
                    return futureGeneraliPaymentGatewayV2::confirm($request);
                }      
                else
                {
             ...
             case 'tata_aig':
                ...
                {
                    return tataAigPaymentGatewayRenewal::confirm($request);
                }
                else{
            ...
            case 'iffco_tokio':
                if (config('IC.IFFCO_TOKIO.V1.CAR.ENABLE') == 'Y') {
                    ...
                    return  Iffco_tokioPaymentGatewayV1::confirm($request);
                } 
                else{
            ...
            case 'raheja':
                return rahejaPaymentGateway::confirm($request);
            break;
            case 'edelweiss':
                if (config('IC.EDELWEISS.V1.CAR.ENABLE') == 'Y') {
            ...
            case 'nic':
                if (config('IC.NIC.V2.CAR.ENABLE') == 'Y') 
                {
                    return NicPaymentGatewayV2::confirm($request);
                }else {
                    return nicPaymentGateway::confirm($request);
                }          
            break;
            case 'tata_aig_v2':
                if( $request->is_renewal == 'Y' && config('IC.TATA.V2.CAR.RENEWAL.ENABLE')=='Y')
