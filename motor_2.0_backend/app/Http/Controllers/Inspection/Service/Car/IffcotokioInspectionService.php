...
class IffcotokioInspectionService extends Controller
{
    //
    ...
    if ($breakinDetails)
    {
        ...
        else
        {
            ...
            {
                $request->api_key = config('IC.IFFCOTOKIO.CAR.BREAKIN.WIMWISURE.API');
            ...
            if (isset($inspection_result['Remarks']) && $inspection_result['Remarks'] == 'APPROVED' || $inspection_done==true) 
            {
                ...
                ->update([
                    ...
                    if($m_policy_start_date->lte($m_payment_end_date))
                    {
                        ...
                        $s_policy_end_date = carbon::now()->addYear()->subDay()->format('d-m-Y');
