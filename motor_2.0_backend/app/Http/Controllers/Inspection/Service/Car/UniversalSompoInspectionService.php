...
class UniversalSompoInspectionService
{
    public static function inspectionConfirm($request)
    {
        ...
        if($breakinDetails)
        {
            ...
            $get_response = getWsData(config('constants.IcConstants.universal_sompo.BREAKIN_STATUS_CHECK_END_POINT_URL_UNIVERSAL_SOMPO_CAR'), $check_inspection_request, 'universal_sompo', [
                'requestMethod' => 'post',
            ...
            if($data)
            {
                ...
                if(isset($response['Status']) && strtolower($response['Status']) == 'accepted')
                {
                    ...
                    ->update([
                        ...
                        'payment_url' => config('constants.motorConstant.CAR_BREAKIN_PAYMENT_URL').customEncrypt($breakinDetails->user_proposal->user_product_journey_id),
                ...
                elseif (isset($response['Status']) && strtolower($response['Status']) == "rejected") 
                {
                    updateJourneyStage([
                                    'user_product_journey_id' => $breakinDetails->user_proposal->user_product_journey_id,
                ...
                else
                {
                    ...
                    $message = (strtolower($response['ErrorMessage']) == "ok") ? "Your inspection is in Pending. Kindly check after some time" : strtolower($response['ErrorMessage']);
