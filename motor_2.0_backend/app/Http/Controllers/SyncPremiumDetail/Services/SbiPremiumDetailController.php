...
class SbiPremiumDetailController extends Controller
{
    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            $master_product_sub_type_code = UserProductJourney::find($enquiryId)->sub_product->parent_product_sub_type_id;
            $type = "PCV";
            if ($master_product_sub_type_code == 4) {
                $type = 'GCV';
            }
            ...
            $Engine_Guard = 0;
            $ncb_discount = $final_od_premium = 0;
            $Tyre_Guard = 0;
            $Consumables = 0;
            $anti_theft = 0;
            $OD_BasePremium = 0;
            $GeoExtn_ODPremium = 0;
            $GeoExtn_TPPremium = 0;
            $Legal_Liability_Conductor_Cleaner = 0;
            $voluntary_excess = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['DeductibleAmount'] ?? 0;
            $tppd_discount = 0;
            ...
            foreach ($array_cover as $key => $cover) {
                if ($cover['ProductElementCode'] == 'C101064') {
                    $final_od_premium = round($cover['BeforeVatPremium'], 2);
                    foreach ($cover['PolicyBenefitList'] as $key => $ODcover) {
                        if ($ODcover['ProductElementCode'] == 'B00002') {
                            $Own_Damage_Basic = $ODcover['AnnualPremium'] + ($cover['LoadingAmount'] ?? 0);
                        } elseif ($ODcover['ProductElementCode'] == 'B00003') {
                            $Non_Electrical_Accessories = $ODcover['AnnualPremium'];
                        } elseif ($ODcover['ProductElementCode'] == 'B00004') {
                            $Electrical_Accessories = $ODcover['AnnualPremium'];
                        } elseif ($ODcover['ProductElementCode'] == 'B00005') {
                            $LPG_CNG_Cover = $ODcover['AnnualPremium'];
                        } elseif ($ODcover['ProductElementCode'] == 'B00006') {
                            $Inbuilt_LPG_CNG_Cover = $ODcover['AnnualPremium'];
                        }
                    }
                    ...
                    foreach ($cover['PolicyBenefitList'] as $key => $BRAcover) {
                        if ($BRAcover['ProductElementCode'] == 'B00025') {
                            $Road_Side_Assistance = $BRAcover['AnnualPremium'];
                        }
                    }
                    ...
                    foreach ($cover['PolicyBenefitList'] as $key => $LLTPcover) {
                        if ($LLTPcover['ProductElementCode'] == 'B00008') {
                            $Third_Party_Bodily_Injury = $LLTPcover['AnnualPremium'];
                        } elseif ($LLTPcover['ProductElementCode'] == 'B00009') {
                            $tppd_discount = abs($LLTPcover['AnnualPremium']);
                        } elseif ($LLTPcover['ProductElementCode'] == 'B00010') {
                            $CNG_LPG_Liability = $LLTPcover['AnnualPremium'];
                        } elseif ($LLTPcover['ProductElementCode'] == 'B00013') {
                            $Legal_Liability_Paid_Drivers = $LLTPcover['AnnualPremium'];
                        } elseif ($LLTPcover['ProductElementCode'] == 'B00069'){
                            $Legal_Liability_Conductor_Cleaner = $LLTPcover['AnnualPremium'];           //They give conductors and cleaners response in single tag
                        }
                    }
                    ...
                    foreach ($cover['PolicyBenefitList'] as $key => $PAcover) {
                        if ($PAcover['ProductElementCode'] == 'B00015') {
                            $PA_Owner_Driver = $PAcover['AnnualPremium'];
                        } elseif ($PAcover['ProductElementCode'] == 'B00016') {
                            $PA_Unnamed_Passenger = $PAcover['AnnualPremium'];
                        } elseif ($PAcover['ProductElementCode'] == 'B00027') {
                            $PA_Paid_Driver = $PAcover['AnnualPremium'];
                        } elseif ($PAcover['ProductElementCode'] == 'B00073') {
                            $PA_Paid_Driver = $PAcover['AnnualPremium'];
                        }
                    }
                } elseif ($cover['ProductElementCode'] == 'C101072') {
                    $Depreciation_Reimbursement = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101067') {
                    $Return_to_Invoice = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101068') {
                    $Protection_NCB = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101073') {
                    $Key_Replacement_Cover = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101075') {
                    $Loss_Personal_Belongings = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101108') {
                    $Engine_Guard = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101110') {
                    $Tyre_Guard = $cover['AnnualPremium'];
                } elseif ($cover['ProductElementCode'] == 'C101111') {
                    $Consumables = $cover['AnnualPremium'];
                }
            }

            $GeoExtn_ODPremium = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['GeoExtnODPrem'] ?? 0;
            $GeoExtn_TPPremium = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['GeoExtnTPPrem'] ?? 0;

            $ncb_discount = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['NCBDiscountAmt'] ?? 0;
            $imtPremium = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['imt23prem'] ?? 0;

            if ($is_anti_theft == '1') {
                $OD_BasePremium = $arr_premium['PolicyObject']['PolicyLobList'][0]['PolicyRiskList'][0]['OD_BasePremium'];
                $anti_theft = round((0.025 * $OD_BasePremium), 2);
            ...
            // if ($type == 'GCV') {
            //     include_once app_path().'/Quotes/Cv/sbi.php';
            //     $ncb_discount = calculateValithoutImt23($ncb_discount);
            // }

            $final_tp_premium = $Third_Party_Bodily_Injury + $CNG_LPG_Liability + $Legal_Liability_Paid_Drivers +  $PA_Paid_Driver + $PA_Unnamed_Passenger + $PA_Owner_Driver + $Legal_Liability_Conductor_Cleaner + $GeoExtn_TPPremium;
            ...
            // $final_od_premium = $Own_Damage_Basic + $LPG_CNG_Cover + $Electrical_Accessories + $Non_Electrical_Accessories + $Inbuilt_LPG_CNG_Cover + $GeoExtn_ODPremium;
            ...
            $net_premium = round($arr_premium['PolicyObject']['BeforeVatPremium'], 2);
            
            $final_payable_amount  = round($arr_premium['PolicyObject']['DuePremium'], 2);
            
            $final_gst_amount   = round($arr_premium['PolicyObject']['TGST'], 2);
            ...
            $updatePremiumDetails = [
                ...
                "ll_paid_conductor" => $Legal_Liability_Conductor_Cleaner,
                "ll_paid_cleaner" => 0,
                "geo_extension_odpremium" => $GeoExtn_ODPremium,
                "geo_extension_tppremium" => $GeoExtn_TPPremium,
                // Discounts
