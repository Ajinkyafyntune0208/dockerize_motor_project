...
class GodigitPremiumDetailController extends Controller
{
        ...
        try {
            ...
            if ($isRenewal) {
                ...
                $methodList = [
                    ...
                    getGenericMethodName('Proposal Submit - IMT-23', 'proposal')
                ];
            }
            ...
            try {
                if (!empty($webserviceId)) {
                    $response = WebServiceRequestResponse::select('request','response','enquiry_id')->find($webserviceId);
                    $enquiryId = $response->enquiry_id;
                    $response = json_decode($response->request);
                    if (!empty($response->motorQuickQuote ?? null)) {
    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            $cng_lpg_tp = 0;
            $cover_pa_unnamed_passenger_premium = $cover_pa_paid_driver_premium =
            $cover_pa_owner_driver_premium = $llpaiddriver_premium = $cover_pa_paid_cleaner_premium =
            $cover_pa_paid_cleaner_premium = $cover_pa_paid_conductor_premium = 0;
            $isLPG = $is_tppd = false;
                    ...
                    case 'thirdPartyLiability':
                        ...
                        case 'legalLiability' :
                            foreach ($value as $cover => $subcover) {
                                ...
                                if ($cover == "cleanersLL") {
                                    $llcleaner_premium = round(str_replace("INR ", "", $subcover->netPremium), 2);
                                }
                            }
                            break;
                            ...
                            foreach ($value as $cover => $subcover) {
                                if (isset($subcover->selection) && $subcover->selection == 1 && isset($subcover->netPremium)) {
                                    ...
                                    if ($cover == 'unnamedCleaner') {
                                        $cover_pa_paid_cleaner_premium = round(str_replace("INR ", "", $subcover->netPremium), 2);
                                    }

                                    if ($cover == 'unnamedConductor') {
                                        $cover_pa_paid_conductor_premium = round(str_replace("INR ", "", $subcover->netPremium), 2);
                                    }
                                }
                            }
                            break;
            ...
            $mmv = get_mmv_details(json_decode(json_encode(['product_sub_type_id' => $product_sub_type_id])), $requestData->version_id, 'godigit');
            
            if (!$isLPG) {
                $isLPG = !empty($mmv['data']) && in_array($mmv['data']['fuel_type'], ['CNG', 'PETROL+CNG', 'DIESEL+CNG', 'LPG']);
            }
            ...
            $tppd_discount = $is_tppd ? (get_parent_code($product_sub_type_id) == 'PCV' ? 150 : 200) : 0;
            $final_total_discount = $ncbDiscount;
            $final_od_premium = $od;
            $addon_premium = $zeroDep + $rsa + $engineProtect + $tyreSecure + $rti + $consumables + $lopb + $keyReplacement;
            $cover_pa_paid_driver_premium = $cover_pa_paid_driver_premium + $cover_pa_paid_cleaner_premium + $cover_pa_paid_conductor_premium;
