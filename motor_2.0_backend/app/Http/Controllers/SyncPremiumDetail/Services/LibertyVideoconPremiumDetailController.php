...
class LibertyVideoconPremiumDetailController extends Controller
{
    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            $llpaiddriver_premium = round($response['LegalliabilityToPaidDriverValue']);
            $cover_pa_owner_driver_premium = round($response['PAToOwnerDrivervalue']);
            $cover_pa_paid_driver_premium = round($response['PatoPaidDrivervalue']);
            $cover_pa_unnamed_passenger_premium = round($response['PAToUnnmaedPassengerValue']);
            ...
            $llPaidPassenger = round(($response['LegalLiabilityPassengerValue'] ?? 0), 2);

            $final_od_premium = round($response['TotalODPremiumValue'], 2);
            $final_tp_premium = round($response['TotalTPPremiumValue'], 2) + $cover_pa_owner_driver_premium;
            ...
            $updatePremiumDetails = [
                ...
                // TP Tags
                "basic_tp_premium" => $tppd + $llPaidPassenger,
