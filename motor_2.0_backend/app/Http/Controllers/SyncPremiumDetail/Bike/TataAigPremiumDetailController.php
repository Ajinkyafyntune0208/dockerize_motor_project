...
class TataAigPremiumDetailController extends Controller
{
    public function syncDetails($enquiryId)
    {
        ...
        if (!$isRenewal) {
            $methodList = [
                'Premium Recalculation - Proposal',
                getGenericMethodName('Premium Recalculation - Proposal', 'proposal')
            ];
            ...
            foreach ($logs as $log) {
                ...
                if (
                    !empty($response) &&
                    !empty($response['data']['premium_break_up']['net_premium'] ?? $response['data'][0]['data']['premium_break_up']['net_premium'] ?? null)
                ) {
                    $webserviceId = $log['id'];
                    break;
                }
    ...
    public static function saveV2PremiumDetails($webserviceId)
    {
        try {
            ...
            $premiumResponse = $response['data']['premium_break_up'] ?? $response['data'][0]['data']['premium_break_up'];
            ...
            $netPremium   = $premiumResponse['final_premium'];
            $finalPayable = $premiumResponse['net_premium'];

            if (
                !empty($netPremium) &&
                !empty($finalPayable) &&
                $netPremium > $finalPayable
            ) {
                $temp = $netPremium;
                $netPremium = $finalPayable;
                $finalPayable = $temp;
            ...
            $electrical = (float) ($odResponse['od']['electrical_prem'] ?? 0);
            $lpgOd = (float) ($odResponse['od']['cng_lpg_od_prem'] ?? 0);
            $geoOd = (float) ($odResponse['od']['geography_extension_od_prem'] ?? 0);


            $basicTp = (float) ($tpResponse['basic_tp'] ?? $tpResponse['basic_tp_prem'] ?? 0);
            $lpgTp = (float) ($tpResponse['cng_lpg_tp_prem'] ?? 0);
            $tppdDiscount = (float) ($tpResponse['tppd_prem'] ?? $tpResponse['disc_tppd_prem'] ?? 0);
            $geoTp = (float) ($tpResponse['geography_extension_tp_prem'] ?? $tpResponse['geo_extension_tp_prem'] ?? 0);

            $paUnnamed = (float) ($tpResponse['pa_unnamed_prem'] ?? 0);
            $llPaid = (float) ($tpResponse['ll_paid_drive_prem'] ?? $tpResponse['ll_paid_prem'] ?? $tpResponse['ll_paid_driver_prem'] ?? 0);
            $papaid = (float) ($tpResponse['pa_paid_drive_prem'] ?? $tpResponse['pa_paid_prem'] ?? 0);
            $cpa = (float) ($tpResponse['cpa_prem'] ?? $tpResponse['pa_owner_prem'] ?? $tpResponse['pa_cover_prem'] ?? 0);

            $antitheft = (float) ($odDiscountResponse['atd_disc_prem'] ?? $odDiscountResponse['disc_antitheft_prem'] ?? 0);
            $automoblie = (float) ($odDiscountResponse['aam_disc_prem'] ?? 0);
            $voluntaryDeductible = (float) ($odDiscountResponse['vd_disc_prem'] ?? 0);
            $ncb = (float) ($odDiscountResponse['ncb_prem'] ?? $addonResponse['ncb_prem'] ?? 0);
            ...
            $finalTp = (float) ($tpResponse['total_tp'] ?? $tpResponse['total_tp_prem'] ?? 0);
