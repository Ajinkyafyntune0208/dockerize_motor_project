...
class OrientalPremiumDetailController extends Controller
{
    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            $final_tp_premium = round(($res_array['TP_PREMIUM'] ?? 0), 2) + round(($res_array['LL_PAID_DRIVER'] ?? 0), 2) + round($res_array['PA_PAID_DRIVER'], 2) +
                round($res_array['UNNAMED_PASSENGER'], 2) + round($res_array['CNG_TP'], 2) + $res_array['OTHER_FUEL2'] + $res_array['PA_OWNER'] + $res_array['GEO_TP'];

            $final_tp_premium -= $res_array['TPPD'];

            $final_od_premium = round(($res_array['OD_PREMIUM'] ?? 0), 2) + $res_array['ELEC'] + $res_array['CNG'] + $res_array['OTHER_FUEL1'] + $res_array['OD_LOADING_AMOUNT'] + $res_array['GEO_OD'];

            $total_addon = round(($res_array['ZERO_DEP'] - $res_array['ZERO_DEP_DISC']), 2) + $res_array['CONSUMABLES'] + $res_array['KEYREPLACEMENT'] +
                $res_array['ENG_PRCT'] +  $res_array['RTI'] + $res_array['LOSSPER_BELONG'];
            ...
            if (!empty($productData)) {
                ...
                if (!empty($premium_type)) {
                    $is_tp = in_array($premium_type, ['third_party', 'third_party_breakin']);
            ...
            $updatePremiumDetails = [
                ...
                "loading_amount" => $res_array['OD_LOADING_AMOUNT'],
                ...
                "geo_extension_odpremium" => $res_array['GEO_OD'],
                "geo_extension_tppremium" => $res_array['GEO_TP'],
                // Discounts
