...
class HdfcErgoPremiumDetailController extends Controller
{
    ...
    public static function saveV1PremiumDetails($webserviceId)
    {
        try {
            ...
            $GeogExtension_tp = $OwnPremises_OD = $OwnPremises_TP = 0;
            ...
            if ($electrical_accessories > 0) {
                $Nil_dep += (float) ($premium_data['Elec_ZD_Premium'] ?? 0);
                $eng_protector += (float) ($premium_data['Elec_ENG_Premium'] ?? 0);
                $ncb_protection += (float) ($premium_data['Elec_NCB_Premium'] ?? 0);
                $consumables_cover += (float) ($premium_data['Elec_COC_Premium'] ?? 0);
                $rti += (float) ($premium_data['Elec_RTI_Premium'] ?? 0);
            }
            if ($non_electrical_accessories > 0) {
                $Nil_dep += (float) ($premium_data['NonElec_ZD_Premium'] ?? 0);
                $eng_protector += (float) ($premium_data['NonElec_ENG_Premium'] ?? 0);
                $ncb_protection += (float) ($premium_data['NonElec_NCB_Premium'] ?? 0);
                $consumables_cover += (float) ($premium_data['NonElec_COC_Premium'] ?? 0);
                $rti += (float) ($premium_data['NonElec_RTI_Premium'] ?? 0);
            }

            if ($lpg_cng > 0) {
                $Nil_dep += (float) ($premium_data['Bifuel_ZD_Premium'] ?? 0);
                $eng_protector += (float) ($premium_data['Bifuel_ENG_Premium'] ?? 0);
                $ncb_protection += (float) ($premium_data['Bifuel_NCB_Premium'] ?? 0);
                $consumables_cover += (float) ($premium_data['Bifuel_COC_Premium'] ?? 0);
                $rti += (float) ($premium_data['Bifuel_RTI_Premium'] ?? 0);
            }
            ...
            $addonPremium = $Nil_dep + $roadside_asst + $consumables_cover + $key_replacement + $eng_protector +
            $key_replacement + $eng_protector + $ncb_protection + $tyre_secure + $rti + $loss_of_personal_belongings;

            $od_premium += $addonPremium;
            $updatePremiumDetails = [
                // OD Tags
                "basic_od_premium" => $basic_od_premium,
                "loading_amount" => 0,
                "final_od_premium" => $od_premium,
                // TP Tags
    ...
    public static function saveV2PremiumDetails($webserviceId)
    {
        try {
            ...
            $is_engine_protector = $request['AddOnCovers']['planType'] ?? '';
                ...
                foreach ($response['Data'][0]['AddOnCovers'] as $addon_cover) {
                    switch ($addon_cover['CoverName']) {
                        ...
                        case 'EngineProtection':
                            $engine_protection = $is_engine_protector == 'ENGEBOX' ? $addon_cover['CoverPremium'] : 0;
                            break;
    ...
    public static function saveRenewalPremiumDetails($webserviceId)
    {
        try {
            ...
            $ElectricalAccessoriesPremium = $TwoWheelerRenewalPremiumList['ElectricalAccessoriesPremium'] ?? 0;
            $NonelectricalAccessoriesPremium = $TwoWheelerRenewalPremiumList['NonelectricalAccessoriesPremium'] ?? 0;
            $LpgCngKitODPremium = $TwoWheelerRenewalPremiumList['LpgCngKitODPremium'] ?? 0;
            ...
            $LLPaidDriversPremium = $TwoWheelerRenewalPremiumList['LLPaidDriversPremium'] ?? 0;
            $UnnamedPassengerPremium = $TwoWheelerRenewalPremiumList['UnnamedPassengerPremium'] ?? 0;
            $PAPaidDriverPremium = $TwoWheelerRenewalPremiumList['PAPaidDriverPremium'] ?? 0;
            $PremiumNoOfLLPaidDrivers = $TwoWheelerRenewalPremiumList['PremiumNoOfLLPaidDrivers'] ?? 0;
            $LpgCngKitTPPremium = $TwoWheelerRenewalPremiumList['LpgCngKitTPPremium'] ?? 0;
            $PACoverOwnerDriverPremium = $TwoWheelerRenewalPremiumList['PACoverOwnerDriverPremium'] ?? 0;

            $finalTp = $basic_tp + $LLPaidDriversPremium + $UnnamedPassengerPremium + $PAPaidDriverPremium + $PremiumNoOfLLPaidDrivers + $LpgCngKitTPPremium + $PACoverOwnerDriverPremium;
            $NewNcbDiscountPercentage = $TwoWheelerRenewalPremiumList['NewNcbDiscountPercentage'] ?? 0;
            //Discount 
            $NcbDiscountAmount = ($requestData->is_claim == 'Y' && $NewNcbDiscountPercentage == 0) ? 0 : ($TwoWheelerRenewalPremiumList['NewNcbDiscountAmount'] ?? 0);
            $OtherDiscountAmount = $TwoWheelerRenewalPremiumList['OtherDiscountAmount'] ?? 0;
            $tppD_Discount = 0;
            ...
            if (is_array($AddOnCovers))  //LOPB cover is discontinued by hdfc
            {
                foreach ($AddOnCovers as $value) {
                    if (in_array($value['CoverName'], $AddOnsOptedLastYear)) {
                        ...
                        } else if (in_array($value['CoverName'], ['ENGEBOX', 'EngineProtection'])) {
            ...
            $NetPremiumAmount = $TwoWheelerRenewalPremiumList['NetPremiumAmount'] ?? $NetPremiumAmount;
            $TaxAmount = round(($NetPremiumAmount * 0.18), 2);
            $TaxAmount = $TwoWheelerRenewalPremiumList['TaxAmount'] ?? $TaxAmount;
            $TotalPremiumAmount = $NetPremiumAmount + $TaxAmount;
            $TotalPremiumAmount = $TwoWheelerRenewalPremiumList['TotalPremiumAmount'] ?? $TotalPremiumAmount;
            $updatePremiumDetails = [
                // OD Tags
