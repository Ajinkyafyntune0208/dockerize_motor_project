...
class IciciLombardPremiumDetailController extends Controller
{
    public function syncDetails($enquiryId)
    {
        try {
            ...
            if ($isRenewal) {
                ...
                $methodList = [
                    'premiumCalculation',
                    getGenericMethodName('premiumCalculation', 'proposal')
                ];
            }
    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            $roadSideAssistanceService = $riskDetails['roadSideAssistance'] ?? 0;

            if (!empty($riskDetails['smartAssist'])) {
                $roadSideAssistanceService = $riskDetails['smartAssist'];
            }
            ...
            $updatePremiumDetails = [
                ...
                "loading_amount" => 0,
                "final_od_premium" => $response['totalOwnDamagePremium'] ?? 0,
                // TP Tags
                "basic_tp_premium" => $riskDetails['basicTP'],
                "final_tp_premium" => $response['totalLiabilityPremium'] ?? 0,
                // Accessories
                ...
                "road_side_assistance" => $roadSideAssistanceService,
                ...
                "ll_paid_conductor" => 0,
                "ll_paid_cleaner" => 0,
                "ll_paid_employee" => $riskDetails['employeesOfInsured'] ?? 0,
                ...
                // Final tags
                "net_premium" => $response['packagePremium'] ?? $response['totalLiabilityPremium'] ?? 0,
                "service_tax_amount" => $response['totalTax'] ?? 0,
                "final_payable_amount" => $response['finalPremium'] ?? 0,
            ];
