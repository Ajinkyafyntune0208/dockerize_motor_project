...
class TataAigPremiumDetailController extends Controller
{
    public function syncDetails($enquiryId)
    {
        $methodList = [
            ...
            'Premium Re-Calculation Renewal',
            getGenericMethodName('Premium Recalculation - Proposal', 'proposal'),
            getGenericMethodName('Premium Re-Calculation Renewal', 'proposal')
        ];
        ...
        foreach ($logs as $log) {
            ...
            if (
                !empty($response) &&
                !empty($response['data']['premium_break_up']['net_premium'] ??
                    $response['data'][0]['data']['premium_break_up']['net_premium'] ??
                    $response['data']['premiumSchemes'][0]['premium_break_up']['net_premium'] ?? null)
            ) {
                $webserviceId = $log['id'];
                break;
            }
    ...
    public static function saveV2PremiumDetails($webserviceId)
    {
        try {
            ...
            $premiumResponse = $response['data']['premium_break_up'] ??
                $response['data'][0]['data']['premium_break_up'] ??
                $response['data']['premiumSchemes'][0]['premium_break_up'];
            ...
            $finalPayable = $premiumResponse['final_premium'];
            
            if ($premiumResponse['net_premium'] > $premiumResponse['final_premium']) {
            ...
            $llPaid = (float) ($tpResponse['ll_paid_drive_prem'] ?? $tpResponse['ll_paid_prem'] ?? 0);
            $papaid = (float) ($tpResponse['pa_paid_drive_prem'] ?? $tpResponse['pa_paid_prem'] ?? 0);
            $cpa = (float) ($tpResponse['cpa_prem'] ?? $tpResponse['pa_owner_prem'] ?? 0);
            ...
            $ncb = (float) ($odDiscountResponse['ncb_prem'] ?? 0);
