    ...
    public static function savePremiumDetails($webserviceId)
    {
        try {
            ...
            if (isset($service_response['premiumSummeryList_out']['typ:WeoMotPremiumSummaryUser'])) {
                ...
                if (isset($covers['od'])) {
                    ...
                    foreach ($covers as $key => $cover) {
                        ...
                        } elseif (in_array($cover['typ:paramDesc'], [
                            '7Commercial Discount',
                            'Commercial Discount'
                        ])) {
                        ...
                        elseif(in_array($cover['typ:paramDesc'], ['CHDH Additional Discount/Loading', 'CHDH Additional Discount/Loading '])) 
                        {
                            if ($cover['typ:od'] > 0) 
                            {
                                $uw_loading_amount = round(abs($cover['typ:od']));
                            }
                            else
                            {
                            ...
                            'LL to employees of the insured',
                            'LL to employee of the insured'
                        ])) {
                            $ll_paid_emp = round($cover['typ:act']);
                        }
                    }
                }
            }
            ...
            $final_od_premium = $basic_od + $non_electrical_accessories + $electrical_accessories
                + $lpg_cng + $ExtraPremiumForRejectedRTO + $uw_loading_amount;
                $totalTP = $tppd + $pa_unnamed + $ll_paid_driver + $lpg_cng_tp + $pa_owner - abs($restricted_tppd) + $ll_paid_emp;
            $total_od_discount = $ncb_discount + $other_discount + $voluntary_deductible  + $antitheft_discount_amount;

            $updatePremiumDetails = [
                // OD Tags
                "basic_od_premium" => $basic_od,
                "loading_amount" => $uw_loading_amount,
