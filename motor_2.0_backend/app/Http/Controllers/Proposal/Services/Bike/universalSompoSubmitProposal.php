...
use Carbon\Carbon;
use DateTime;
use App\Http\Controllers\Proposal\Services\Bike\V2\UniversalSompoSubmitProposal as v2ProposalSubmit;
...
class universalSompoSubmitProposal
{
    ...
    public static function submit($proposal, $request, $step='')
    {
        if (config('IC.UNIVERSAL_SOMPO.V2.BIKE.ENABLE') == 'Y'){
            return  v2ProposalSubmit::submit($proposal, $request, $step='');
            }
        ...
        try {
                        ...
                        'PreviousPolDtlGroupData' => [
                            ...
                            'PolicyEffectiveTo' => [
                                ...
                                '@attributes' => [
                                    ...
                                    'Value' => ($requestData->business_type == 'newbusiness'|| $requestData->previous_policy_type == 'Not sure' ? '' : ($premium_type == 'own_damage' ? date('d/m/Y', strtotime($proposal->tp_end_date)) : $PreviousPolExpDt))
                                ],
                            ],
                            'PolicyEffectiveFrom' => [
                                '@value' => '',
                                '@attributes' => [
                                    ...
                                    'Value' => ($requestData->business_type == 'newbusiness' || $requestData->previous_policy_type == 'Not sure' ? '' : ($premium_type == 'own_damage' ? date('d/m/Y', strtotime($proposal->tp_start_date)) : $previous_policy_start_date))
                                ],
                            ],
                            'DateOfInspection' => [
                                '@value' => '',
            ...
            if (isset($get_response)) {
                $returnData['webservice_id']=$get_response['webservice_id'];
                $returnData['table']=$get_response['table'];
            }
