    ...
    public static function submit($proposal, $request)
    {
        ...
        $current_ncb_rate = 0;
            ...
            if ($requestData->business_type == 'newbusiness')
                ...
                $get_response = getWsData(config('IC.EDELWEISS.V1.BIKE.END_POINT_URL_TOKEN_GENERATION'),'', 'edelweiss', //constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_TOKEN_GENERATION
                [
                    'enquiryId' => $enquiryId,
                    'requestMethod' =>'post',
               ...
               if(isset($token_data['access_token']))
            {
                ...
                if (!empty($additional['applicable_addons'])) {
                    foreach ($additional['applicable_addons'] as $key => $data) {
                        ...
                        if ($data['name'] == 'Zero Depreciation' && $productData->zero_dep == 0 ) {
        ...
        if(!$is_od){
            ...
            $quote_request_data = [
                'channelCode'                       => '002',
                'branch'                            => config('IC.EDELWEISS.V1.BIKE.BRANCH'), //constants.IcConstants.edelweiss.EDELWEISS_BRANCH
                'make'                              => $mmv_data->make,
            ...
            //constants.IcConstants.edelweiss.END_POINT_URL_EDELWEISS_BIKE_QUOTE_GENERATION
            $get_response = getWsData(config('IC.EDELWEISS.V1.BIKE.END_POINT_URL_QUOTE_GENERATION'),$quote_request_data, 'edelweiss',
                        [
                            'enquiryId' => $enquiryId,
                            'requestMethod' =>'post',
                            'productName'  => $productData->product_name,
                ...
                if($allowableDiscount < 300){
                    foreach($contractDetails as $key => $value)
                        ...
                        $proposal_request_data = [
                            'commissionContractID'            => config('IC.EDELWEISS.V1.BIKE.CONTRACT_COMMISION_ID'), //constants.IcConstants.edelweiss.EDELWEISS_CONTRACT_COMMISION_ID
                            'branch'                          => config('IC.EDELWEISS.V1.BIKE.BRANCH'), //constants.IcConstants.edelweiss.EDELWEISS_BRANCH
                            'agentEmail'                      => config('IC.EDELWEISS.V1.BIKE.AGENT_EMAIL'),//'shivakumar.bale@qualitykiosk.com', //constants.IcConstants.edelweiss.EDELWEISS_AGENT_EMAIL
                            'saleManagerCode'                 => config('IC.EDELWEISS.V1.BIKE.SALES_MANAGER_CODE'), //constants.IcConstants.edelweiss.EDELWEISS_SALES_MANAGER_CODE
                            'saleManagerName'                 => config('IC.EDELWEISS.V1.BIKE.SALES_MANGER_NAME'), //constants.IcConstants.edelweiss.EDELWEISS_SALES_MANGER_NAME
                        ...
                        if ($is_pos_enabled == 'Y' && isset($pos_data->seller_type) && $pos_data->seller_type == 'P' && $quote->idv <= 5000000)
                        {
                            ...
                            //
                        $get_response = getWsData(config('IC.EDELWEISS.V1.BIKE.END_POINT_URL_PROPOSAL_GENERATION'),$proposal_request_data, 'edelweiss',
                        [
                            'enquiryId' => $enquiryId,
                            'requestMethod' =>'post',
                            'productName'  => $productData->product_name,
