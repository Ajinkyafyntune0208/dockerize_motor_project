    ...
    public static function submit($proposal, $request) {
        ...
        /* if(($requestData->business_type != "newbusiness") && ($productData->zero_dep == '0') && ($requestData->zero_dep_in_last_policy != 'Y'))
        {
            return  response()->json([
                'status' => false,
                'message' => 'Zero_dep is not available because zero dep is not part of your previous policy'
            ]);
        } */
        ...
        if ($requestData->business_type == 'newbusiness') {
            ...
            $tp_end_date =  date('d-m-Y', strtotime(date('Y-m-d', strtotime('+5 year -1 day', strtotime($tp_start_date))))) ;
        ...
        if ($premium_type == 'third_party' || $premium_type == 'third_party_breakin') {
            ...
            $tp_start_date = date('d-m-Y', strtotime($policy_start_date)) ;
            $tp_end_date =   $requestData->business_type == 'newbusiness'? date('d-m-Y', strtotime(date('Y-m-d', strtotime('+5 year -1 day', strtotime($tp_start_date))))) : date('d-m-Y', strtotime(date('Y-m-d', strtotime('+1 year -1 day', strtotime($tp_start_date))))) ;
        ...
        if (!empty($proposal->previous_insurance_company) && $requestData->business_type != 'newbusiness') {
            ...
            if (in_array($requestData->previous_policy_type, ['Comprehensive', 'Own-damage'])) {
        ...
        $UserID = (($tp_only == 'true') && !empty(config('IC.RELIANCE.V1.BIKE.TP_USERID'))) ? config('IC.RELIANCE.V1.BIKE.TP_USERID') : config('IC.RELIANCE.V1.BIKE.USERID');

        $SourceSystemID = (($tp_only == 'true') && !empty(config('IC.RELIANCE.V1.BIKE.TP_SOURCE_SYSTEM_ID')) )? config('IC.RELIANCE.V1.BIKE.TP_SOURCE_SYSTEM_ID') : config('IC.RELIANCE.V1.BIKE.SOURCE_SYSTEM_ID');

        $AuthToken = (($tp_only == 'true') && !empty(config('IC.RELIANCE.V1.BIKE.TP_AUTH_TOKEN')) ) ? config('IC.RELIANCE.V1.BIKE.TP_AUTH_TOKEN') : config('IC.RELIANCE.V1.BIKE.AUTH_TOKEN');
        ...
        $premium_req_array = [
            ...
            'Cover' => [
                ...
                'EMIprotectionCover' => '',
                "IsA2KSelected" => $RoadSideAssistance,
                "A2KDiscountCover" =>  [
                    "A2KCover" => [
                        "CoverageName" => "Assistance Cover",
                        "IsChecked" => $RoadSideAssistance,
                        "Rate" => 100.00,
                        "CoverCode" => "Cover15",
                        "SubCoverName" => "",
        ...
        $RTI2w = config('IC.RELIANCE.V1.BIKE.RTI2W');
        ...
        $get_response = getWsData(
            Config::get('IC.RELIANCE.V1.BIKE.END_POINT_URL_COVERAGE'),
            $premium_req_array,
            'reliance',
            [
                ...
                'headers' => [
                    'Ocp-Apim-Subscription-Key' => config('IC.RELIANCE.V1.BIKE.OCP_APIM_SUBSCRIPTION_KEY')
                ]
            ]
        );
        ...
        if ($coverage_res_data) {
            ...
            if (empty($coverage_res_data->ErrorMessages)) {
                ...
                if (isset($coverage_res_data->LstAddonCovers)) {
                    ...
                    if($v->CoverageName == 'Assistance Cover (Two Wheeler Shield)' && $RoadSideAssistance == 'true')
                    {
                        ...
                        $PcRoadSideAssistance = "true";
                        $PcCovercode = $v->A2KCoverCode;
                        $PcCoverageName = $v->CoverageName;
                        $PcPremium = $v->rate;
                    }
                    }
                    ...
                    $get_response = getWsData(
                        config('IC.RELIANCE.V1.BIKE.END_POINT_URL_PREMIUM'),
                        $premium_req_array,
                        'reliance',
                        [
                            'root_tag' => 'PolicyDetails',
                        ...
                        if (trim($motorPolicy->ErrorMessages) == '') {
                            ...
                            $proposal_array = [
                                ...
                                'Cover' => [
                                    ...
                                    'IsVoluntaryDeductableOpted' => $is_voluntary_deductible,
                                    'IsElectricalItemFitted' => 'false',
                                    'ElectricalItemsTotalSI' => 'false',
                                    'IsPAToOwnerDriverCoverd' => $IsPAToOwnerDriverCoverd,
                                    'IsTPPDCover' => $TPPDCover,
                                    'IsBasicODCoverage' => 'true',
                                    'IsBasicLiability' => $TPPDCover,
                                    'IsNonElectricalItemFitted' => 'false',
                                    'NonElectricalItemsTotalSI' => 'false',
                                    'IsPAToDriverCovered' => 'false',
                                    ...
                                    "IsA2KSelected" => $PcRoadSideAssistance,
                                    "A2KDiscountCover" =>  [
                                        "A2KCover" => [
                                            "CoverageName" => $PcCoverageName,
                                            "IsChecked" => $PcRoadSideAssistance,
                                            "Rate" => $PcPremium,
                                            "CoverCode" => $PcCovercode,
                                            "SubCoverName" => "",
                                            "CalculationType" => "ODDiscount"
                                        ]
                                    ],
                            ...
                            $get_response = getWsData(
                                config('IC.RELIANCE.V1.BIKE.END_POINT_URL_PROPOSAL'),
                                $proposal_array,
                                'reliance',
                                [
                                    'root_tag' => 'PolicyDetails',
                            ...
                            if($motorPolicy->InspectionErrorMessage != ''){
                                return [
                                    'status' => false,
                                    'webservice_id' => $get_response['webservice_id'],
                                    'table' => $get_response['table'],
                                    'message' => $motorPolicy->InspectionErrorMessage
                                ];
                            }
                            if($motorPolicy->status == '1') {
                                ...
                                $od_discount = $ncb_discount + $voluntary_deductible + $anti_theft + $other_discount;
                                $total_od_amount = $basic_own_damage + $electrical_accessories + $non_electrical_accessories + $lpg_cng - $od_discount;
                                $total_tp_amount = $tppd + $liabilities + $pa_unnamed + $lpg_cng_tp + $pa_paid_driver + $pa_owner - $tppd_discount;
                                $total_addon_amount = $zero_dep_amount + $RTIAddonPremium;
