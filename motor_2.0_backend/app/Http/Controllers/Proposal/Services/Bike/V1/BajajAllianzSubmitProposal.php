...
class BajajAllianzSubmitProposal extends Controller
{
    public static function submit($proposal, $request, $return_request = false)
    {
        ...
        if($requestData->business_type == 'newbusiness') {
            ...
            $polType = '1';
            $product4digitCode = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_NEW_BUSINESS");;
            $prvInsCompany = '0';
            $no_claim_bonus = '0';
            $date_diff = 0 ;
            if ($premium_type == "third_party") {
                $product4digitCode = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_TP");
            }
            ...
            $product4digitCode =  config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE");
            if($premium_type == 'own_damage') {
                ...
                $product4digitCode =  config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_OD");
            }
            if ($premium_type == "third_party") {
                $product4digitCode = config("IC.BAJAJ_ALLIANZ.V1.BIKE.PRODUCT_CODE_TP");
            }
        ...
        $pUserId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_NAME");
        $bajaj_new_tp_url = config("IC.BAJAJ_ALLIANZ.V1.BIKE.NEW_TP_URL_ENABLE");
        $tp_only = ($premium_type == 'third_party') ? 'true' : 'false';
        
        if (config('constants.posTesting.IS_POS_TESTING_MODE_ENABLE_BAJAJ') == 'Y') {
            $extCol40 = 'DNPPS5548E';
            $pUserId = ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') ? config("IC.BAJAJ_ALLIANZ.V1.BIKE.USERNAME_POS_TP") : config("IC.BAJAJ_ALLIANZ.V1.BIKE.CAR_USERNAME_POS");
        }
        ...
        if ($is_pos == 'Y' && isset($pos_data->seller_type) && $pos_data->seller_type == 'P') {
            ...
            $extCol40 = 'DNPPS5548E';
            $pUserId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_NAME_POS");
        }else {
            $extCol40 = '';
        }

        $pPassword = ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') ? config("IC.BAJAJ_ALLIANZ.V1.BIKE.PASSWORD_TP") : config("IC.BAJAJ_ALLIANZ.V1.BIKE.AUTH_PASS");
        $branch = config("IC.BAJAJ_ALLIANZ.V1.BIKE.BRANCH_OFFICE_CODE");
        ...
        if ($proposal->owner_type == "I") {
            if($quote_log->premium_json['businessType'] == 'New Business') {
                ...
                $extCol24 = isset($selected_addons->compulsory_personal_accident[0]['tenure']) ? $selected_addons->compulsory_personal_accident[0]['tenure'] : '5';
        ...
        if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
            $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.PROPOSAL_TP_URL'), $input_array, 'bajaj_allianz', $additional_data);
        } else {
            $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.PROPOSAL_END_POINT_URL'), $input_array, 'bajaj_allianz', $additional_data);
        }
        ...
        if ($service_response['pErrorCode_out'] == '0') {
            ...
            if (isset($service_response['premiumSummeryList_out']['typ:WeoMotPremiumSummaryUser'])) {
                    ...
                    'soapenv:Body'   => [
                        'web:issuePolicy' => [
                            ...
                            'pWeoMotPolicyIn_inout' => [
                                ...
                                'typ:deptCode' => '18',
                                'typ:branchCode' => config("IC.BAJAJ_ALLIANZ.V1.BIKE.BRANCH_OFFICE_CODE"),
                ...
                if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
                    $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.POLICY_ISSUE_TP_URL'), $input_array, 'bajaj_allianz', $additional_data);
                } else {
                    $get_response = getWsData(config('IC.BAJAJ_ALLIANZ.V1.BIKE.END_POINT_URL_ISSUE_POLICY'), $input_array, 'bajaj_allianz', $additional_data);
                }
                $response = $get_response['response'];
                ...
                if($status) {
                    if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
                        ...
                        $source_name = config('IC.BAJAJ_ALLIANZ.V1.BIKE.PAYMENT_GATEWAY_SOURCE_NAME') ?? 'WS_MOTOR';
                        $url = config('IC.BAJAJ_ALLIANZ.V1.BIKE.PAYMENT_GATEWAY_URL') . '?requestId=' . $proposal_no . '&Username=' . $pUserId. '&sourceName='.$source_name;
                    }
                    UserProposal::where('user_product_journey_id', $enquiryId)
                    ->where('user_proposal_id', $proposal->user_proposal_id)
                    ->update([
    ...
    public static function renewalSubmit($proposal, $request, $return_request = false)
    {
        ...
        $extCol40 = (isset($proposal->pan_number) ? $proposal->pan_number : '');
        $pUserId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.USERNAME");
        if ($is_pos_enabled == 'Y') {
            if (isset($pos_data->seller_type) && $pos_data->seller_type == 'P') {
                $extCol40 = $pos_data->pan_no;
                $pUserId = config("IC.BAJAJ_ALLIANZ.V1.BIKE.USERNAME_POS");
            }
        }
        ...
        [
            "userid" => $pUserId,
            "password" => config('IC.BAJAJ_ALLIANZ.V1.BIKE.RENEWAL_PASSWORD'),
            "weomotpolicyin" => [
        ...
        $fetch_url = config('IC.BAJAJ_ALLIANZ.V1.BIKE.FETCH_RENEWAL');
        ...
        if (isset($response_data->errorcode) && $response_data->errorcode == 0) 
        {
            ...
            [
                ...
                // 'pTransactionId_inout' => !empty($proposal->proposal_no) && (config('constants.IcConstants.bajaj_allianz.IS_NEW_FLOW_ENABLED_FOR_CKYC') == 'Y') ? $proposal->proposal_no : '',
                "rcptlist"=> [[]]
                ,
                "custdetails"=> [
            ...
            $issue_policy_url = config('IC.BAJAJ_ALLIANZ.V1.BIKE.ISSUE_POLICY_URL_RENEWAL');
            ...
            else 
            {
                ...
                {
                    $source_name = config('IC.BAJAJ_ALLIANZ.V1.BIKE.PAYMENT_GATEWAY_SOURCE_NAME') ?? 'WS_MOTOR';
                    $url = config('IC.BAJAJ_ALLIANZ.V1.BIKE.PAYMENT_GATEWAY_URL') . '?requestId=' . $response_data->motextracoverout->extrafield2 . '&Username=' . $pUserId . '&sourceName='.$source_name;
                    UserProposal::where('user_product_journey_id', $enquiryId)
                    ->where('user_proposal_id', $proposal->user_proposal_id)
                    ->update([
                        'payment_url' => $url,
