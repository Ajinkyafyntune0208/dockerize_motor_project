...
class tataAigSubmitProposal
{
    ...
    public static function submit($proposal, $request)
    {
        ...
        $DepreciationReimbursement = $RoadsideAssistance = 'N';
        ...
        $zero_dep_age_limit = !$is_new
            ? date('d-m-Y', strtotime($requestData->previous_policy_expiry_date . ' - 28 days - 8 months - 3 year'))
            : date('d-m-Y');
        if (
            !$is_new
            &&
        ...
        $RoadsideAssistance    = ($zero_dep_age_available && $RoadsideAssistance == 'Y') ? 'Y' : 'N';
        $ReturnToInvoice        = ($zero_dep_age_available && $rti == 'Y') ? 'Y' : 'N';
        ...
        $input_array_info =    [
            ...
            "regno_2" => $is_new ? "" : (string)(explode('-', RtoCodeWithOrWithoutZero($requestData->rto_code, true))[1] ?? ($vehicle_register_no[1] ?? "")), //$vehicle_register_no[1] ?? '',
            ...
            'driver_declaration' => ($is_od ? 'ODD01' : $driver_declaration),
        ];
            ...
            "cover" => [
                ...
                "C3" => [
                    "opted" => (($is_individual && !$is_od) ? $cpa_cover : 'N'),
                     'tenure'  => ($is_new ? $pa_owner_tenure : ($is_individual ? ($cpa_cover == 'Y' ? '1' : '0') : '0'))
                ]
            ]
        ];
        // quick quote service input
        ...
        $inputArray = [
            ...
            'productid' => config('constants.IcConstants.tata_aig.bike.PRODUCT_ID'),
        ];
        ...
        $additional_data['method'] = 'Full Quote - Proposal';
        $get_response = getWsData(config('constants.IcConstants.tata_aig.END_POINT_URL_TATA_AIG_QUOTE'), $inputArray, 'tata_aig', $additional_data);
        $response = $get_response['response'];
        ...
        $proposal_input = [
            ...
            "sp_license" => "",
            "sp_place" => "",
            "productcod" => config('constants.IcConstants.tata_aig.bike.PRODUCT_CODE'),
            "customer" => [
        ...
        if($is_od){
            $proposal_input["bundpolicy"] = [
                ...
                "tp_pol_edate"  => Carbon::parse(date('Y-m-d',strtotime($tp_start_date.'+5 year -1 day')))->format('Ymd'),
            ];
        }
        ...
        $inputArray = [
            ...
            'T' => config('constants.IcConstants.tata_aig.TOKEN'),
            'product_code' => config('constants.IcConstants.tata_aig.bike.PRODUCT_CODE'),
            'THANKYOU_URL' => route('bike.payment-confirm', ['tata_aig']),
        ];

        // additional data
        $additional_data['method'] = 'Proposal Submition - Proposal';
        ...
        if ($response['data']['status'] == "1") {
            ...
            $total_od = ((isset($quote_response['data']['C1']) ? ((float)$quote_response['data']['C1']['premium']) : 0) 
                        + (isset($quote_response['data']['C4']) ? ((float)$quote_response['data']['C4']['premium']) : 0) 
                        + (isset($quote_response['data']['C5']) ? ((float)$quote_response['data']['C5']['premium']) : 0));

            $total_addon = ((isset($quote_response['data']['C35']) ? ((float)$quote_response['data']['C35']['premium']) : 0) 
                        + (isset($quote_response['data']['C37']) ? ((float)$quote_response['data']['C37']['premium']) : 0) 
                        + (isset($quote_response['data']['C38']) ? ((float)$quote_response['data']['C38']['premium']) : 0) 
                        + (isset($quote_response['data']['C47']) ? ((float)$quote_response['data']['C47']['rate']) : 0));
            
            $total_tp = isset($quote_response['data']['C2']) ? ((float)$quote_response['data']['NETTP']) : 0;

            $final_total_discount               = (isset($quote_response['data']['C15']) ? ((float)$quote_response['data']['C15']['premium']) : 0) + (isset($quote_response['data']['C11']) ? ((float)$quote_response['data']['C11']['premium']) : 0) + (isset($quote_response['data']['C10']) ? ((float)$quote_response['data']['C10']['premium']) : 0) + (isset($response['data']['C12']) ? ($response['data']['C12']['premium']) : 0);
            ...
            $proposal->od_premium               = $total_od - $final_total_discount + $total_addon;
            // $proposal->od_premium               = (isset($quote_response['data']['C1']) ? ((float)$quote_response['data']['TOTALOD']) : 0) - $final_total_discount;
            $proposal->tp_premium               = isset($quote_response['data']['C2']) ? ((float)$quote_response['data']['NETTP']) : 0;
            $proposal->cpa_premium              = isset($quote_response['data']['C3']) ? ((float)$quote_response['data']['C3']['premium']) : 0;
            $proposal->addon_premium            = (isset($quote_response['data']['NETADDON']) ? ((float)$quote_response['data']['NETADDON']) : 0) + (isset($quote_response['data']['C47']) ? ((float)$quote_response['data']['C47']['rate']) : 0);
            $proposal->ncb_discount             = isset($quote_response['data']['C15']) ? ((float)$quote_response['data']['C15']['premium']) : 0;
            $proposal->service_tax_amount       = (isset($quote_response['data']['TAX']) ? ((float)$quote_response['data']['TAX']['total_prem']) : 0) + ((float)isset($quote_response['data']['C47']) ? ((float)$quote_response['data']['C47']['rate']) * 0.18 : 0);
            $proposal->total_premium            = $total_od - $final_total_discount + $total_addon + $total_tp;
            // $proposal->total_premium            = (isset($quote_response['data']['NETPREM']) ? ((float)$quote_response['data']['NETPREM']) : 0);
            $proposal->discount_percent         = isset($quote_response['data']['discount']) ? ((float)$quote_response['data']['discount']['rate']) : 0;
