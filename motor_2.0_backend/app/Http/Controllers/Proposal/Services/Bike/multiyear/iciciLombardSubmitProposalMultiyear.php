...
namespace App\Http\Controllers\Proposal\Services\Bike\Multiyear;
...
class iciciLombardSubmitProposalMultiyear
{
    /**
     * @param $proposal
     * @param $request
    ...
    public static function submit($proposal, $request)
    {
        ...
        if (!empty($token)) 
        {
            ...
            if (isset($token['access_token'])) 
            {
                ...
                if ($requestData->business_type == 'rollover' &&  $productData->tenure == 22) {
                    $tenure_year = 2;
                    $model_config_premium['Tenure'] = 2;
                    $model_config_premium['TPTenure'] = 2;
                } elseif ($requestData->business_type == 'rollover' && $productData->tenure == 33) {
                    $tenure_year = 3;
                    $model_config_premium['Tenure'] = 3;
                    $model_config_premium['TPTenure'] = 3;
                }  elseif ($requestData->business_type == 'rollover' && $productData->tenure == 02) {
                    $tenure_year = 2;
                    $model_config_premium['Tenure'] = 0;
                    $model_config_premium['TPTenure'] = 2;
                } elseif ($requestData->business_type == 'rollover' && $productData->tenure == 03) {
                    $tenure_year = 3;
                    $model_config_premium['Tenure'] = 0;
                    $model_config_premium['TPTenure'] = 3;
                }
                ...
                $proposal_array['PolicyEndDate'] = $policy_end_date;
                if($IsPos == 'N')
                {
                    switch($premium_type)
                    {
                ...
                if (!empty($data)) 
                {
                    ...
                    if ($premiumResponse['status'] == 'Success') 
                    {
                        ...
                        if ($requestData->business_type == 'rollover' &&  $productData->tenure == 22) {
                            $tenure_year = 2;
                            $proposal_array['Tenure'] = 2;
                            $proposal_array['TPTenure'] = 2;
                        } elseif ($requestData->business_type == 'rollover' && $productData->tenure == 33) {
                            $tenure_year = 3;
                            $proposal_array['Tenure'] = 3;
                            $proposal_array['TPTenure'] = 3;        
                        }  elseif ($requestData->business_type == 'rollover' && $productData->tenure == 02) {
                            $tenure_year = 2;
                            $proposal_array['Tenure'] = 0;
                            $proposal_array['TPTenure'] = 2;
                        } elseif ($requestData->business_type == 'rollover' && $productData->tenure == 03) {
                            $tenure_year = 3;
                            $proposal_array['Tenure'] = 0;
                        ...
                        if (!empty($proposalServiceResponse)) {
                            ...
                            if ($arr_premium['status'] == 'Success') 
                            {
                                ...
                                $tyre_secure = round($arr_premium['riskDetails']['tyreProtect'] ?? 0);
