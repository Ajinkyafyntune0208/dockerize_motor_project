    ...
    public static function submit($proposal, $request)
    {
        ...
        $bifuel = 'false';
            ...
            if ($requestData->vehicle_owner_type == 'I'  && $premium_type != "own_damage") {
                if ($requestData->business_type == 'newbusiness')
                {
                    $cpa_year = !empty($cpa_year) ? $cpa_year : 5;
                }
                else
                {
        ...
        if ($data) {
            ...
            if (isset($quote_output['s:Body']['CreatePolicyResponse']['CreatePolicyResult']['Root']['Policy'])) 
            {
                ...
                if ($quote_output['Status'] == 'Fail') {
                    ...
                    $min_idv = ($total_idv * 0.9);      //changes as per kit #20121
                    ...
                    if($premium_type !== 'own_damage' && !(config('constants.IcConstants.future_generali.DISABLE_RSA_CALCULATION') == 'Y'))
                    {
                        $tax_for_addons = 1.18;
