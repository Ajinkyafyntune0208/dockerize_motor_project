...
function submitV1Gcv($proposal, $request )
{
    ...
    $additional_data['url'] = config('IC.SHRIRAM.V1.GCV.PROPOSAL_SUBMIT_URL'); //constants.IcConstants.shriram.SHRIRAM_PROPOSAL_GCV_SUBMIT_URL
    ...
    if ($response['MessageResult']['Result'] == 'Success') {
        ...
        if (($premium_type == 'breakin' || ($requestData->previous_policy_type == 'Third-party' && ! in_array($premium_type, ['third_party', 'third_party_breakin']))) && ($proposal->is_inspection_done == 'N' || $proposal->is_inspection_done == NULL))
        {
            ...
            $pre_inspection_request = [
                'BranchPartyId' => config('IC.SHRIRAM.V1.GCV.BREAKIN_BRANCH_PARTY_ID'), //constants.IcConstants.shriram.SHRIRAM_BREAKIN_BRANCH_PARTY_ID
                'SurveyorPartyId' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_BREAKIN_SURVEYOR_PARTY_ID'), //constants.IcConstants.shriram.SHRIRAM_BREAKIN_SURVEYOR_PARTY_ID
                'ProductType' => 'PCCV',
                ...
                'UserPartyId' => config('IC.SHRIRAM.V1.GCV.BREAKIN_USER_PARTY_ID'), //constants.IcConstants.shriram.SHRIRAM_BREAKIN_USER_PARTY_ID
                'SourceFrom' => config('IC.SHRIRAM.V1.GCV.BREAKIN_SOURCE_FROM'), //constants.IcConstants.shriram.SHRIRAM_BREAKIN_SOURCE_FROM
                'LoginId' => config('IC.SHRIRAM.V1.GCV.BREAKIN_LOGIN_ID'), //constants.IcConstants.shriram.SHRIRAM_BREAKIN_LOGIN_ID
       ...
       //constants.IcConstants.shriram.SHRIRAM_BREAKIN_ID_GENERATION_URL
            $get_response = getWsData(config('IC.SHRIRAM.V1.GCV.BREAKIN_ID_GENERATION_URL'), $pre_inspection_request, $request['companyAlias'], [
                'enquiryId' => customDecrypt($request['userProductJourneyId']),
                'headers' => [
                    'Username' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_USERNAME'),//constants.IcConstants.shriram.SHRIRAMGCV_USERNAME
                    'Password' => config('IC.SHRIRAM.V1.GCV.SHRIRAM_PASSWORD'), //constants.IcConstants.shriram.SHRIRAM_PASSWORD
