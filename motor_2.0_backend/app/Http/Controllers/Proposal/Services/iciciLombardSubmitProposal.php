...
use App\Http\Controllers\Proposal\Services\iciciLombardSubmitProposalMisd;
...
class iciciLombardSubmitProposal
{
    ...
    { 
        ...
        if($is_MISC == 3){
            return iciciLombardSubmitProposalMisd::submit($proposal, $request);
        }
        ...
        if ($master_product_sub_type_code == 'PICK UP/DELIVERY/REFRIGERATED VAN' || $master_product_sub_type_code == 'DUMPER/TIPPER' ||$master_product_sub_type_code == 'TRUCK' ||$master_product_sub_type_code == 'TRACTOR' ||$master_product_sub_type_code == 'TANKER/BULKER') {
            ...
            $type = 'MISC';
            if ($premium_type == 'third_party'|| $premium_type == 'third_party_breakin') {
                $ICICI_LOMBARD_DEAL_ID = config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_DEAL_ID_TP'); #TP Deal for misc
...
//            }
            ...
            $corelationId = getUUID($request['userProductJourneyId']);

            $proposerVehDet = CorporateVehiclesQuotesRequest::where('user_product_journey_id', $request['userProductJourneyId'])
                ->select('*')
                ->get();
            ...
            if($master_product_sub_type_code == 'TAXI' || $master_product_sub_type_code == 'ELECTRIC-RICKSHAW' || $master_product_sub_type_code == 'AUTO-RICKSHAW'){
                ...
                $rto_location_code = DB::table('gcv_icici_lombard_rto_master')
                                ->where('ILStateCode', $state_code)
                                ->where('CityDistrictCode', $city_district_code)
                                ->first();
                $rto_location_code = keysToLower($rto_location_code);
            ...
            if ($type !== 'MISC') {
                ...
                if(isset($mmv->model_build))
                {
                    $model_build = $mmv->model_build;
                }else{
                    $model_build = '';
                }

                $mmvDetails['manf_name']        = $mmv->manf_name;
                $mmvDetails['model_name']       = $mmv->model_name;
                $mmvDetails['version_name']     = $mmv->model_name;
                $mmvDetails['version_id']       = $mmv->model_code;
                $mmvDetails['seating_capacity'] = $mmv->seating_capacity;
                $mmvDetails['cubic_capacity']   = $mmv->cubic_capacity;
                $mmvDetails['fuel_type']        = $mmv->fuel_type;
            }else{
                $model_build = ''; #remove this
            }
            ...
            $selectedAddons = SelectedAddons::where('user_product_journey_id', $request['userProductJourneyId'])
                ->select('*')
                ->get();
            ...
            // $chassisPrice = QuoteLog::where('user_product_journey_id', $request['userProductJourneyId'])->pluck('ex_showroom_price_idv')->first();
            $vehiclebodyPrice = QuoteLog::where('user_product_journey_id', $request['userProductJourneyId'])->pluck('ex_showroom_price_idv')->first();
            ...
            $premiumRecalcRequest = [
                ...
                "RTOLocationCode" => $rtoLocationCode, //$rtoLocationCode[0]->rto_location_code,
                "VehicleMakeCode" =>  $type == 'MISC' ? '366' : $mmv->manf_code,//$manfCode[0]->manf_code,
                "VehicleModelCode" => $type == 'MISC' ? '6338' : $mmv->ic_version_code,//$modelCode[0]->ic_version_code,
                    ...
                    if (!empty($proposalServiceResponse)) {
                        ...
                        if (isset($proposalServiceResponseData['statusMessage']) && $proposalServiceResponseData['statusMessage'] == 'SUCCESS') {
                           ...
                           $mmvDet = MotorModelVersion::where('version_id', $requestData->version_id)
                            ...
                            if($type !== 'MISC'){
                                $vehDetails['manf_name'] = $mmv->manf_name;
                                $vehDetails['model_name'] = $mmv->model_name;
                                $vehDetails['version_name'] = $mmv->model_name;
                                $vehDetails['version_id'] = $mmv->model_code;
                                $vehDetails['seating_capacity'] = $mmv->seating_capacity;
                                $vehDetails['cubic_capacity'] = $mmv->cubic_capacity;
                                $vehDetails['fuel_type'] = $mmv->fuel_type;
                                $vehDetails['vehicle_type'] = 'PCV';
                            }else{ #remove this
                            ...
                            if (isset($proposalServiceResponseData['breakingFlag']) && $proposalServiceResponseData['breakingFlag'] == 'true' && in_array($premium_type, ['breakin', 'short_term_3_breakin', 'short_term_6_breakin'])) {
                                ...
                                $brekinIdRequestArray = [
                                    ...
                                    "VehicleMake" => $mmv->manf_name,
                                    "VehicleModel" =>  $mmv->model_name,
                                    ...
                                    "DealId" =>  $type == 'GCV' ? config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_DEAL_ID_GCV_BREAKIN') :config('constants.IcConstants.icici_lombard.ICICI_LOMBARD_DEAL_ID_BREAKIN'), //$ICICI_LOMBARD_DEAL_ID,
                                    "IsPOSDealId" => false,
                                    "CorrelationId" => $corelationId,
                                    "selfInspection" => 'Yes'
                                ];
