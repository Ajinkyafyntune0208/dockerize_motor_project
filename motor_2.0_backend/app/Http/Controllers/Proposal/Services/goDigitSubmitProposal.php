...
use App\Http\Controllers\Proposal\Services\Pcv\V2\GoDigitSubmitProposal as oneapi;
...
class goDigitSubmitProposal
{
    public static function submit($proposal, $request)
    {
        if (config('IC.GODIGIT.V2.CV.ENABLE') == 'Y') {
            return oneapi::oneApiSubmit($proposal, $request);
        }
        ...
        if($is_MISC == 3){
            return goDigitSubmitProposal_Misc::submit($proposal, $request);
        }      
        ...
        if(!empty($additional['additional_covers']))
        {
            ...
            $total_covers_amt = round($cover_pa_paid_driver + $cover_pa_unnamed_passenger);
        }
        ...
        $isConsumables = $consumablesAvailable;
        ...
        if (!empty($data)) 
        {
            ...
            { 
                ...
                [
                    ...
                    "motorQuestions" => [
                        "furtherAgreement" => "",
                        "selfInspection" => config('constants.motor.IS_WIMWISURE_GODIGIT_ENABLED') == 'Y' ? false : (isset($self_inspection) ? $self_inspection : false),
                ...
                if ($mmv->vehicle_type != 'Passenger Carrying') {
                    $proposal_data['contract']['coverages']['isIMT23'] = $is_imt_23;
                }
                ...
                [
                    ...
                    'productName'  => $productData->product_name,
                    'company'  => 'godigit',
                    'method'   => 'Proposal Submit' . ($productData->product_identifier == 'IMT23' ? ' - IMT-23' : ''),
                    'section' => $productData->product_sub_type_code,
                    'webUserId' => $webUserId,
                    'password' => $password,
                    'transaction_type' => 'proposal',
                ...
                if(!empty($data))
                {
                    ...
                    {        
                        ...
                        $imt23 = 0;
                        // $imt23 = $mmv->vehicle_type == 'Passenger Carrying' ? 0 : round($od - ($od * (100/115))); // imt23 will be calculated in IMT23 product in basic_od as inbuilt
                        ...
                        // $final_od_premium = $od - $imt23 - $final_total_discount;
                        ...
                        $final_gst_amount = round(str_replace("INR ", "", $proposal_response->serviceTax->totalTax)); // 18% IC 
