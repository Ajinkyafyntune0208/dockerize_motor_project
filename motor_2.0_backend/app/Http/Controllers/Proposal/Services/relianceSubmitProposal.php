...
use App\Models\ReliancePincodeStateCityMaster;
...
class relianceSubmitProposal
{
    ...
    public static function submit($proposal, $request)
    {
        ...
        if (trim($response->ErrorMessages) == '') {
            ...
            if ((in_array($premium_type, ['breakin', 'short_term_3_breakin', 'short_term_6_breakin']) || ($requestData->previous_policy_type == 'Third-party' && ! in_array($premium_type, ['third_party', 'third_party_breakin']))) && ($proposal->is_inspection_done == 'N' || $proposal->is_inspection_done == NULL) && !empty($response->InspectionErrorMessage))
            {
                $is_breakin = 'Y';
                ...
                $inspection_type = strtoupper($proposal->inspection_type);

                if($inspection_type == 'MANUAL'){
                    $inspection_address_data = [
                        'address' => $additional_details->vehicle->inspectionAddress,
                        'address_1_limit'   => 70,
                        'address_2_limit'   => 70            
                    ];
                    $getInspectionAddress = getAddress($inspection_address_data);
    
                    $inspection_corres_address_data = ReliancePincodeStateCityMaster::where('pincode', $additional_details->vehicle->inspectionPincode)
                        ->select('*')
                        ->first();
                }
                $lead_array = [
                    'LEADDESCRIPTION' => [
                        ...
                        'VENDORCODE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_VENDORCODE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_VENDORCODE'),
                        'VENDORCODEVALUE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_VENDORCODEVALUE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_VENDORCODEVALUE'),
                        'BASCODE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASCODE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASCODE'),
                        'BASCODEVALUE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASCODEVALUE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASCODEVALUE'),
                        'BASMOBILE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASMOBILE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_BASMOBILE'),
                        'SMCODE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMCODE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMCODE'),
                        'SMCODEVALUE' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMCODEVALUE_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMCODEVALUE'),
                        'SMMOBILENO' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMMOBILENO_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_SMMOBILENO'),
                        ...
                        'VECH_INSP_ADDRESS' => $inspection_type == 'MANUAL' ? trim($getInspectionAddress['address_1']) : trim($getAddress['address_1']),
                        'VECH_INSP_ADDRESS2' => $inspection_type == 'MANUAL' ? trim($getInspectionAddress['address_2']) : trim($getAddress['address_2']),
                        'VECH_INSP_ADDRESS3' => $inspection_type == 'MANUAL' ? trim($getInspectionAddress['address_3']) : trim($getAddress['address_3']),
                        'REMARK' => 'GZB',
                        'LEADTYPE' => 'F',
                        'STATE_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->state_id_pk : $corres_address_data->state_id_pk,
                        'STATEVALUE_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->state_name : $corres_address_data->state_name,
                        'DISTRICT_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->district_id_pk : $corres_address_data->district_id_pk,
                        'DISTRICTVALUE_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->district_name : $corres_address_data->district_name,
                        'CITY_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->city_or_village_id_pk : $corres_address_data->city_or_village_id_pk,
                        'CITYVALUE_VEH' => $inspection_type == 'MANUAL' ? $inspection_corres_address_data->city_or_village_name : $corres_address_data->city_or_village_name,
                        'PINCODE_VEH' => $inspection_type == 'MANUAL' ? $additional_details->vehicle->inspectionPincode : $proposal->pincode,
                        ...
                        'LEADCREATEDBYSYSTEM' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_LEADCREATEDBYSYSTEM_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_LEADCREATEDBYSYSTEM'),
                        'INTIMATORNAME' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_INTIMATORNAME_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_INTIMATORNAME'),
                        'INTIMATORMOBILENO' => $inspection_type == 'MANUAL' ? config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_INTIMATORMOBILENO_MANUAL') : config('constants.IcConstants.reliance.RELIANCE_MOTOR_LEAD_INTIMATORMOBILENO'),
                ...
                if($inspection_type == 'SELF' && $parent_id == 'PCV'){
                    $lead_array['LEADDESCRIPTION']['INSPECTIONTOBEDONE'] = '19';
                    $lead_array['LEADDESCRIPTION']['INSPECTIONTOBEDONEVALUE'] = 'Self Inspection';
                }
                if($inspection_type == 'MANUAL' && $parent_id == 'GCV'){
                    return [
                        'status' => false,
                        'webservice_id' => $get_response['webservice_id'],
                        'table' => $get_response['table'],
                        'message' => 'Agency/Manual Inspection is not allowed in GCV.'
                    ];
                }
                ...
                $get_response = getWsData(config('constants.IcConstants.reliance.END_POINT_URL_RELIANCE_MOTOR_LEAD'),
                    $lead_array,
                    'reliance',
                    [
                        'section' => $productData->product_sub_type_code,
    ...
    public static function getPremiumRequestData($data)
    {
        ...
        } else if ($mmv_data->gross_weight < 3500 && $parent_id == 'GCV' && $requestData->business_type == 'breakin') {
            $PreviousNCB = $NCB_ID[$requestData->previous_ncb];
            $IsClaimedLastYear = 'true';
            $IsNCBApplicable = 'false';
            $NCBEligibilityCriteria = '1';
