...
namespace App\Http\Controllers\Proposal\Services\V2\PCV;
...
class tataAigSubmitProposals {
    ...
    public static function getTataAigV2CvCreds()
    {
        ...
        return (object)[
            'scope' => config('IC.TATA_AIG.V2.PCV.SCOPE'), 
            'client_id' => config('IC.TATA_AIG.V2.PCV.CLIENT_ID'), 
            'client_secret' => config('IC.TATA_AIG.V2.PCV.CLIENT_SECRET'), 
            'api_key' => config('IC.TATA_AIG.V2.PCV.XAPI_KEY'), 
        ];
    }
    ...
    public static function getToken($enquiryId, $productData, $transaction_type = 'proposal')
    {
        ...
        $tokenRequest = [
            'grant_type'    => config('IC.TATA_AIG.V2.PCV.GRANT_TYPE'), //constants.IcConstants.tata_aig_v2.cv.TATA_AIG_V2_GRANT_TYPE
            'scope'         => $configCreds->scope,
            'client_id'     => $configCreds->client_id,
            'client_secret' => $configCreds->client_secret,
        ];

        $get_response = getWsData(config('IC.TATA_AIG.V2.PCV.END_POINT_URL_TOKEN'), $tokenRequest, 'tata_aig_v2', $additional_data); //constants.IcConstants.tata_aig_v2.cv.TATA_AIG_V2_END_POINT_URL_TOKEN
...
public static function submitPcv($proposal, $request)
    {
        ...
        if (config('IC.TATA_AIG.V2.PCV.MMV_CHANGE_TO_UAT') == 'Y') 
         {
        ...
        $vehicle_register_no = explode("-", splitAndFormatRegistrationNumber($proposal->vehicale_registration_number));
        ...
        // removed age validation #30076, #31459
        ...
        if ($is_pos == 'Y' && isset($pos_data->seller_type) && $pos_data->seller_type == 'P' && $quote_log_data->idv <= 5000000) {
          ...
          } elseif (config('IC.TATA_AIG.V2.PCV.TESTING_MODE_ENABLE') == 'Y')  
          {
            $is_posp = 'Y';
            $sol_id     = '55554'; //'840372';
        ...
        $quoteRequest = [
            ...
            "product_id" => config("IC.TATA_AIG.V2.PCV.PCV_PRODUCT_ID"),
    ...
    if($premium_type =="short_term_3_breakin")
    {
        ...
        if($productData->product_identifier == "zerodept_withaddons")
        {
            ...
            $quoteRequest['consumbale_expense'] = $applicableAddon['ConsumablesExpenses'];
            $quoteRequest['rsa'] = $applicableAddon['RoadsideAssistance'] ;
        }
        if($requestData->business_type =="newbusiness"  &&  $premium_type == "comprehensive")
        {
            
            $quoteRequest['q_producer_code'] = config('IC.TATA_AIG.V2.PCV.PRODUCER_CODE_ANNUAL');
        ...
        if (!$is_od) {
            ...
            $quoteRequest['ll_pass'] = $quoteRequest['pa_paid'] = 'No';
      ...
      #inception approved response check quote  
       ...
       if (($pol["inspectionOverride"] == true) ?? '') { 
        ...
        //Blocking Previous Insurer renewal for one year policies.
        if(config('DISABLING_TATA_RENEWAL_FOR_COMP') == 'Y'){
            if($any_short_term == false){
                if($proposalRequest['pre_insurer_name'] === 'TAGIC' || $proposalRequest['pre_insurer_code'] === 'TAGIC'){
                    return response()->json([
                        'status' => false,
        ...
        $get_response = getWsData(config('IC.TATA_AIG.V2.PCV.SHORT_TERM_END_POINT_URL_PROPOSAL'), $proposalRequest, 'tata_aig_v2', $additional_data);//constants.IcConstants.tata_aig_v2.cv.SHORT_TERM_END_POINT_URL_PROPOSAL
        ...
        if (!isset($proposalResponse['status']) || $proposalResponse['status'] != 200) {
            ...
            if ($isBreakinInspectionRequired && !(config('IC.TATA_AIG.V2.PCV.TATA_AIG_V2_PCV_INSPECTION_ENABLED') == 'Y')) 
             {
                return [
                    'status'    => false,
            ...
            {
                $proposalSubmitResponse['data']['verification_status'] = true;
            }
            return response()->json($proposalSubmitResponse);
        }
        return response()->json([
            'status' => false,
