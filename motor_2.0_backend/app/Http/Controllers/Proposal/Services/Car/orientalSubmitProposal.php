...
use App\Http\Controllers\SyncPremiumDetail\Car\OrientalPremiumDetailController;
...
class orientalSubmitProposal
{
    ...
    {   
        ...
        if (!$rto_location) {
            return [
                'status' => false,
                'message' => 'Rto location not found'
            ];
        }
        ...
        $ElectricalaccessSI = $RSACover = $PAforUnnamedPassengerSI = $nilDepreciationCover = $antitheft = $Electricalaccess = $NonElectricalaccess = $NonElectricalaccessSI = $PAforUnnamedPassenger = $PAPaidDriverConductorCleaner = $PAPaidDriverConductorCleanerSI = $externalCNGKIT = $externalCNGKITSI = $LimitedTPPDYN = 0;
        ...
        if ($requestData->business_type == 'newbusiness') {
            ...
            $policy_end_date =  $premium_type == 'comprehensive' ? Carbon::parse($policy_start_date)->addYear(1)->subDay(1)->format('d-M-Y') :Carbon::parse($policy_start_date)->addYear(3)->subDay(1)->format('d-M-Y') ;
            ...
            {
                $PreviousPolicyFromDt = ($master_policy->premium_type_id != '3') ? Carbon::parse(str_replace('/', '-', $proposal->prev_policy_expiry_date))->subYear(1)->addDay(1)->format('d-M-Y') : Carbon::parse($requestData->previous_policy_expiry_date)->subYear(1)->addDay(1)->format('d-M-Y');
                $PreviousPolicyToDt = ($master_policy->premium_type_id != '3') ? Carbon::parse(str_replace('/', '-', $proposal->prev_policy_expiry_date))->format('d-M-Y') :Carbon::parse($PreviousPolicyFromDt)->addYear(3)->subDay(1)->format('d-M-Y');
            }
        ...
        $break_in = "NO";
        if(!empty($proposal->prev_policy_expiry_date) && !empty($policy_start_date) && ($proposal->prev_policy_expiry_date !== 'NEW'))
        {
        ...
        foreach ($addons as $key => $value) {
            ...
            if ((in_array('Key Replacement', $value)) && !(($interval->y > 5) || ($interval->y == 5 && $interval->m > 0) || ($interval->y == 5 && $interval->m == 0 && $interval->d > 0) || ($interval->y == 5))) {
                $KeyReplacementYN = 'Y';
            }
            ...
            if (in_array('Consumable', $value) && !(($interval->y > 5) || ($interval->y == 5 && $interval->m > 0) || ($interval->y == 5 && $interval->m == 0 && $interval->d > 0) || ($interval->y == 5))) {
                $consumable = 'Y';
            }
            ...
            if (in_array('Tyre Secure', $value)) {
                $tyre_secure = 'MOT-SMI-018~' . $quote_log->idv;
            }
        }

        if(!$is_liability && $InvReturnYN == '1')
        {
            if(($interval->y <= 2) || ($interval->y == 2 && $interval->m <= 11) || ($interval->y == 2 && $interval->m == 11 && $interval->d <= 26))
            {
                $InvReturnYN = '1';
            }
            else
        ...
        $engine_secure_age_limit = strtotime((!$is_new && !$is_liability) ? date('d-m-Y', strtotime($requestData->previous_policy_expiry_date . ' - 28 days - 11 months - 9 year')) : date('d-m-Y'));
        if (!$is_new && $engine_secure_age_limit >= strtotime($requestData->vehicle_register_date))
        {
            $engine_secure_age_available = false;
        }

        $engine_protection = ($engine_protection == 'Y' && $engine_secure_age_available ? 'Y' : 'N');
        ...
        if($is_new)
        {
            ...
            {
                $vehicale_registration_no = 'NEW-1234';
            }
            else
            {
                $vehicale_registration_no = 'NEW-'.
                    $vehicale_registration_number[1].'-'.
                    $vehicale_registration_number[2].'-'.
                    $vehicale_registration_number[3];
            }
        }
        else
        {
        ...
        */
                  ...
                  [
                    ...
                    [
                      ...
                      [
                          ...
                          'TEL_NO' => '',
                          'FAX_NO' => '',
                          'ROAD_TRANSPORT_YN' => '0',//$RSACover, //addon rsa
                          ...
                          'FIBRE_TANK_VALUE' => '0',
                          'ALT_CAR_BENEFIT' => '',
                          'PERS_EFF_COVER' => (($LossOfPersonBelongYN == 'Y') ? 'LPE_02' : 'LPE_00'), //Loss of personal belongings
                          ...
                          'RETN_TO_INVOICE' => (($interval->y > 2) || ($interval->y == 2 && $interval->m > 11) || ($interval->y == 2 && $interval->m == 11 && $interval->d > 28)) ? '0' : $InvReturnYN,#($car_age < 3) ? '1' : '0', //Return to invoice
                          ...
                          'HYPO_COMP_PINCODE' => '',
                          'PAYMENT_TYPE' => 'OT',
                          'NCB_PERCENTAGE' => ($requestData->is_claim == 'Y') ? '' :$requestData->previous_ncb,    //previous policy
                          ...
                          'LPE_01' => (($LossOfPersonBelongYN == 'Y') ? '10000' : ''), //Loss of personal belongings
                          ...
                          'FLEX_06' => '',
                          'FLEX_07' => '',
                          'FLEX_08' => !empty($proposal->gst_number) ? $proposal->gst_number : '',//GSTNO
                          'FLEX_09' => '', //towing
                          'FLEX_10' => '',
                          'FLEX_19' => $posp_code,
                          'FLEX_20' => ($cpa_selected == '1') ? 'N':'Y',
                          'FLEX_12' => (($nilDepreciationCover == '1' && $car_age < 5) ? '20' : ''),
                          'FLEX_21' => $engine_protection, //engine protector
                          ...
                          'FLEX_25' => $KeyReplacementYN . '~' . $consumable, //Key replacement & consumables
                      ],
                    '_attributes' => [
        ...
        if ((($productData->product_identifier == 'engine_protector') || $engine_protection == 'Y') && $engine_secure_age_available) {
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_21'] = 'Y';
        } else {
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_21'] = 'N';
        }
        if($productData->product_identifier == 'without_addon')
        {
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_12'] = 'N';
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_21'] = 'N';
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_25'] = 'N~N';
        }
        if (($externalCNGKITSI != '0')) {
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['CNG_KIT_VALUE'] = $externalCNGKITSI;
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_04'] = 'Y';
        }
        if (($internalCNG == '1'))
        {
            $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_04'] = 'Y';
        }
        $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['FLEX_32'] =  (($premium_type != 'third_party') && !(($interval->y > 5) || ($interval->y == 5 && $interval->m > 0) || ($interval->y == 5 && $interval->m == 0 && $interval->d > 0) || ($interval->y == 5))) ? $tyre_secure : '';
            ...
            {
                $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['PERS_EFF_COVER'] = 'LPE_00';
                $proposal_array['soap:Body'][$soapAction]['objGetQuoteMotorETT']['LPE_01'] = '0';
            }
        ...
        $vehicleDetails = [
            'manufacture_name'  => $mmv_data->manf_name,
            'model_name'        => $mmv_data->model_name,
            'version'           => $mmv_data->varient_name,
        ...
        if ($quote_res_array['ERROR_CODE'] == '0 0') {
            ...
            $addon_premium =  $Depreciation_Reimbursement + $res_array['KEYREPLACEMENT'] + $res_array['LOSSPER_BELONG'] + $res_array['ENG_PRCT'] + $res_array['RTI'] + $res_array['TYRE_SECURE'];
            $od_premium = (!$is_liability) ? (($res_array['OD_PREMIUM']) + ($res_array['ELEC']) + ($res_array['CNG']) + ($res_array['OTHER_FUEL1']) + $geog_Extension_OD_Premium) :'0';
            $tp_premium = (!$is_od) ? (($res_array['TP_PREMIUM']) + ($res_array['LL_PAID_DRIVER']) + ($res_array['UNNAMED_PASSENGER']) + ($res_array['CNG_TP']) + ($res_array['OTHER_FUEL2']) + $geog_Extension_TP_Premium):'0';
            ...
            //         'od_premium'            => $od_premium - ($res_array['NCB_DIS']),
            ...
            //         'total_discount'        => (($res_array['NCB_DIS'])),
        ...
        $save = UserProposal::where('user_proposal_id', $proposal->user_proposal_id)->update($updateData);
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'finalPayableAmount' => ($res_array['ANNUAL_PREMIUM']),
                    'is_breakin' => '',
                    'inspection_number' => '',
