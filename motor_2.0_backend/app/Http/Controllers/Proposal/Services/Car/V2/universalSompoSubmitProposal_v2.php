...
class universalSompoSubmitProposal_v2
{
    /**
     * @param $proposal
     * @param $request
    ...
    public static function submit_v2($proposal, $request, $step='')
    {
        ...
        if(strtolower($mmv_data->make) != 'honda' && strtolower($mmv_data->make) != 'tata' && strtolower($mmv_data->make) != 'maruti' && strtolower($mmv_data->make) != 'maruti suzuki')
        {
...
/*             $expdate = $requestData->previous_policy_expiry_date == 'New' ? date('Y-m-d') : $requestData->previous_policy_expiry_date;
            ...
            $get_response = getWsData(config('IC.UNIVERSAL_SOMPO.V2.CAR.END_POINT_URL'), $xmlQuoteRequest, 'universal_sompo', $additionData);
