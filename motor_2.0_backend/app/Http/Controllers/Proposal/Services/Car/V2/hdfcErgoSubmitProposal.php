...
class hdfcErgoSubmitProposal
{
                ...
                if (isset($arr_premium['StatusCode']) && $arr_premium['StatusCode'] == '200') {
                    ...
                    UserProposal::where('user_product_journey_id', $enquiryId)
                        ...
                        ->update([
                            ...
                            'service_tax_amount' => round($premium_data['Service_Tax']),
            ...
            if (isset($token_data['Authentication']['Token'])) {
                ...
                if (isset($arr_premium['StatusCode']) && $arr_premium['StatusCode'] == '200') {
                    ...
                    $Nil_dep = $pa_unnamed = $ncb_discount = $liabilities = $pa_paid_driver = $pa_owner_driver = $electrical = $non_electrical = $age_discount = $lpg_cng_tp = $lpg_cng = $Bonus_Discount = $automobile_discount = $antitheft
                        ...
                        $key_replacement = $loss_of_personal_belongings = $eng_protector =
                        $rti = $incon_allow = $Basic_OD_Discount = $electrical_Discount =
                        $non_electrical_Discount = $lpg_od_premium_Discount = $tyre_secure = $GeogExtension_od= $GeogExtension_tp=$OwnPremises_OD= $OwnPremises_TP = $legal_liability_to_employee = 0;
    ...
    public static function submitV2($proposal, $request)
    {
        ...
        $address2 = $getAddress['address_2'];
        $address3 = $getAddress['address_3'];

        if (strlen($getAddress['address_1']) < 10) {
            $address1 = $getAddress['address_1'] ." ". $proposal->state ." ". $proposal->city." ". $proposal->pincode;
        }
        if (strlen($getAddress['address_2']) < 10) {
            $address2 = $getAddress['address_2'] ." ". $proposal->state ." ". $proposal->city ." ". $proposal->pincode;
        }
        if (strlen($getAddress['address_3']) < 10) {
            $address3 = $getAddress['address_3'] ." ". $proposal->state ." ". $proposal->city ." ". $proposal->pincode;
        }
            ...
            if (isset($premium_response['Status']) && $premium_response['Status'] == 200)
            {
                $proposal_request = [
                    ...
                    'ProposalDetails' => [
                        ...
                        'PolicyType' => $tp_only ? 'ThirdParty' : $policy_type,
                        'CustomerType' => $requestData->vehicle_owner_type == 'I' ? "INDIVIDUAL" : "CORPORATE",
                        'IsEssentialZD' => $productData->product_identifier == 'Essential ZD'  ? "true" : "false",
                        'Isessentialegp' => $productData->product_identifier == 'Essential_EGP'  ? "true" : "false",
                        'VehicleMakeCode' => $mmv_data->manufacturer_code,
                        'VehicleModelCode' => $mmv_data->vehicle_model_code,
                        'RtoLocationCode' => $rto_data->rto_location_code,
                        'RequiredIDV' => $quote_log_data->idv,
                    ...
                    'CustomerDetails' => [
                        ...
                        'LGCode' => '',//"LGCODE123",
                        'CorrespondenceAddress1' => $address1,//trim($getAddress['address_1']),#$proposal->address_line1,
                        'CorrespondenceAddress2' => $address2,//trim($getAddress['address_2']) != '' ? trim($getAddress['address_2']) : '.',#$proposal->address_line2,
                        'CorrespondenceAddress3' => $address3,//trim($getAddress['address_3']),#$proposal->address_line3,
...
// }';
    ...
    if($policy_data_response['Status'] == 200)
    {
        ...
        $address = HdfcErgoMotorPincodeMaster::where('num_pincode', $proposal->pincode)->first();
        $address_data = [
            'address' => removeSpecialCharactersFromString($proposal->address_line1, true),
            'address_1_limit'   => 50,
            'address_2_limit'   => 50,         
        ...
        $proposal_input_data = [
            ...
            'CustomerDetails' => [
              ...
              //'LGCode'          => 'LGCODE123',
              'CorrespondenceAddress1' => trim($getAddress['address_1']),
              'CorrespondenceAddress2' => trim($getAddress['address_2']) != '' ? trim($getAddress['address_2']) : '.',
              'CorrespondenceAddress3' => trim($getAddress['address_3']),
