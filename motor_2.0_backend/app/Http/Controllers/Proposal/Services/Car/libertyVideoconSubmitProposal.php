...
class libertyVideoconSubmitProposal
{
    public static function submit($proposal, $request)
    {
        ...
        $is_breakin = (
            ...
            $vehicleDate  = !empty($requestData->vehicle_invoice_date) ?  $vehicle_invoice_date:  $requestData->vehicle_register_date;
            $vehicleDate = $vehicleDate->format('Y-m-d');

            $date1 = !empty($requestData->vehicle_invoice_date) ? new \DateTime($requestData->vehicle_invoice_date) : ($registration_date);
        ...
        $is_bh = (isBhSeries($proposal->vehicale_registration_number));
        if($is_bh){
            $proposal_request['isBHSeries'] = true;
            $proposal_request['BHSeriesNumber'] = str_replace('-', '', $proposal->vehicale_registration_number);
            $proposal_request['RegNo1'] = '';
            $proposal_request['RegNo2'] = '';
            $proposal_request['RegNo3'] = '';
            $proposal_request['RegNo4'] = '';
        ...
        if ($data) {
            ...
            if (trim($proposal_response['ErrorText']) == "") {
                ...
                UserProposal::where('user_proposal_id' , $proposal->user_proposal_id)
                    ...
                    'tp_end_date' => $requestData->business_type == 'newbusiness'  ? date('d-m-Y', strtotime('+3 year -1 day', strtotime($policy_start_date))):   date('d-m-Y', strtotime('+1 year -1 day', strtotime($policy_start_date))),
    ...
    public static function leadCreation($proposal, $mmv, $request_details, $productData, $requestData){
        $lead_request = [
            ...
            "ChannelType"       => "",
            "FeestobeBornBy"    => "",
            "SalesManager"      => config('IC.LIBERTY_VIDEOCON.CAR.SALESMANAGER'),
        ];
