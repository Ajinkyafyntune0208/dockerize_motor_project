...
if (config("IC.GODIGIT.CAR.CVWEBSERVICEHELPER") == "Y") {
...
use App\Http\Controllers\Proposal\Services\Car\V2\GoDigitSubmitProposal as oneapi;
...
class goDigitSubmitProposal
{
    public static function submit($proposal, $request)
    {
        ...
        $voluntary_deductible_amount = 'ZERO';
        if (!empty($proposal->previous_insurance_company)) {
        ...
        if ($premium_type == "own_damage") {
            $quote_premium_request['previousInsurer']['originalPreviousPolicyType'] = "1OD_3TP";
        ...
        if (strtoupper($requestData->previous_policy_type) == 'NOT SURE') {
            $quote_premium_request['previousInsurer']['isPreviousInsurerKnown'] = 'false';
        ...
        {
            return self::submitGdd($proposal, $requestData, $productData, [
        ...
        $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_QUICK_QUOTE_PREMIUM'),$quote_premium_request, 'godigit',
        [
        ...
        if (!empty($data))  {
            ...
            if (isset($response->error) && isset($response->error->errorCode) && $response->error->errorCode == '0')  { 
                ...
                if ($premium_type == "own_damage") {
                    $proposal_data['previousInsurer']['originalPreviousPolicyType'] = "1OD_3TP";
                ...
                if (strtoupper($requestData->previous_policy_type) == 'NOT SURE') {
                    $proposal_data['previousInsurer']['isPreviousInsurerKnown'] = 'false';
                ...
                $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_CREATE_QUOTE_PROPOSAL'),$proposal_data, 'godigit',
                [
                    'enquiryId' => $proposal->user_product_journey_id,
                    'requestMethod' =>'post',
                ...
                if (!empty($data)) {
                    ...
                    if ($proposal_response->error->errorCode == '0') {        
                        ...
                        $update_proposal=[
                            ...
                            GodigitPremiumDetailController::savePremiumDetails($get_response['webservice_id']);
    ...
    public static function submitGdd($proposal, $requestData, $productData, $additional_data)
    {
        if ((config('constants.IS_CKYC_ENABLED') == 'Y') && (config('constants.IS_CKYC_ENABLED_GODIGIT') === 'Y') && (config('DIGIT_STATUS_CHECK_BEFORE_PROCEEDING') == 'Y') &&  !empty($proposal->proposal_no ?? '')) {
            if ($proposal->is_ckyc_verified == 'Y') {
                $responseArray = [
        ...
        $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_GDD_QUICK_QUOTE_FIRST_CALL'), $premium_req_array, 'godigit', [
        ...
        if ($premium_response)
        {
            $premium_request_data = json_decode($premium_response, TRUE);
            ...
            if (isset($premium_request_data['error']['errorCode']) && $premium_request_data['error']['errorCode'] == 0)
            {
                ...
                $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_GDD_QUICK_QUOTE_RECALCULATE'), $premium_request_data, 'godigit', [
                    'enquiryId' => $proposal->user_product_journey_id,
                    'requestMethod' =>'post',
                    'productName'  => $productData->product_name,
                    'company'  => 'godigit',
                ...
                {
                    $proposal_request_data = json_decode($premium_response_data, TRUE);
                    ...
                    if (isset($proposal_request_data['error']['errorCode']) && $proposal_request_data['error']['errorCode'] == 0)
                    {
                        ...
                        $selectedAddons = SelectedAddons::where('user_product_journey_id', $proposal->user_product_journey_id)
                        ->select('compulsory_personal_accident')
                        ->first();
                        
                        $cpaSelected = array_search(['name' => 'Compulsory Personal Accident'], $selectedAddons['compulsory_personal_accident']) !== false;
                        // if ($requestData->vehicle_owner_type == 'I' && $coverages['accessories']['cng']['selection'] == 'true')
                        if ($requestData->vehicle_owner_type == 'I' && $cpaSelected === true)
                        {
                        ...
                        $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_GDD_PROPOSAL_GENERATION'), $proposal_request_data, 'godigit', [
                            'enquiryId' => $proposal->user_product_journey_id,
                            'requestMethod' =>'post',
                            'productName'  => $productData->product_name,
                            'company'  => 'godigit',
                        ...
                        {
                            $proposal_response_data = json_decode($proposal_response_data, TRUE);
                            // dd($proposal_response_data);
                                ...
                                UserProposal::where('user_proposal_id' , $proposal->user_proposal_id)
                                    ...
                                    if(config('constants.IS_CKYC_ENABLED_GODIGIT') === 'Y')
                                    {
                                        if(config('GODIGIT_KYC_VERIFICATION_API_PREPAYMENT_ENABLE') === 'Y')
                                        {
                                            ...
                                            if (config('constants.IS_CKYC_ENABLED') == 'Y') 
                                            {
                                                $KycStatusApiResponse = GetKycStatusGoDIgit( $proposal->user_product_journey_id,$proposal_response_data['policyNumber'],  $productData->product_sub_type_name,$proposal->user_proposal_id,customEncrypt( $proposal->user_product_journey_id));
                                                if($KycStatusApiResponse['status'] !== true)
                                                {
                                                    CkycGodigitFailedCasesData::updateOrCreate(
                                                        ['user_product_journey_id' => $proposal->user_product_journey_id],
    ...
    public static function renewalSubmit($proposal, $request)
    {
        ...
        // $fetch_url = config('constants.IcConstants.godigit.GODIGIT_BIKE_MOTOR_FETCH_RENEWAL_URL').'D400626518';
        ...
        if(isset($response_data->error->errorCode) && $response_data->error->errorCode == '0')
        {
            ...
            $policy_start_date = date('Y-m-d', strtotime('+1 day', strtotime($prev_pol_end_date)));
            $policy_end_date = date('Y-m-d', strtotime('+1 year -1 day', strtotime($policy_start_date)));
            ...
            // Un comment below code for editing addons as per selection in DIGIT renewal service
            // if (!empty($additional['addons'])) {
            ...
            $premium_calculation_array = [
                ...
                "previousInsurer" => [
                    ...
                    "originalPreviousPolicyType" => $response_data->previousInsurer->originalPreviousPolicyType ?? '',
                ],
                "pospInfo" => [
                ...
                $get_response = getWsData(config('constants.IcConstants.godigit.GODIGIT_QUICK_QUOTE_PREMIUM'),$premium_calculation_array, 'godigit',
                [
                    'enquiryId' => $enquiryId,
                    'requestMethod' =>'post',
                    'productName'  => $productData->product_name,
                    ...
                    $proposal_data = [
                                    ...
                                    if(!empty($data))
                                    {
                                        ...
                                        {        
                                            ...
                                            // dd($proposal_response->policyNumber);
