...
class IffcoTokioSubmitProposal {
    ...
    public static function submit($proposal, $request) {
        ...
        if( $productData->zero_dep == '0') //$vehicle_age <= 5 &&
         {
            $is_zero_dep = 'Y';
        ...
        $model_config_premium = [
            ...
            'policy' => [
                ...
                'vehicle' => [
                    ...
                    'zcover' => ($tp_only) ? config('IC.IFFCO_TOKIO.V1.CAR.ZCOVER_BIKE_TP') : config('IC.IFFCO_TOKIO.V1.CAR.ZCOVER_BIKE_CO'), //constants.IcConstants.iffco_tokio.zcover_bike_tp //constants.IcConstants.iffco_tokio.zcover_bike_co
                ],
            ],
            'partner' =>
            [
        ...
        if($data) {
            ...
            if ($requestData->business_type == 'rollover' || $requestData->business_type == 'breakin') {
                ...
                foreach($coveragepremiumdetail as $k => $v) {
                    ...
                    if($coverage_name == 'IDV Basic') {
                        ...
                        $prm = self::calculateNBPremium(($v));
                        $consumable_value = round($prm['od']);
                    }
                    else if ($coverage_name == 'NCB Protection') {
                        $prm = self::calculateNBPremium(($v));
                        $ncb_protection_value = round($prm['od']);
                    }
                }
            ...
            $total_amount_payable = round($net_premium + $service_tax);
