...
class RelianceSubmitProposal
{
    ...
    public static function submit($proposal, $request)
    {
        ...
        }elseif(config('IC.RELIANCE.V1.CAR.IS_POS_TESTING_MODE_ENABLE') == 'Y') {
            $POSType = '2';
        ...
        if($coverage_res_data)
        {
            ...
            if(!isset($coverage_res_data->ErrorMessages))
            {
                ...
                if (trim($response->ErrorMessages) == '')
                {
                    ...
                    if (
                        ...
                        $leadArray = [
                            'LEADDESCRIPTION' => [
                                ...
                                'VENDORCODE' => config('IC.RELIANCE.V1.CAR.LEAD_VENDORCODE'),
                                'VENDORCODEVALUE' => config('IC.RELIANCE.V1.CAR.LEAD_VENDORCODEVALUE'),
                                'BASCODE' => config('IC.RELIANCE.V1.CAR.LEAD_BASCODE'),
                                'BASCODEVALUE' => config('IC.RELIANCE.V1.CAR.LEAD_BASCODEVALUE'),
                                'BASMOBILE' => config('IC.RELIANCE.V1.CAR.LEAD_BASMOBILE'),
                                'SMCODE' => config('IC.RELIANCE.V1.CAR.LEAD_SMCODE'),
                                'SMCODEVALUE' => config('IC.RELIANCE.V1.CAR.LEAD_SMCODEVALUE'),
                                'SMMOBILENO' => config('IC.RELIANCE.V1.CAR.LEAD_SMMOBILENO'),
                                'VEHICLETYPE' => 6,
                                ...
                                'RGICL_OFFICE' => config('IC.RELIANCE.V1.CAR.LEAD_RGICL_OFFICE'),
                                'RGICL_OFFICEVALUE' => config('IC.RELIANCE.V1.CAR.LEAD_RGICL_OFFICEVALUE'),
                                ...
                                'LEADCREATEDBYSYSTEM' => config('IC.RELIANCE.V1.CAR.LEAD_LEADCREATEDBYSYSTEM'),
                                'INTIMATORNAME' => config('IC.RELIANCE.V1.CAR.LEAD_INTIMATORNAME'),
                                'INTIMATORMOBILENO' => config('IC.RELIANCE.V1.CAR.LEAD_INTIMATORMOBILENO'),
                        ...
                        $requestArray = [
                            'soapenv:Header' => [
                                'tem:UserCredential' => [
                                    'tem:UserID' => config('IC.RELIANCE.V1.CAR.LEAD_USERID'),
                                    'tem:UserPassword' => config('IC.RELIANCE.V1.CAR.LEAD_USERPASSWORD')
                                ]
                            ],
                            'soapenv:Body' => [
                                'tem:InsertLead' => [
                        ...
                        $getResponse = getWsData(
                            config('IC.RELIANCE.V1.CAR.CAR_LEAD_CREATION_URL'),
                            $leadArray,
                            'reliance',
                            [
                                'section' => $productData->product_sub_type_code,
                    ...
                    $get_response = getWsData(
                        config('IC.RELIANCE.V1.CAR.END_POINT_URL_PROPOSAL'),
                        $premium_req_array,
                        'reliance',
                        [
                            'root_tag'      => 'PolicyDetails',
                        ...
                        foreach($proposal_resp->lstPricingResponse as $single)
                        {
                            if (isset($single->CoverageName)) {
                                ...
                                elseif ($single->CoverageName == 'Basic Liability') {
                                    $tppd = round(abs( (float) $single->Premium));
                                    ...
                                    $geog_Extension_TP_Premium = round(abs($single->Premium));
                                } elseif ($single->CoverageName == 'Return to Invoice'  && $enableRTI) {
                                    $RTIAddonPremium = round(abs($single->Premium));
                                
                                } elseif ($single->CoverageName == 'Liability to Employees') {
                                    $liability_to_employee_premium = round(abs($single->Premium));
                                }
                            }
                        }
    ...
    public static function renewal_data($proposal,$request,$enquiryId,$requestData,$productData)
    {
        ...
        $url=config('IC.RELIANCE.V1.CAR.END_POINT_URL_FETCH_RENEWAL');
        ...
        if($coverage_res_data)
        {
            ...
            if(!isset($coverage_res_data->ErrorMessages))
            {
                ...
                if (trim($response->ErrorMessages) == '')
                {
                    ...
                    $get_response = getWsData(
                        config('IC.RELIANCE.V1.CAR.END_POINT_URL_PROPOSAL_POST_INSPECTION'),
                        $premium_req_array,
                        'reliance',
                        [
                            'root_tag'      => 'PolicyDetails',
                    ...
                    if($proposal_resp->status == '1')
                    {
                        UserProposal::where('user_product_journey_id', $enquiryId)->update([
                            'policy_start_date' => date('d-m-Y', strtotime($policy_start_date)),
                            'policy_end_date' => date('d-m-Y', strtotime($policy_end_date)),
                            'final_payable_amount' => round($proposal_resp->FinalPremium),
                            'proposal_no' => $proposal_resp->ProposalNo
                        ]);
