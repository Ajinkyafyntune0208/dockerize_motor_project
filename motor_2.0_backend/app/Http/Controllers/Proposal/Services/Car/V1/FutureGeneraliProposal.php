...
class FutureGeneraliProposal
{
    ...
    public static function submit($proposal, $request)
    {
        ...
        $IsPos = 'N';
        $is_FG_pos_disabled = config('IC.FUTURE_GENERALI.V1.CAR.IS_POS_DISABLED');
        ...
        //$is_employee_enabled = config('IC.FUTURE_GENERALI.V1.CAR.IS_EMPLOYEE_ENABLED');
        $PanCardNo = '';
        ...
        else
        {
            ...
            if($is_pos_enabled == 'Y' && isset($pos_data->seller_type) && $pos_data->seller_type == 'P' && config('IC.FUTURE_GENERALI.V1.CAR.FUTURE_GENERALI_IS_NON_POS') != 'Y')
            {
                if($pos_data)
                {
                    $IsPos = 'Y';
        ...
        $quote_array = [
            ...
            'Risk'         => [
                ...
                'Addon'             =>(config('IC.FUTURE_GENERALI.V1.CAR.IS_ADDON_ENABLE')=='N' ? ['CoverCode' => ''] : ($addon!=null ? $addon : array_merge(['CoverCode'=>''],$addon)) ),
                'PreviousTPInsDtls' => [
        ...
        if ($data) {
            ...
            if (isset($quote_output['s:Body']['CreatePolicyResponse']['CreatePolicyResult']['Root']['Policy']))
            {
                ...
                if ($quote_output['Status'] == 'Fail') {
                    ...
                    $total_idv = ($premium_type == 'third_party') ? 0 : ($quote_output['VehicleIDV']);
                    $min_idv = ($total_idv * 0.9);
                    $max_idv = ($total_idv * 1.2);
...

                    {
                        ...
                        $inspection_data['companyId'] = config('IC.FUTURE_GENERALI.V1.CAR.COMPANY_ID_MOTOR');
                        $inspection_data['branchId'] = config('IC.FUTURE_GENERALI.V1.CAR.BRANCH_ID_MOTOR');
                        $inspection_data['refId'] = 'FG'.rand(00000,99999);
                        $inspection_data['appId'] = config('IC.FUTURE_GENERALI.V1.CAR.APP_ID_MOTOR');
                        ...
                        if($inspection_response_array)
                        {
                            ...
                            if($inspection_response_array_output['status']['code'] == 200)
                            {
                                ...
                                DB::table('cv_breakin_status')->insert([
                                    ...
                                    'breakin_check_url' => config('IC.FUTURE_GENERALI.V1.CAR.BREAKIN_CHECK_URL'),
    ...
    public static function renewalSubmit($proposal, $request)
    {
        ...
        $url = config('IC.FUTURE_GENERALI.V1.CAR.RENEWAL_FETCH_POLICY_DETAILS');
