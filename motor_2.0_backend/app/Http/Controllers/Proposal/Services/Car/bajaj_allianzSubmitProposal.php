    ...
    public static function submit($proposal, $request, $return_request = false)
    {
        ...
        if ($requestData->business_type == 'newbusiness') {
            ...
            if ($premium_type == 'comprehensive') {
                $policy_end_date =  today()->addYear(3)->subDay(1)->format('d-M-Y'); //policy end date is change by referring the git #32315
        ...
        $externalCNGKITSI = $NonElectricalaccessSI = $NonElectricalaccessSI = $ElectricalaccessSI = $LLtoPaidDriverYN = $PAforUnnamedPassenger = $PAforUnnamedPassengerSI = $LLtoPaidEMPYN = 0;
        ...
        $LLtoPaidEMPYN = $mmv_data->carrying_capacity - 1;
        $cover_data[] = [
            'typ:paramDesc' => 'LLE',
            'typ:paramRef' => 'LLE',
        ];
        ...
        if (in_array($premium_type, ['third_party', 'third_party_breakin'])) { // Only TP
            $product4digitCode = config("constants.motor.bajaj_allianz.PRODUCT_CODE_TP_BAJAJ_ALLIANZ_MOTOR");
        } else if ($BusinessType == '1') { // New Business
            $product4digitCode = config("constants.motor.bajaj_allianz.PRODUCT_CODE_BAJAJ_ALLIANZ_MOTOR_NEW");
            ...
            $product4digitCode = config("constants.motor.bajaj_allianz.PRODUCT_CODE_BAJAJ_ALLIANZ_MOTOR");
        }
        ...
        if (config('constants.posTesting.IS_POS_TESTING_MODE_ENABLE_BAJAJ') == 'Y') {
            $extCol40 = 'DNPPS5548E';
            $pUserId = ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') ? config("constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_USERNAME_POS_TP") : config("constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_USERNAME_POS");
        }
        ...
        if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
            $pUserId = config("constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_USERNAME_TP");
        }
        ...
        $data = [
            ...
            'soapenv:Body' => [
                'web:calculateMotorPremiumSig' => [
                    ...
                    'pPassword' => ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') ? config("constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_PASSWORD_TP") : config("constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_PASSWORD"),
                    ...
                    'pWeoMotPolicyIn_inout' => [
                        ...
                        'typ:registrationNo' => str_replace('-', '', implode('', $vehicale_registration_number)),
                    ...
                    'motExtraCover' => [
                        ...
                        'typ:noOfEmployeesLle' => $LLtoPaidEMPYN,
        ...
        if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
            $get_response = getWsData(config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_PROPOSAL_TP_URL'), $input_array, 'bajaj_allianz', $additional_data);
        } else {
            $get_response = getWsData(config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_PROPOSAL_URL'), $input_array, 'bajaj_allianz', $additional_data);
        }
        ...
        if ($service_response['pErrorCode_out'] == '0') {
                ...
                if ($premium_type == 'own_damage' && $requestData->is_claim == 'Y' && $productData->zero_dep == '1' && isset($covers['od'])) {
                    ...
                    if (isset($covers[0]))
                    {
                        foreach ($covers as $key => $cover) {
                            ...
                            } elseif (in_array($cover['typ:paramDesc'], ['1Bonus / Malus','Bonus / Malus'])) {
                            ...
                            elseif(in_array($cover['typ:paramDesc'], ['LL to employees of the insured', 'LL to employee of the insured'])){
                                $ll_paid_emp = ($cover['typ:act']);
                            }
                            elseif(in_array($cover['typ:paramDesc'], ['LL to employees of the insured', 'LL to employee of the insured'])){
                                $ll_paid_emp = ($cover['typ:act']);
                            }
                        ...
                        if (($productData->zero_dep == '0') && ($cover['typ:paramDesc'] === 'Depreciation Shield')) {
                            ...
                            $lpg_cng = ($cover['typ:od']);
                            $lpg_cng_tp = ($cover['typ:act']);
                        } elseif (in_array($cover['typ:paramDesc'], ['Commercial Discount', 'Commercial Discount3','7Commercial Discount'])) {
                            $other_discount = (abs($cover['typ:od']));
                        } elseif (in_array($cover['typ:paramDesc'], ['1Bonus / Malus','Bonus / Malus'])) {
                            $ncb_discount = (abs($cover['typ:od']));
                        } elseif ($cover['typ:paramDesc'] === 'Personal Baggage Cover') {
            ...
            $total_od = $basic_od + $electrical_accessories + $non_electrical_accessories + $lpg_cng;
            $od_discount = $ncb_discount + $other_discount + $voluntary_deductible + $antitheft_discount_amount;
            $total_discount = $od_discount + $restricted_tppd;
            ...
            if ($is_premium_to_be_stored) {
                ...
                UserProposal::where('user_product_journey_id', $enquiryId)
                    ...
                    ->update([
                        ...
                        'od_premium' => ($total_od - $od_discount),
            ...
            $issue_array = [
                'web:issuePolicy' => [
                    ...
                    'pCustDetails_inout' => [
                        ...
                        'typ:addLine2' => empty($getAddress['address_2']) ? '.' : $getAddress['address_2'],
                        'typ:addLine3' => empty($getAddress['address_3']) ? '.' : $getAddress['address_3'],
                    ...
                    'motExtraCover_inout' => [
                        ...
                        'typ:subImdcode' => '',
                        'typ:extraField1' => '',
                        'typ:extraField2' => $user_proposal['first_name'].' '.$user_proposal['last_name'],
                        'typ:extraField3' => '',
                    ],
                    'premiumDetails' => [
            ...
            if ((($requestData->business_type == 'breakin') && ($premium_type == "own_damage" || $premium_type != "third_party")) || $is_breakin) {
                ...
                // $pin_fields['bag:pFirstName'] = $proposal->first_name;
                // $pin_fields['bag:pLastName'] = $proposal->last_name;
                $pin_array = [
                    'x:Header' => [],
                    'x:Body' => [
                ...
                if ($tp_only == 'true' && $bajaj_new_tp_url == 'Y') {
                    $get_response = getWsData(config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_POLICY_ISSUE_TP_URL'), $input_array, 'bajaj_allianz', $additional_data);
                } else {
                    $get_response = getWsData(config('constants.motor.bajaj_allianz.BAJAJ_ALLIANZ_CAR_POLICY_ISSUE_URL'), $input_array, 'bajaj_allianz', $additional_data);
                }
                $response = $get_response['response'];
