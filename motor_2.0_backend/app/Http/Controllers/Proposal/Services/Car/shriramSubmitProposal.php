...
class shriramSubmitProposal
{
    ...
    public static function submitXML($proposal, $request)
    {
        ...
        foreach ($addons as $key => $value) {
            ...
            if (in_array('Key Replacement', $value)) {
                $KeyReplacementYN = ($pkg_selected == 'ADDON_01' ? 'Y' : 'N' );
            }
            ...
            if (in_array('Loss of Personal Belongings', $value)) {
                $LossOfPersonBelongYN = ($pkg_selected == 'ADDON_01' ? 'Y' : 'N' );
            }
        ...
        switch ($master_policy->premium_type_id) 
               {
                   case '1':
                       ...
                       $URL = $is_new ? config('constants.motor.shriram.NBQUOTE_URL') : config('constants.motor.shriram.NEW_PROPOSAL_URL');
                       $quote_log->ex_showroom_price_idv = $quote_log->ex_showroom_price_idv;
                       break;
                    case '4':
                   ...
                   case '2':
                       ...
                       $URL = $is_new ? config('constants.motor.shriram.RENEW_PROPOSAL_URL') : config('constants.motor.shriram.NEW_PROPOSAL_URL');
                       $quote_log->ex_showroom_price_idv = '';
                       break;
                    case '3':
            ...
            "soap:Body" => [
                $soapAction => [
                    ...
                    'objPolicyEntryETT' => [
                        ...
                        'PolicyType'        => $is_new ? 'MOT-PLT-014' : $policy_type,
                        ...
                        "RegNo1"            => $is_new ? "" : explode('-', $rto_code)[0],
                        "RegNo2"            => $is_new ? "" : explode('-', $rto_code)[1],
                        "RegNo3"            => $is_new ? "" : $vehicale_registration_number[2],
                        "RegNo4"            => $is_new ? "" : $vehicale_registration_number[3],
                        'RTOCode'           => $rto_code,
                        ...
                        'InBuiltCNGKitYN'       => $quote_data['fuel_type'] == 'CNG' ? '1' : '0', // maSTER and  fuel type
                        'VoluntaryExcess'       => $voluntary_insurer_discounts,
                        'Bangladesh'                => '0',
                        'Bhutan'                    => '0',
                        'SriLanka'                  => '0',
                        'Pakistan'                  => '0',
                        ...
                        'DepDeductWaiverYN'         => $zero_dep ? '1' : '0',
                        ...
                        'RSACover'                  => $RSACover,
                        'Eng_Protector'             => $is_new ? '0': $engine_protection,
                        'Consumables'               => $is_new ? '0': $consumable,
                        'TpFmDt'                    => $is_od ? $tp_start_date : '',
                        'TpToDt'                    => $is_od ? $tp_end_date : '',
                        'TpPolNo'                   => $is_od ? $prev_tp_policy_no:'',
                        'TpCompName'                => $is_od ? $prev_tp_comp_name : '',
                        'TpAddress'                 => $is_od ? $prev_tp_address : '',
        ...
        if (in_array($master_policy->premium_type_id, ['3', '6'])) {
            if (isset($input_array['soap:Body']['GenerateLTPvtCarProposal']['objPolicyEntryETT']['PAOwnerDriverExclusion']) && isset($input_array['soap:Body']['GenerateLTPvtCarProposal']['objPolicyEntryETT']['PAOwnerDriverExReason'])) {
                unset($input_array['soap:Body']['GenerateLTPvtCarProposal']['objPolicyEntryETT']['PAOwnerDriverExclusion']);
                unset($input_array['soap:Body']['GenerateLTPvtCarProposal']['objPolicyEntryETT']['PAOwnerDriverExReason']);
            }
        }
        //return $input_array;
        $additional_data = [
        ...
        $input_xml = ArrayToXml::convert($input_array, $root, false, 'utf-8');

        $get_response = getWsData($URL, $input_xml, $request['companyAlias'], $additional_data);
        ...
        if($proposalType == "RENEWAL"){
            $response = $response['soap:Body']['GenerateProposalResponse']['GenerateProposalResult'];
        }else{
            $response = isset($response['soap:Body']['GenerateLTPvtCarProposalResponse']['GenerateLTPvtCarProposalResult']) ? $response['soap:Body']['GenerateLTPvtCarProposalResponse']['GenerateLTPvtCarProposalResult'] : $response['soap:Body']['GenerateProposalResponse']['GenerateProposalResult'];
        }
        if ($response['ERROR_CODE'] == 0) {
            ...
            $addons_available = [
                'Nil Depreciation', 'Nil Depreciation Cover', 'ROAD SIDE ASSISTANCE', 'KEY REPLACEMENT', 'Engine Protector Cover', 'Engine Protector Loading', 'Consumables Cover', 'Consumables Loading', 'LOSS OF PERSONAL BELONGINGS'
            ];
            $discounts_available = [
                'VOLUNTARY EXCESS DISCOUNT-IMT-22A', 'DETARIFF DISCOUNT ON BASIC OD', 'DETARIFF DISCOUNT ON BASIC OD - 1 YEAR', 'NO CLAIM BONUS-GR27', 'GR39A-TPPD COVER', 'GR39A-TPPD COVER - 1 YEAR', 'GR39A-TPPD COVER - 2 YEAR', 'GR39A-TPPD COVER - 3 YEAR'
            ];
    ...
    public static function renewalSubmit($proposal, $request)
    {
        ...
        {            
            ...
            foreach ($premium_lists as $key => $premium_list) {
                ...
                // }
                else if (in_array($premium_list['CoverDesc'], ['GR36A-PA FOR OWNER DRIVER', 'GR36A-PA FOR OWNER DRIVER - 1 YEAR'])) {
                    $pa_owner = $premium_list['Premium'];
                }
    ...
    public static function submitJSON($proposal, $request)
    {
        ...
        $productData = getProductDataByIc($request['policyId']);
        $proposal->gender = (strtolower($proposal->gender) == "male" || $proposal->gender == "M") ? "M" : "F";
        $insuranceCompany = DB::table('previous_insurer_lists')
        ->where('name',$requestData->previous_insurer)
        ->where('company_alias','shriram')
        ->pluck('code')
        ->first();
        // if(($requestData->business_type != "newbusiness") && ($productData->zero_dep == '0') && ($requestData->zero_dep_in_last_policy != 'Y'))
        // {
        ...
        if($requestData->business_type == 'Breakin' || $requestData->business_type == 'breakin'){
            $tp_start_date = $policy_start_date = Carbon::parse(today())/*->addDay(2)*/->format('d-M-Y');
            $tp_end_date = $policy_end_date = Carbon::parse($policy_start_date)->addYear(1)->subDay(1)->format('d-M-Y');
        }
        if($requestData->business_type == 'rollover' && (in_array($premium_type , ['comprehensive','third_party']))){
        ...
        foreach ($addons as $key => $value) {
            ...
            if (in_array('Loss of Personal Belongings', $value) && $interval->y < 5) {
        ...
        if(isset($request['declaredAddons'])){
            if($request['declaredAddons']['zeroDepreciation']) {
        ...
        if($NilDepreciationCoverYN == 'YES' && $PreviousNilDepreciation != 1){
            $is_breakin = 'Y';
        }


        // if (in_array('Zero Depreciation', $value) && $car_age<=5) {
            //     // 
            //     $NilDepreciationCoverYN = "YES";
            //     $PreviousNilDepreciation = 1; // addon
            // }
        ...
        $input_array = [
            "objPolicyEntryETT" => [
                ...
                "InsuredPrefix" => ($requestData->vehicle_owner_type == 'I') ? $insured_prefix : "3", //chnaged by #34288 
                ...
                "PreviousInsurer" => $is_new ? "" :  $proposal->previous_insurance_company,//$insuranceCompany,
                "PreviousPolicyFromDt" => $is_new ? "" : $PreviousPolicyFromDt,//"25-JAN-2021",
                "PreviousPolicyToDt" => $is_new ? "" : $PreviousPolicyToDt,//"24-JAN-2022",
                "PreviousPolicySI" => "",
                "PreviousPolicyClaimYN" => $requestData->is_claim == 'Y' ? '1' : '0', 
                "PreviousPolicyUWYear" => "",
        ...
        if($rto_code == 'AP-39')
        {
            $input_array['objPolicyEntryETT']['RTOCity'] = 'Prakasam';
        }
        ...
        if($is_not_sure_case)
        {
            $input_array['objPolicyEntryETT']['ProposalType']           =  'RENEWAL.WO.PRV INS DTL';
            $input_array['objPolicyEntryETT']['PreviousPolicyNo']       =  '';
            $input_array['objPolicyEntryETT']['PreviousInsurer']        =  '';
            $input_array['objPolicyEntryETT']['PreviousPolicyFromDt']   =  '';
            $input_array['objPolicyEntryETT']['PreviousPolicyToDt']     =  '';
        }
        if($requestData->vehicle_owner_type == 'C'){
            $input_array['objPolicyEntryETT']['NoEmpCoverLL'] = '1';
        ...
        $str = $quote_response['GenerateProposalResult']['ERROR_DESC'];

        $breakin_case = (preg_match($pattern, $str));
       
        if ((isset($quote_response['MessageResult']['Result']) && $quote_response['MessageResult']['Result'] == "Success") || ($get_response['status_code'] == 200 && !empty($quote_response['GenerateProposalResult']['PROPOSAL_NO']) && $breakin_case == 1)) {
            $idv = $quote_response['GenerateProposalResult']['VehicleIDV'];
            $quote_response = $quote_response['GenerateProposalResult'];
            $proposalNo = $quote_response['PROPOSAL_NO'];
            ...
            // dd($quote_response['CoverDtlList']);
            ...
            //Breakin call
            if ((in_array($requestData->business_type,['breakin','Breakin']) && $premium_type != 'third_party') || $is_breakin == 'Y'){
                $isBreakin = 'Y';
                 ...
                 $leadArray = [
                     ...
                     'INTIMATIONPURPOSE' => (($NilDepreciationCoverYN == 'YES' && $PreviousNilDepreciation == 1) || ($NilDepreciationCoverYN == 'YES' && $PreviousNilDepreciation != 1)) ? "Break-In with Nil Dep." : "Break-In",
             ...
             $finalleadResponse = json_decode( $leadResponse, true);
                     ...
                     $inspectionId = $matches[1];}
                        ...
                        'webservice_id' => $getResponse['webservice_id'],
                        'table' => $getResponse['table'],
                        'message' => 'Breakin Id Already created and wait until approved/rejected/cancelled'
                        ];
                     }
                    ...
                    ->update([
                        ...
                        'electrical_accessories'    => $ElectricalaccessSI,
                        'non_electrical_accessories'=> $NonElectricalaccessSI,
                        'additional_details_data'  => json_encode($input_array)
                    ]);
