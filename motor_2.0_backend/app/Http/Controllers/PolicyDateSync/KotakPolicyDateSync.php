...
class KotakPolicyDateSync extends Controller
{
    public function syncDetails($enquiryId, $data)
    {
        ...
        foreach ($logs as $log) {
            try {
                ...
                if (!empty($response['vPolicyStartDate'])) {
                    $startDate = str_replace('/', '-', $response['vPolicyStartDate']);
                    $startDate = date('d-m-Y', strtotime($startDate));

                    $endDate = str_replace('/', '-', $response['vPolicyEndDate']);
