...
class MagmaPolicyDateSync extends Controller
{
    public function syncDetails($enquiryId, $data)
    {
        ...
        foreach ($logs as $log) {
            try {
                ...
                if (!empty($response['GeneralProposalInformation']['PolicyEffectiveFromDate'])) {
                    $startDate = str_replace('/', '-', $response['GeneralProposalInformation']['PolicyEffectiveFromDate']);
                    $startDate = date('d-m-Y', strtotime($startDate));

                    $endDate = str_replace('/', '-', $response['GeneralProposalInformation']['PolicyEffectiveToDate']);
