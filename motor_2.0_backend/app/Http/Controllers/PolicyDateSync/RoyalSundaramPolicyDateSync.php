...
class RoyalSundaramPolicyDateSync extends Controller
{
    public function syncDetails($enquiryId, $data)
    {
        ...
        $logs = \App\Models\WebServiceRequestResponse::select('response', 'enquiry_id', 'id')
            ...
            ->whereIn('company', ['royal_sundaram'])
        ...
        foreach ($logs as $log) {
            try {
                ...
                if (!empty($response['PREMIUMDETAILS']['DATA']['POLICY_START_DATE'])) {
                    $startDate = str_replace('/', '-', $response['PREMIUMDETAILS']['DATA']['POLICY_START_DATE']);
                    $startDate = date('d-m-Y', strtotime($startDate));

                    $endDate = str_replace('/', '-', $response['PREMIUMDETAILS']['DATA']['POLICY_EXPIRY_DATE']);
