...
class PaymentGatewayConfigurationController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->method() == 'GET') {
            return view('admin_lte.payment-gateway-config.setting', compact('gatewayList'));
        } else {
    ...
    public function globalConfig(Request $request)
    {
        ...
        if ($request->method() == 'GET') {
            return view('admin_lte.payment-gateway-config.global-config', compact('gatewayList'));
        } else {
            $validator = Validator::make($request->all(), [
                'paymentGateway' => 'required|in:' . implode(',', array_keys($gatewayList))
            ]);
    ...
    public function icWiseConfig(Request $request)
    {
        ...
        if ($request->method() == 'GET') {
            return view('admin_lte.payment-gateway-config.ic-wise-config', compact('gatewayList', 'ics'));
        } else {
            $validator = Validator::make($request->all(), [
                'paymentGateway' => 'required|in:' . implode(',', array_keys($gatewayList)),
