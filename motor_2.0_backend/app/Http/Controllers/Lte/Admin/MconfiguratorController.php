...
class MconfiguratorController extends Controller
{
    ...
    {
        if (!auth()->user()->can('master_configurator.list')) {
        ...
        $company = DB::table('master_company')
                        ...
                        ->pluck('company_alias')
                        ->toArray();
        return view ('admin_lte.master_configurator.ProposalVal',compact('data'))->with('comp',$company);
    }
    public function onboardingShow()
    {
        ...
        //     return view ('admin_lte.master_configurator.Onboarding',compact('broker'));
        // }else {
            $bname = 'Current';
            ...
            if(!$data['status']){
                ...
                return view ('admin_lte.master_configurator.OnboardingShow',compact('bname','data'))->with('error','Something went wrong while sending request to api! '.'Server Response Code: '.$data['status']);
            ...
            if ($data == [] || null ){
                ...
                return view ('admin_lte.master_configurator.OnboardingShow',compact('bname','data','warning'));
            ...
            $company = DB::table('master_company')
                ...
                $titlExistingData = BrokerConfigAsset::select('value')->where('key', 'title_description')->get();
                $titlExistingData = $titlExistingData->first() ?? $titlExistingData->toArray();

                return view ('admin_lte.master_configurator.OnboardingShow',compact('bname','data', 'brokerConfigAsset' , 'titlExistingData'))->with('comp',$company);
        }
    ...
    public function onboardingFetch(Request $request)
    {
        ...
        return view ('admin_lte.master_configurator.OnboardingShow',compact('bname','data'))->with('comp',$company);
    ...
    public function onboardingStore($broker,Request $request)
    {
        ...
        if($request->preventRegistrationAutoSubmit) {
            $changedbrokerconfig['preventRegistrationAutoSubmit'] = true;
        }
    ...
    public function fieldShow(){
        ...
        return view ('admin_lte.master_configurator.FieldConfig');
    ...
    public function otpShow(){
        ...
        return view ('admin_lte.master_configurator.OtpConfig');
    }
    ...
    public function brokerScripts(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'scriptType.*' => 'required|in:code,url',
        ...
        foreach ($input['brokerScript'] as $key => $value) {
            $temp = [
                ...
                'type' => $input['scriptType'][$key]
            ];
            if ($temp['type'] == 'url') {
                $temp['attributes'] = base64_encode($input['brokerScriptAttributes'][$key] ?? null);
            }
