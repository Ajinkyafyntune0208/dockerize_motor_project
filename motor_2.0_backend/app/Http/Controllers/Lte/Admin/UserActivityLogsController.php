...
class UserActivityLogsController extends Controller
{
    ...
    {
        if (!auth()->user()->can('user_activity_logs.list')) {
        ...
        return view('admin_lte.activityLogs.index', compact('activities', 'types', 'operations', 'users'));
    }
    ...
    public function show($id)
    {
        ...
        if(auth()->user()->hasRole('Admin') || ($record['user_id'] ?? 0) == auth()->user()->id) {
            return view('admin_lte.activityLogs.show', compact('pageOptions', 'record'));
        }
