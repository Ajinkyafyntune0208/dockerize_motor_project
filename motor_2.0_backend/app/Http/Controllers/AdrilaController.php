...
class AdrilaController extends Controller
{
    public static function adrilaProposalRedirection(Request $request)
    {
        ...
        try {
            ...
            if($rcno[0]=='DL'){
                $request->vehicleRegNo =$rcno[0].((int)$rcno[1]*1).$rcno[2].$rcno[3];
            } else {
                $request->vehicleRegNo = preg_replace('/-/', '', $request->registration_no);
            }
            $request_array = [
                'application_name' => 'ACE',
                'rc_no' => $request->vehicleRegNo,
                'api_type' => 'PROPLUS',
                'rc_source' => 1,
            ...
            $insudetails = Http::withHeaders([
                'Authorization' => config('ADRILA_API_KEY'),
            ])->post(config('ADRILA_END_POINT_URL'), $request_array);
            ...
            FastlaneRequestResponse::insert([
                ...
                'endpoint_url' => config('ADRILA_END_POINT_URL'),
            ...
            else if (self::findMyWord($response['data']['rc_vh_class_desc'], 'LMV') || self::findMyWord($response['data']['rc_vh_class_desc'], 'Motor Car')) {
            ...
            $vehicle_details = self::getAdrilaVehicleDetails($enquiryId, $request->registration_no, $type, $response, $insudetails, $journey_type = 'adrila', $pro = true);
    ...
    public static function adrilaJourneyRedirection(Request $request)
    {
        $validate = $request->validate([
            'enquiryId' => 'required',
            'registration_no' => 'required',
       ...
       if(empty($variantnewdata)){
        ...
        $user_product_journey = UserProductJourney::where('user_product_journey_id', $enquiryId)->update($user_product_journey);
        $vehicle_details = self::getAdrilaVehicleDetails($enquiryId, $request->registration_no, $type, $response, $insudetails, $journey_type = 'adrila', $pro = false);
        if ($vehicle_details['status'] == 101) {
            return response()->json([
                "data" => [
                    "status" => 101,
        ...
        if(config('ADRILA_SECTION_ENABLE')=='Y'){
    ...
    public static function getAdrilaVehicleDetails($enquiryId, $new_rc_number, $type, $response, $insudetails, $journey_type, $pro = false)
    {
     ...
     $address = '';
        ...
        if (isset($response['data']['rc_present_address']) && $response['data']['rc_present_address'] != null) {
            ...
            $return_data['vehicle_color'] = removeSpecialCharactersFromString($insudetails['data']['rc_color'], true);
            ...
            if (!empty($response['data']['pin_code']) && $response['data']['pin_code'] != "999999") {
                $pincode = $response['data']['pin_code'];
            }
            $return_data['pincode'] = $pincode;
        }

        if (empty($return_data['previousInsurer'] ?? null) || empty($return_data['previousInsurerCode'] ?? null)) {
            $corporateData = CorporateVehiclesQuotesRequest::where('user_product_journey_id', $enquiryId)->first();
            if (!empty($corporateData) && !empty($corporateData->previous_insurer_code)) {
                $return_data['previousInsurer'] = $corporateData->previous_insurer;
                $return_data['previousInsurerCode'] = $corporateData->previous_insurer_code;
            }
        }
        ...
        if ($pro) {
            UserProposal::updateOrCreate(
                ...
                [
                    ...
                    'vehicle_color' => removeSpecialCharactersFromString($insudetails['data']['rc_color'], true),
            ...
            // adding validation for previous_insurance_company against empty value and not allowed strings
            if(!empty($insudetails['data']['rc_insurance_policy_no']) && isValidPolicyNumber($insudetails['data']['rc_insurance_policy_no']))
            {
                $update_array['previous_policy_number'] = $insudetails['data']['rc_insurance_policy_no'];
                $update_data = true;
            }

            if(!empty($insudetails['data']['rc_insurance_comp']) && !in_array($insudetails['data']['rc_insurance_comp'], ["-", "--", "/", "//"]))
            {
                $update_array['previous_insurance_company'] = $insudetails['data']['rc_insurance_comp'];
                $update_array['insurance_company_name'] = $insudetails['data']['rc_insurance_comp'];
                $update_data = true;
            }

            if(!empty($curlResponse['additional_details']['address_line1']))
            {
                $update_array['address_line1'] = $curlResponse['additional_details']['address_line1'];
                 $update_data = true;
            }

            if(!empty($curlResponse['additional_details']['address_line2']))
            {
                $update_array['address_line2'] = $curlResponse['additional_details']['address_line2'];
                 $update_data = true;
            }

            if(!empty($curlResponse['additional_details']['address_line3']))
            {
                $update_array['address_line3'] = $curlResponse['additional_details']['address_line3'];
                 $update_data = true;
            }

            if(!empty($curlResponse['additional_details']['pincode']) && $curlResponse['additional_details']['pincode'] != "999999")
            {
                $update_array['pincode'] = $curlResponse['additional_details']['pincode'];
                 $update_data = true;
            }

            if($update_data)
            {
                UserProposal::updateOrCreate(['user_product_journey_id' => $enquiryId], $update_array );
            }
            $user_product_journey = [
